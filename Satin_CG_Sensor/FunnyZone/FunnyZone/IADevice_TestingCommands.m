...
//function for crc test add by Jockey
-(NSNumber *)SEND_AND_READ_COMMANDS:(NSDictionary*)dicSendContents
                       RETURN_VALUE:(NSMutableString*)szReturnValue
{
    BOOL SEND=YES;
    BOOL READ=YES;
    BOOL CRC=YES;
    NSArray *ary = [NSArray arrayWithArray:[dicSendContents objectForKey:@"ArrayCommands"]];
    NSArray *ary1 = [NSArray arrayWithArray:[dicSendContents objectForKey:@"ArrayBEGIN"]];
    if ([ary count]==[ary1 count]&&ary&&ary1&&SEND&READ&CRC) {
        for (int i =0; i<[ary count]; i++) {
            NSDictionary *command = [NSDictionary dictionaryWithObjectsAndKeys:[ary objectAtIndex:i],@"STRING",[dicSendContents objectForKey:@"TARGET"],@"TARGET",nil];
            SEND = [[self SEND_COMMAND:command]boolValue];
            SEND &=SEND;
            ATSDebug(@"Send command %@",SEND?@"PASS":@"FAIL");
            if (SEND) {
                NSMutableDictionary *readcommand = [dicSendContents objectForKey:@"ReadCommandSetting"];
                [readcommand setObject:[ary1 objectAtIndex:i] forKey:@"BEGIN"];
                READ = [[self READ_COMMAND:readcommand RETURN_VALUE:szReturnValue]boolValue];
                READ &=READ;
                ATSDebug(@"Read command %@",READ?@"PASS":@"FAIL");
                if (READ) {
                    NSDictionary *memorykey = [dicSendContents objectForKey:@"MemoryKey"];
                    [self MEMORY_VALUE_FOR_KEY:memorykey RETURN_VALUE:szReturnValue];
                    NSDictionary *calsetting = [dicSendContents objectForKey:@"CAL_CRCSetting"];
                    CRC  = [[self Cal_CRC_Value:calsetting RETURN_VALUE:szReturnValue]boolValue];
                    CRC &=CRC;
                    ATSDebug(@"CRC command %@",CRC?@"PASS":@"FAIL");
                }
                else
                {
                    break;
        ...
        return [NSNumber numberWithBool:SEND&READ&CRC];
    }
    else
    {
        ATSDBgLog(@"Script file format wrong");
        return [NSNumber numberWithBool:NO];
    }
}
...
-(NSNumber*)SEND_COMMAND:(NSDictionary*)dicSendContents
{
    if (!dicSendContents || [dicSendContents count] == 0)
	{
        ...
        alert.messageText = [NSString stringWithFormat:@"警告(Warning) (Slot:%@)",m_szPortIndex];
        alert.informativeText = @"SEND_COMMAND函数没有参数。(There're no parameters for SEND_COMMAND!)";
    ...
    // For D10 P1 stage, need set 400 mircosecond between every bity sent to unit.
    NSInteger   iIntervalTime	= 400;
		   ...
		   RETURN_VALUE:(NSMutableString *)szReturn
{
    ...
    {
        if([m_dicMemoryValues objectForKey:strAttributeKey])
        {
            NSMutableString	*strAttribute	= [[NSMutableString alloc] init];
            [strAttribute setString:[NSString stringWithFormat:@"%@", [m_dicMemoryValues objectForKey:strAttributeKey]]];
            if ([strAttribute isEqualToString:@""])
            {
                ATSDebug(@"UPLOADATTRI => no need set attribute for key [%@]",
						 strAttributeKey);
                [strAttribute release];
                return [NSNumber numberWithBool:YES];
                
            }
            else
            {
                if ([strAttribute ContainString:@"Kirin"])
                {
                    [strAttribute setString:[strAttribute stringByReplacingOccurrencesOfString:@"Kirin" withString:@"D10"]];
                    NSString *strConfig = [m_dicMemoryValues objectForKey:@"config"];
                    if (strConfig == nil ||
                        [strConfig isEqualToString:@""] ||
                        [[strConfig lowercaseString] isEqualToString:@"nill"] ||
                        [[strConfig lowercaseString] isEqualToString:@"null"])
                    {
                        NSDictionary *dicQueryItem = [NSDictionary dictionaryWithObjectsAndKeys:@"",@"config", nil];
                        NSDictionary *dicContents = [NSDictionary dictionaryWithObjectsAndKeys:dicQueryItem,@"QueryItems",nil];
                       if([self QUERY_SFIS:dicContents RETURN_VALUE:szReturn])
                       {
                         strConfig = [m_dicMemoryValues objectForKey:@"config"];
                       }
                       else
                           ATSDebug(@"Can not get config from SFIS");
                    }
                    NSString *strType = [strConfig subByRegex:@".*?/.*?/.*?/(.)\\w*/.+" name:nil error:nil];
                    if ([strType isEqualToString:@"3"])
                    {
                        [strAttribute setString:[strAttribute stringByReplacingOccurrencesOfString:@"D10" withString:@"D101"]];
                        ATSDebug(@"This unit is D101, config is [%@]", strType);
                    }
                }
                [m_objPudding SetQT_Attributes:strAttribute
                                           Key:[strAttributeKey uppercaseString]];
				 ...
				 RETURN_VALUE:(NSMutableString *)szReturn
{
    ...
    BOOL		bNOWriteToCsv	= [dictUploadContents valueForKey:kFunnyZoneNOWriteCsvToLogFile]? [[dictUploadContents valueForKey:kFunnyZoneNOWriteCsvToLogFile] boolValue] : YES;
    ...
    BOOL        bTestResult         = [dictUploadContents objectForKey:@"Result"]? [[dictUploadContents objectForKey:@"Result"] boolValue] : YES;
					...
					RETURN_VALUE:(NSMutableString*)szReturn
{
    ...
    if (m_bOfflineDisablePudding || m_bValidationDisablePudding)
    {
        [szReturn setString:[NSString stringWithFormat:@"%ld",(long)nPosition]];
        return [NSNumber numberWithBool:YES];
    }
...
-(NSNumber *)AMIOK_CHECK:(NSDictionary *)dicSubSetting
			RETURN_VALUE:(NSMutableString *)szReturnValue
{
    if (m_bOfflineDisablePudding || m_bValidationDisablePudding || gbAuditMode)
	{
		[szReturnValue setString:@"PASS"];
        return [NSNumber numberWithBool:YES];
    }
    ...
    else
    {
        ...
        NSAlert *alert = [[NSAlert alloc]init];
        [alert setMessageText:@"Am I OK Error"];
        [alert setInformativeText:strErrorMsg];
                 ...
                 RETURN_VALUE:(NSMutableString*)szReturnValue
{
    ...
    if (![szCatchedResult isKindOfClass:[NSString class]]
		|| [szCatchedResult isEqualToString:@""])
    {
        ATSDebug(@"CATCH_VALUE => Catch Fail : Can't Find location %ld or length %ld.",
				 (long)iLocation,(long)iLength);
        if(bData)
        {
            [szReturnValue setString:kFZ_99999_Value_Issue];
            ATSDebug(@"-99999issue");
    ...
    if(nil == dicArray) //return string value by Row and Column
    {
        if([dicpara valueForKey:@"ROW"] && [dicpara valueForKey:@"COLUMN"])
        {
            ...
            if (abs(iRow) < [arrRow count])
            {
                ...
                NSInteger		iRowTemp	= (iRow < 0) ? ([arrRow count] + iRow) : iRow;
                ...
                if (iColumn < [arrColumn count])
                {
                    ...
                    ATSDebug(@"the iColumn%i > the count %lu",
							 iColumn,(unsigned long)[arrColumn count]);
            }
        }
        if (bData)
        {
		  ...
		  RETURN_VALUE:(NSMutableString*)szReturnValue
{
    ...
    NSString	*szMPSpec			= @"{NA}";
    // NSString	*szCoFSpec			= @"{NA}";
    ...
    if (dicForCommon != nil)
    {
        ...
        szMPSpec	= [dicForCommon objectForKey:m_strSpecName];
        [self TransformKeyToValue:szMPSpec
                      returnValue:&szMPSpec];
        
        NSArray *arrSpec = [szMPSpec componentsSeparatedByString:@";cof"];
        for (NSInteger i = 0; i<[arrSpec count]; i++)
        {
            NSString *strSpec = [arrSpec objectAtIndex:i];
            //MP spec
            if (i == 0)
            {
                NSArray *arrSubSpec = [strSpec componentsSeparatedByString:@" || "];
                for (NSString *strSubSpec in arrSubSpec)
                {
                    bJudgeResult   |= [self JudgeSpec:strSubSpec
                    ...
                    if (!bUnShowFlag)
                    {
                        @synchronized(m_dicMemoryValues)
                        {
                            [m_dicMemoryValues setObject:strSubSpec forKey:kFZ_MP_TestLimit];
                            [m_dicMemoryValues setObject:strSubSpec
            ...
            {
                NSArray *arrCofSubSpec = [strSpec componentsSeparatedByString:@" || "];
                [m_dicMemoryValues setObject:[arrCofSubSpec objectAtIndex:0] forKey:kFZ_CoF_TestLimit];
                if (!bJudgeResult)
                {
                    [m_muArrayLocalCofTest addObject:[m_dicMemoryValues objectForKey:kFunnyZoneCurrentItemName]];
                    for (NSString *strCofSubSpec in arrCofSubSpec)
                    {
                        bJudgeResult   |= [self JudgeSpec:strCofSubSpec
                                             return_value:szCatchedValue
                                          caseInsensitive:bCaseMode];
                        if (!bUnShowFlag)
                        {
                            @synchronized(m_dicMemoryValues)
                            {
                                [m_dicMemoryValues setObject:strCofSubSpec forKey:kFZ_CoF_TestLimit];
                                [m_dicMemoryValues setObject:strCofSubSpec
                                                      forKey:kFZ_TestLimit];
                            }
                        }
                        if (bJudgeResult) {
...
//Add by York 2015.6.23 Judge config spec and spec's spec just for CSN
-(NSNumber*)JUDGE_CSN_SPEC:(NSDictionary*)dicSpecSettings
              RETURN_VALUE:(NSMutableString*)szReturnValue
{
    NSString	*szSpec			= @"";
    bool bJudgeResult           = NO;
    BOOL	bCaseMode			= [[dicSpecSettings objectForKey:kFZ_Script_JudgeMode]
                                   boolValue];
    NSString	*szCatchedValue	= [NSString stringWithFormat:@"%@",szReturnValue];
    NSDictionary	*dicForCommon	= [dicSpecSettings objectForKey:kFZ_Script_JudgeCommonSpec];
    //else common spec judge, defualt spec name is P_LimitBlack.
    ...
    bJudgeResult    = [self JudgeSpec:szSpec
                         return_value:szCatchedValue
                      caseInsensitive:bCaseMode];
    if (bJudgeResult && [szSpec isNotEqualTo:@"{NA}"])
    {
        [szReturnValue setString:@"Need upload"];
        return [NSNumber numberWithBool:YES];
    }
    [szReturnValue setString:@"No need upload"];
    return [NSNumber numberWithBool:NO];
}
				  ...
				  RETURN_VALUE:(NSMutableString *)szReturn;
{
    ...
    [self TransformKeyToValue:szUnit
                  returnValue:&szUnit];
    if (nil != [dicSettingItems objectForKey:kFunnyZoneShowCoFONUI])
    {
        NSNumber    *numNoShowUI    = [NSNumber numberWithBool:[[dicSettingItems objectForKey:kFunnyZoneShowCoFONUI] boolValue]];
        @synchronized(m_dicMemoryValues)
        {
            [m_dicMemoryValues setObject:numNoShowUI forKey:kFunnyZoneShowCoFONUI];
        }
    }
		   ...
		   RETURN_VALUE:(NSMutableString *)szReturnValue
{
    ...
    if (bRet)
    {
        ...
        if (bRet)
        {
			...
			{
				int	iDecimalPlaces	= [[dicExpression objectForKey:@"DecimalPlaces"] intValue];
...
-(NSNumber *)CALCULATOR_FORCOMPASS:(NSDictionary *)dicExpression
                      RETURN_VALUE:(NSMutableString *)szReturnValue
{
    NSString *key1 = [dicExpression objectForKey:@"Key1"];
    NSString *key2 = [dicExpression objectForKey:@"Key2"];
    double i1 = [[m_dicMemoryValues objectForKey:key1]doubleValue];
    double i2 = [[m_dicMemoryValues objectForKey:key2]doubleValue];
    ATSDebug(@"number1 is %f,number2 is %f",i1,i2);
    [szReturnValue setString:[NSString stringWithFormat:@"%.4f",i1/i2]];
    return [NSNumber numberWithBool:YES];
}
						...
						RETURN_VALUE:(NSMutableString *)szReturnValue
{
	...
	{
		ATSDebug(@"[%lu] + [%lu] Beyond length [%lu]",
				 ulBegin, ulLength, (unsigned long)[strSource length]);
		return [NSNumber numberWithBool:NO];
	}
	...
	NSUInteger		Dec		= 0;
    bool	bFlag	= NO;
	for(NSInteger i=[strCut length] - 1; i>=0; i--)
	{
		...
		Dec		+= (NSUInteger)(Digit * pow(iSourceSystem, [strCut length] - i - 1));
        bFlag	= YES;
	}
	...
	strResult	= [NSString stringWithFormat:@"%@%@",
				   [self Int2Char:(int)Dec], strResult];
	
	//	Hold enough places
	// Default place is result length
	unsigned long	uiPlace	= ([[dictSettings objectForKey:@"PLACE"]
								isKindOfClass:[NSNumber class]]
	...
	{
		NSInteger	iLength	= [strResult length];
			 ...
			 RETURN_VALUE:(NSMutableString*)strReturnValue
{
    [m_dicMemoryValues setObject:[[NSDate date]
								  descriptionWithCalendarFormat:@"YYYYMMddHHmmss"
								  timeZone:nil
								  locale:nil]
						  forKey:kFZ_NowTime];
    return [NSNumber numberWithBool:YES];
...
//add for IQC DEV-6 , loop send commands
-(NSNumber *)LOOP_COMMAND:(NSDictionary *)dicPara
             RETURN_VALUE:(NSMutableString *)szReturnValue
{
    FlagTrgOrg =[NSNumber numberWithBool:YES];
    FlagTrgFnl =[NSNumber numberWithBool:YES];
    FlagTrFnl = YES;
    FlagTrOrg = YES;
    
    NSDictionary * dic_firstSendCommands =[dicPara objectForKey:@"SEND_COMMAND:"];
    NSDictionary * dic_firstReadCommands =[dicPara objectForKey:@"READ_COMMAND:RETURN_VALUE:"];
    NSString * Regex    =[NSString string];
    NSString * SPEC     =[NSString string];
    Regex ? Regex =[dicPara objectForKey:@"REGEX"] : NULL;
    SPEC ? SPEC =[dicPara objectForKey:@"SPEC"] : NULL;
    int MaxTest =[[dicPara objectForKey:@"MaxTime"] intValue];
    // define two values to store every register
    
    NSDictionary * dic_ResendCommands =[[dicPara objectForKey:@"FAIL_RESEND"] objectForKey:@"SEND_COMMAND:"];
    NSDictionary * dic_ReResponse =[[dicPara objectForKey:@"FAIL_RESEND"] objectForKey:@"READ_COMMAND:RETURN_VALUE:"];
  
    // for touch load_firmware ,which don't need check spec.
    if (SPEC == NULL && Regex == NULL)
    {
        for (int testTime = 0; testTime < MaxTest; testTime ++) {
            if ([[self SEND_COMMAND:dic_firstSendCommands] boolValue] && [[self READ_COMMAND:dic_firstReadCommands RETURN_VALUE:szReturnValue] boolValue])
            {
                ATSDBgLog( @"NO. %d ,SEND_COMMAND:%@, Read_Command:%@ >>> OK",testTime+1, [dic_firstSendCommands objectForKey:@"STRING"],szReturnValue);
                
                return [NSNumber numberWithBool:YES];
            }
            
        }
        FlagTchFWNoErr =[NSNumber numberWithBool:NO];
        [m_dicMemoryValues setObject:FlagTchFWNoErr forKey:@"FlagTchFWNoErr"];
        return [NSNumber numberWithBool:NO];
    }
    // for register value check
    else
    {
        for (int testTime = 0; testTime < MaxTest; testTime ++) {
            // if send_command and read_command are ok
            if ([[self SEND_COMMAND:dic_firstSendCommands] boolValue] && [[self READ_COMMAND:dic_firstReadCommands RETURN_VALUE:szReturnValue] boolValue])
            {
                NSString	*strSub	= [szReturnValue subByRegex:Regex
                                                        name:nil
                                                       error:nil];
                BOOL bJudgeResult    = [self JudgeSpec:SPEC
                                          return_value:strSub
                                       caseInsensitive:NO];
                // response OK, get what we need.
                if (bJudgeResult)
                {
                    ATSDBgLog( @"NO. %d ,SEND_COMMAND:%@, Read_Command:%@ >>>OK",testTime+1, [dic_firstSendCommands objectForKey:@"STRING"],strSub);
                ...
                else
                {
                    ...
                    if ([self SEND_COMMAND:dic_ResendCommands] && [self READ_COMMAND:dic_ReResponse RETURN_VALUE:szReturnValue])
                    {
                        continue;
                    }
                }
            }
            //if send_command and read-command is not ok
            else
            {
                FlagTrOrg = NO;
                FlagTrgOrg =[NSNumber numberWithBool: [FlagTrgOrg boolValue] && FlagTrOrg];

                if ([self SEND_COMMAND:dic_ResendCommands] && [self READ_COMMAND:dic_ReResponse RETURN_VALUE:szReturnValue])
                {
                    ATSDBgLog( @"NO. %d ,SEND_COMMAND:%@, Read_Command:%@",testTime +1, [dic_ResendCommands objectForKey:@"STRING"],szReturnValue);
                    continue;
                }
            }
        }
...
-(NSNumber *)CHECK_REGISTER_VALUE:(NSDictionary *)dicPara RETURN_VALUE:(NSMutableString *)szReturnValue
{
    

    BOOL FlagTrgOrg1st;
    NSInteger FailMaxTest =[[dicPara objectForKey:@"FailMaxTest"] integerValue];
    NSNumber *Before_Measure ;
    Before_Measure ? Before_Measure =[dicPara objectForKey:@"Before_Measure"] : nil;
    
    NSArray * arrRegisterValue =[dicPara objectForKey:@"LOOP_TEST"];
    NSDictionary * dicFailResendCommands =[dicPara objectForKey:@"FAIL_RESEND"];
    NSDictionary * Touch_Off_Send =[dicFailResendCommands objectForKey:@"Touch off Send Command"];
    NSDictionary * Touch_Off_Read =[dicFailResendCommands objectForKey:@"Touch off Read Command"];
    NSDictionary * Touch_On_Send =[dicFailResendCommands objectForKey:@"Touch on Send Command"];
    NSDictionary * Touch_On_Read =[dicFailResendCommands objectForKey:@"Touch on Read Command"];
    NSDictionary * Load_Firmware_Loop =[dicFailResendCommands objectForKey:@"Load_firmware Loop Command"];
    
    
    for (int NumOfTest = 0 ; NumOfTest < FailMaxTest; NumOfTest ++)
    {
        // to do loop command for 6 registers and get flags.
        for ( int LoopTestNum = 0; LoopTestNum < [arrRegisterValue count]; LoopTestNum ++)
        {
            BOOL bresult =[self LOOP_COMMAND:[arrRegisterValue objectAtIndex:LoopTestNum] RETURN_VALUE:szReturnValue];
            
            FlagTrgOrg =[NSNumber numberWithBool: [FlagTrgOrg boolValue] && FlagTrOrg];
            FlagTrgFnl =[NSNumber numberWithBool:[FlagTrgFnl boolValue] && FlagTrFnl];
         
            //FlagTrgFnlBfWhl and FlagTrgOrgAfWhl are global value, which just change once, if values are NO, don't change any more.
            if ([Before_Measure boolValue] && [FlagTrgFnlBfWhl boolValue])
            {
                BOOL Flag_temp =[FlagTrgFnlBfWhl boolValue] && [FlagTrgFnl boolValue];
                FlagTrgFnlBfWhl =[NSNumber numberWithBool: Flag_temp];
                [m_dicMemoryValues setObject:FlagTrgFnlBfWhl forKey:@"FlagTrgFnlBfWhl"];
            }
            
            if (![Before_Measure boolValue] && [FlagTrgOrgAfWhl boolValue] && Before_Measure != nil ) {
                BOOL Flag_temp =[FlagTrgOrgAfWhl boolValue] && [FlagTrgOrg boolValue];
                FlagTrgOrgAfWhl =[NSNumber numberWithBool: Flag_temp];

                [m_dicMemoryValues setObject:FlagTrgOrgAfWhl forKey:@"FlagTrgOrgAfWhl"];
        ...
        if (NumOfTest == 0) {
            FlagTrgOrg1st =[FlagTrgOrg boolValue];
        }
        if ([FlagTrgFnl boolValue]) {
            break;
        }
        if (![FlagTrgFnl boolValue ] && NumOfTest < FailMaxTest-1)
        {
            [self SEND_COMMAND:Touch_Off_Send];
            [self READ_COMMAND:Touch_Off_Read RETURN_VALUE:szReturnValue];
            [self SEND_COMMAND:Touch_On_Send];
            [self READ_COMMAND:Touch_On_Read RETURN_VALUE:szReturnValue];
            [self LOOP_COMMAND:Load_Firmware_Loop RETURN_VALUE:szReturnValue];
        }
    }
    FlagTrgOrg = FlagTrgOrg1st;
    if (![FlagTrgFnl boolValue]) {
        return  [NSNumber numberWithBool:NO];
    }
    return [NSNumber numberWithBool:YES];
...
-(NSNumber *)REPLACE_CONFIG_VALUE:(NSDictionary *)dicPara
                     RETURN_VALUE:(NSMutableString *)szReturnValue;
{
    NSString *key = [dicPara objectForKey:kFZ_Script_MemoryKey];
    NSMutableString *NeedReplaceStr= [[NSMutableString alloc]init];
    if ([dicPara objectForKey:@"PreKey"]&&[m_dicMemoryValues objectForKey:key]!=nil&&key) {
        key = [dicPara objectForKey:kFZ_Script_MemoryKey];
        [NeedReplaceStr setString:[m_dicMemoryValues objectForKey:key]];
    }
    else
    {
        [NeedReplaceStr setString:szReturnValue];
    }
    if ([NeedReplaceStr containsString:@"_"]) {
        ...
        [m_dicMemoryValues setObject:str forKey:[dicPara objectForKey:@"PostKey"]];
    }
    else
    {
        [szReturnValue setString:@"No Need To Replace"];
    }
    return [NSNumber numberWithBool:YES];
}
