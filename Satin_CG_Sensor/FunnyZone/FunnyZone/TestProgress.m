...
@synthesize m_bTestItemSYNCContinue;
...
- (id)initWithPublicParam:(publicParams *)publicParams
{
        ...
        
	{
        ...
        //for local cof test items
        m_muArrayLocalCofCase = [[NSMutableArray alloc]init];
        m_muArrayLocalCofTest = [[NSMutableArray alloc]init];
        //pudding canceled
        ...
        FlagTchFWNoErr =[[NSNumber alloc]initWithBool:YES];
//        FlagTchRegNoErr =[[NSNumber alloc]initWithBool:YES];
        FlagTrgFnlBfWhl =[NSNumber numberWithBool:YES];
        FlagTrgOrgAfWhl =[NSNumber numberWithBool:YES];
...
- (void)dealloc
{
    ...
    [m_muArrayLocalCofCase  release];
    [m_muArrayLocalCofTest  release];
    ...
    [FlagTchFWNoErr release];

    [FlagTrgFnlBfWhl release];
    [FlagTrgOrgAfWhl release];
...
- (NSDictionary *)getStationInfo
{
    ...
    NSString			*szError	= @"";
	// Modified by lorky.
    for (int iIndex=1; iIndex<IP_GHSTATIONINFO_COUNT; iIndex++)
    {
...
- (NSArray *)parserScriptFile:(NSString *)szScriptFilePath
{
	...
	if (!arrTestItems)
	{
        ...
        alert.informativeText = @"解析剧本档失败。(Parser Script Fail)";
	...
	for (NSDictionary *dictTemp in arrTestItems)//second level
	{
           ...
           1 != [dict count])
        {
            ...
            else
            {
                ...
                alert.informativeText = [NSString stringWithFormat:@"剧本档格式错误: %@ (Script file format error : [%@].)",dict,dict];
                [alert addButtonWithTitle:@"确认(OK)"];
                [alert setAlertStyle:NSWarningAlertStyle];
                [alert runModal];
                [alert release];
		...
		while((key = [enumerator nextObject])) //I think this execute once at most ,Leehua
		{
			if ([[dict objectForKey:key] isKindOfClass:[NSArray class]])
			{
				...
				for (NSUInteger i = 0; i < [arrSubItemTemp count]; i++)
				{
                    ...
                    NSInteger j           = [arrSubItemTemp count] - [arrSubItem count];
				...
				ItemName:(NSString *)szItemName
{
	...
	{
        NSAlert *alert = [[NSAlert alloc]init];
        alert.messageText = [NSString stringWithFormat:@"错误(Error) (Slot:%@)",m_szPortIndex];
        alert.informativeText = [NSString stringWithFormat:@"无法响应函数: %@。(Can't response function [%@])",
                                 szSubName,szSubName];
        [alert addButtonWithTitle:@"确认(OK)"];
        [alert setAlertStyle:NSWarningAlertStyle];
        [alert runModal];
	...
	ATSDebug(@"RETURN_VALUE ===> %@\n%@",
			 m_szReturnValue,[self formatLog:@"+" title:[NSString stringWithFormat:@"[%lu] %@",(unsigned long)[m_arrSingleItemInfo count],szTitle]]);
		 ...
		 currentIndex:(NSInteger)iIndex
{
    ...
    NSString		*szStartItemLog		= [self formatLog:@"="
										   title:[NSString stringWithFormat:
												  @"START TEST %@ (Item%ld)",
												  [szItemName uppercaseString],(long)iIndex]];
    ...
    // Here end for the Press Test
				 ...
				 && (rangeLoopName.location+rangeLoopName.length) <= [szSubItemName length])
		{
				...
				&& loopRepeatTime != nil)
            {
                ...
                for(NSInteger iIndex=iStart; iIndex<=iEnd; iIndex++)
                {
				   ...
				   || ((bPassContinue) && ([bPassContinue boolValue])))
                {
                    ...
                    for (int iLoop = 0; iLoop < iMaxRepeatTime; iLoop++)
                    {
                        ...
                        bStatus		= YES;
                        for(NSInteger j=iStart; j<=iEnd; j++)
                        {
...
- (void)start
{
    // for IQC-DEV 5, adding flag to store register and load firmware status
    if ([m_szScriptName isEqualToString:@"IQC-DEV5"]) {
    ...
    if (gbAuditMode)
    {
        [m_dicMemoryValues   setObject:@"YES" forKey:@"IS_AUDIT_MODE"];
    }
    else
    {
        [m_dicMemoryValues   setObject:@"NO" forKey:@"IS_AUDIT_MODE"];
    }
   ...
   // [self LoadLimitFile];
	...
	for (NSUInteger i = 0; i < [m_arrayScript count]; i++)
	{
        ...
        if ([m_muArrayCancelCase containsObject:szCurrentItemName])
        {
            ...
            if ([m_muArrayLocalCofCase containsObject:szCurrentItemName])
            {
                [szTestNames appendFormat:@",\"%@\"", [NSString stringWithFormat:@"CoF_%@",szCurrentItemName]];
                [szUpperLimits appendString:@","];
                [szDownLimits appendString:@","];
                [szValueList appendString:@","];
            }
        ...
        //set image for "pass" and "fail" and "Cof Pass"
        ...
        if (m_bSubTest_PASS_FAIL && [m_muArrayLocalCofTest containsObject:szCurrentItemName])
        {
		...
		[self writeForNormalSpecName:szCurrentItemName
							  status:[m_muArrayLocalCofTest containsObject:szCurrentItemName]?NO:m_bSubTest_PASS_FAIL
        ...
        // If use COF Limit test, upload "CoF_subName" to PDCA
        if([m_muArrayLocalCofCase containsObject:szCurrentItemName])
        {
            if ([m_muArrayLocalCofTest containsObject:szCurrentItemName])
            {
                [self writeForCocoSpecName:szCurrentItemName
        ...
        if ([m_dicMemoryValues valueForKey:kFunnyZoneShowCoFONUI] == nil)
        {
            NSDictionary	*dictALLView	= [NSDictionary dictionaryWithObjectsAndKeys:
											   ...
											   [NSString stringWithFormat:@"[%lu]",i+1],			kAllTableViewIndex,
											   ...
											   [NSNumber numberWithUnsignedInteger:i],						kFunnyZoneCurrentIndex,
											   [NSNumber numberWithUnsignedInteger:[m_arrayScript count]],	kFunnyZoneSumIndex,
        ...
        if (m_bCancelToEnd && !m_bSubTest_PASS_FAIL)
		{
			...
			{
				// Added by Izual_Lu on 2013-03-19 for summary log columns corresponding.
				for(NSInteger i0=i+1; i0<=[m_arrayScript count]-iCancelNumber; i0++)
				{
                    NSString *strTestName =[(TestItemStruct*)[m_arrayScript objectAtIndex:i0] ItemName];
					[szTestNames appendFormat:@",\"%@\"", strTestName];
					[szUpperLimits appendString:@","];
					[szDownLimits appendString:@","];
					[szValueList appendString:@","];
                    if ([m_muArrayLocalCofCase containsObject:strTestName])
                    {
                        [szTestNames appendFormat:@",\"%@\"", [NSString stringWithFormat:@"CoF_%@",strTestName]];
                        [szUpperLimits appendString:@","];
                        [szDownLimits appendString:@","];
                        [szValueList appendString:@","];
                    }
	...
	NSString	*szEndInfoToCSV		= [NSString stringWithFormat:
									   @"%@Test Start Time,0,%@,,,\nTest End Time,0,%@,,,\nTest Cost Time,0,%f,,,",
									   szSNLog,
									   [dateStartProgress descriptionWithCalendarFormat:@"HH:mm:ss"
																			   timeZone:nil
																				 locale:nil],
									   [dateEnd descriptionWithCalendarFormat:@"HH:mm:ss"
																	 timeZone:nil
																	   locale:nil],
									   dProgressCostTime];
    ...
    NSString	*szInfo	= [NSString stringWithFormat:
						   ...
						   m_szISN,
						   [m_dicMemoryValues objectForKey:descriptionOfGHStationInfo(IP_STATION_ID)],
                           (0 == [m_muArrayLocalCofTest count])? sz_TestStatus:@"Fail" ,
						   [dateStartProgress descriptionWithCalendarFormat:@"YYYY-MM-dd HH:mm:ss"
																   timeZone:nil
																	 locale:nil],
						   [dateEnd descriptionWithCalendarFormat:@"YYYY-MM-dd HH:mm:ss"
														 timeZone:nil
														   locale:nil],
						   m_szFailLists,
						   szValueList];
	...
	[self endDUTPrg:dProgressCostTime TestResult:m_bFinalResult & (0 == [m_muArrayLocalCofTest count])];
...
- (NSString *)formatLog_transferObject:(id)objItem
{
    ...
    else if ([objItem isKindOfClass:[NSArray class]]) 
    {
        ...
        for(NSInteger iIndex=0; iIndex<iCount; iIndex++)
        {
            ...
            [szMutRet appendFormat:@"Item%ld=%@ ; ",(long)iIndex,szMid];
        }
		 ...
		 withSum:(NSString *)szSum
{
    ...
    else
    {
        ...
        else
        {
            ...
            NSAlert *alert = [[NSAlert alloc]init];
            alert.messageText = [NSString stringWithFormat:@"警告(Warning) (Slot:%@)",m_szPortIndex];
            alert.informativeText = szMessage;
            [alert addButtonWithTitle:@"确认(OK)"];
            [alert setAlertStyle:NSWarningAlertStyle];
            [alert runModal];
            [alert release];
...
-(BOOL)startPuddingUploadFlow
{
	...
	if ([m_objPudding StartPDCA_Flow] != kFZ_InstantPudding_Success)
    {
        if (!gbIfNeedRemoteCtl)
        {
            NSAlert *alert = [[NSAlert alloc]init];
            alert.messageText = [NSString stringWithFormat:@"错误(Error) (Slot:%@)",m_szPortIndex];
            alert.informativeText = @"开始使用Pudding时发生错误。(Pudding StartPDCA Flow Error)";
            [alert addButtonWithTitle:@"确认(OK)"];
            [alert setAlertStyle:NSWarningAlertStyle];
            [alert runModal];
            [alert release];
...
- (BOOL)setInstantPuddingSerialNumber
{
    ...
    if (!m_objPudding.haveSetISN)
	{
        ...
        if (iRet != kSuccessCode)
		{
            ...
            {
                NSAlert *alert = [[NSAlert alloc]init];
                alert.messageText = [NSString stringWithFormat:@"Pudding Error (Slot:%@)",m_szPortIndex];
                alert.informativeText = strErrorCode;
                [alert addButtonWithTitle:@"确认(OK)"];
                [alert setAlertStyle:NSWarningAlertStyle];
                [alert runModal];
                [alert release];
 ...
 //       }
        ...
        if (bComplete != TEST_SUCCESS)
        {
            ...
            if (!gbIfNeedRemoteCtl)
            {
                NSAlert *alert = [[NSAlert alloc]init];
                alert.messageText = [NSString stringWithFormat:@"Pudding Error (Slot:%@)",m_szPortIndex];
                alert.informativeText = szPuddingErr;
                [alert addButtonWithTitle:@"确认(OK)"];
                [alert setAlertStyle:NSWarningAlertStyle];
                [alert runModal];
                [alert release];
...
- (void)renameLogs
{
	...
	{
		if (m_bFinalResult & (0 == [m_muArrayLocalCofTest count]))
    ...
    if ([[kPD_UserDefaults objectForKey:@"gyroSanity"]boolValue])
    {
        ...
        [fileManager moveItemAtPath:[NSString stringWithFormat:@"/vault/%@",Earthbound_pdca_gyrosanity] toPath:[NSString stringWithFormat:@"%@/%@",szFinalLogFolderName,Earthbound_pdca_gyrosanity] error:nil];
        [fileManager moveItemAtPath:[NSString stringWithFormat:@"/vault/%@",gyroSanityRawGyroReadings_0] toPath:[NSString stringWithFormat:@"%@/%@",szFinalLogFolderName,gyroSanityRawGyroReadings_0] error:nil];
        [fileManager moveItemAtPath:[NSString stringWithFormat:@"/vault/%@",gyroSanityLiveGyroReadings_0] toPath:[NSString stringWithFormat:@"%@/%@",szFinalLogFolderName,gyroSanityLiveGyroReadings_0] error:nil];
        [fileManager moveItemAtPath:[NSString stringWithFormat:@"/vault/%@",gyroSanityGyroReadings_0] toPath:[NSString stringWithFormat:@"%@/%@",szFinalLogFolderName,gyroSanityGyroReadings_0] error:nil];
    }
							  ...
							  RETURNVALUE:(NSMutableString *)szReturn
{
    ...
    BOOL			bTestResult			= [dicUploadInfo objectForKey:@"Result"]? [[dicUploadInfo objectForKey:@"Result"] boolValue] : YES;
				 ...
				 CurrentIndex:(NSInteger)index
{
    ...
    {
        NSString	*szTestSpec	= [m_dicMemoryValues objectForKey:kFZ_TestLimit];
        [self getSpecFrom:szTestSpec lowLimt:&szDnLimit highLimit:&szUpLimit];
...
- (NSNumber *)UPLOAD_GYROSANITY:(NSDictionary *)dicParam RETURN_VALUE:(NSMutableString *)strReturnValue
{
    ...
    ATSDebug(@"The result is [%@]", bResult);
...
-(BOOL)LoadLimitFile
{
    ...
    NSFileManager * fileManager = [NSFileManager defaultManager];
    if (![fileManager fileExistsAtPath:szLimitFilePath])
    {
        NSAlert *alert = [[NSAlert alloc]init];
        alert.messageText = @"错误(Error)";
        alert.informativeText = @"Load limit file fail!";
        [alert addButtonWithTitle:@"确认(OK)"];
        [alert runModal];
        [alert release];
        ATSDebug(@"Terminate at [%@], Parser Script fail, Get Nil from the path [%@]",
                 [[NSDate date] description], szLimitFilePath);
        [NSApp terminate:nil];
    }
    //Insert the limits to the m_dicMemoryValues
    NSDictionary    *dicSOCLimit    = [NSDictionary dictionaryWithContentsOfFile:szLimitFilePath];
    for (NSString *szKey in [dicSOCLimit allKeys])
    {
        [m_dicMemoryValues setObject:[[dicSOCLimit objectForKey:szKey] objectForKey:@"spec"]
                           ...
                           RETURN_VALUE:(NSMutableString *)szReturnValue
{
    ...
    for (int testTime = 0; testTime < MaxTest; testTime ++)
    {
        ...
        if ([self COMMUNICATE_WITH_TASK:dic_firstSendCommands RETURN_VALUE:szReturnValue])
        {
            ...
            if ([strSub length])
            {
                ...
                [szReturnValue setString:strSub];
