                   ...
                   RETURN_VALUE:(NSMutableString*)strReturnValue
{
    ...
    BOOL        *bConfirm       = [[dicContents objectForKey:@"ConfirmToStopCheck"]boolValue];
    ...
    do
    {
        @synchronized(m_dicButtonBuffer)
        {
                ...
                
            {
                ...
                if (bConfirm && [strReturnValue isEqualToString:@"Yes"]) {
                    //bAllPass = 0;
                    NSLog(@"break button status check");
                    break;
				   ...
				   RETURN_VALUE:(NSMutableString*)strReturnValue
{
	...
	if (!dicContents || [dicContents count] == 0) {
        ...
        alert.informativeText = @"CHECK_RINGER_STATUS:RETURN_VALUE:没有参数。(There are no parameters for CHECK_RINGER_STATUS!)";
		...
		|| [strRingerOriginal isEqualToString:@""])
	{
        NSAlert *alert = [[NSAlert alloc]init];
        alert.messageText = @"警告(Warning)";
        alert.informativeText = @"CHECK_RINGER_STATUS:RETURN_VALUE:里的参数为空。(The parameters for CHECK_RINGER_STATUS is null.)";
                       ...
                       RETURN_VALUE:(NSMutableString *)strReturnValue
{
    ...
    if (!dicContents || [dicContents count] == 0) {
        NSAlert *alert = [[NSAlert alloc]init];
        alert.messageText = @"警告(Warning)";
        alert.informativeText = @"BUTTONTEST_FINAL_RESULT:RETURN_VALUE:没有参数。(There are no parameters for BUTTONTEST_FINAL_RESULT!)";
        [alert addButtonWithTitle:@"噢(OK)"];
        [alert setAlertStyle:NSWarningAlertStyle];
        [alert runModal];
        [alert release];
    ...
    NSArray *arySubItems = [dicContents objectForKey:@"SubItems"];
    
    for (NSString *strSubItem in arySubItems)
    {
        if ([m_szFailLists containsString:strSubItem])
        {
            bResult = NO;
        }
    }
