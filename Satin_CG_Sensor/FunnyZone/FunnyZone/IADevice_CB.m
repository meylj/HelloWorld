				  ...
				  RETURN_VALUE:(NSMutableString*)szReturnValue
{
    ...
    if(!m_bFinalResult || [[m_dicMemoryValues objectForKey:@"IS_AUDIT_MODE"]isEqualToString:@"YES"])
    {
        //test fail
    ...
    else
    {
		...
		NSDictionary * dictPassword = [NSDictionary dictionaryWithObjectsAndKeys:
									   @"8D1A63C810E2C3981F88CF7862DBB831F2138B06", @"0x0b",  // SMT-QT
                                       @"AEB3B87649755B899E9321BA55C0E9A433FD7B0C", @"0x77",  // CT1
									   @"515AF16037583EB88C25BB18A37CB0ABDC5485DE", @"0x7a",  // UNIT-SHUTDOWN
									   @"28F39922F2036686523FC4292036AB8820945E0D", @"0x7b",  // CG-INSTALL
									   @"FDDF743F3E0917A89BE2A3D8C24C3360554D193B", @"0x80",  // QT0
									   @"90FA18E23B50E72F6F6802BA0939CD7976B1FC6E", @"0x81",  // QT1
									   @"FDD4835578BEA7BAA231E3EEB67AEF3B77934AF4", @"0x82",  // QT2
                                       @"4D4B72405CD3742DA5CB07FCC7AAA3ADC04F899B", @"0x8a",  // QT3
									   @"6C623F5BF45FBA557A51DA1C1E8645FA6EC0999B", @"0xd2",  // STOM
                                       nil];
...
- (NSNumber*)CHECK_CONTROL_BITS:(NSDictionary *)dicSubTestItem
				   RETURN_VALUE:(NSMutableString*)szReturnValue
{
    if ([[m_dicMemoryValues objectForKey:@"IS_AUDIT_MODE"]isEqualToString:@"YES"])
    {
        return [NSNumber numberWithBool:YES];
    }
    NSNumber		*numRet;
...
//For Audit Mode
- (NSNumber*)WRITE_CB_FAIL:(NSDictionary *)dicSubTestItem
              RETURN_VALUE:(NSMutableString*)szReturnValue
{
    int		iAllowedFailCount	= [m_CBTestBase getStationFailCountAllowed];
    if ([[m_dicMemoryValues objectForKey:@"IS_AUDIT_MODE"]isEqualToString:@"NO"])
    {
        [szReturnValue setString:[NSString stringWithFormat:@"No need wirite CB Fail,It is not Audit Mode.The AllowedFailCount is %d",iAllowedFailCount]];
        return [NSNumber numberWithBool:YES];
    }

    NSDictionary	*dicSend_setCBFail		= [dicSubTestItem objectForKey:
                                                   @"SEND_COMMAND:"];
    NSDictionary	*dicReceive_setCBFail	= [dicSubTestItem objectForKey:
                                                   @"READ_COMMAND:RETURN_VALUE:"];
    NSNumber	*numRet = YES;
    [m_dicMemoryValues setObject:uiVersion
                          forKey:kFZ_SoftVersion];
    for (NSInteger i = 0; i<iAllowedFailCount+1; i++)
    {
        [self SEND_COMMAND:dicSend_setCBFail];
         numRet	= [self READ_COMMAND:dicReceive_setCBFail
                                 RETURN_VALUE:szReturnValue];
    }
    if (![numRet boolValue])
    {
