				...
				RETURN_VALUE:(NSMutableString*)szReturnValue
{
    ...
    NSArray		*aryKeyNames	= [[dicPara valueForKey:kFZ_Script_MemoryKey]
								   componentsSeparatedByString:kFunnyZoneComma];
	// modified by desikan 2012.5.2 for current when Volt is NA
	//	but current is not NA, pow should be NA, and return NO.
    NSString	*szValueOne, *szValueTwo;
    ...
    if (szValueOne== NULL
		|| szValueTwo==NULL
		|| [szValueOne isEqualTo:@"NA"]
		|| [szValueTwo isEqualTo:@"NA"])
    {
	...
	if ([arrData count] >= 3) 
	{
		for(i = 0; i < 3; i++)
		{
			...
			if ((NSNotFound != rangeStart.location)
				...
				strStart	= [[arrData objectAtIndex:i]
							   substringFromIndex:(rangeStart.location + rangeStart.length)];
            else
            {
			...
			[m_dicMemoryValues setValue:strStart
