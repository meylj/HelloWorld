...
{
    // Basic judge
    NSArray *arrayUpperCaseKeys = [NSArray arrayWithObjects:
								   @"URL",			@"PRODUCT",
								   @"STATION_ID",	@"OS_BUNDLE_VERSION",
								   @"MAC_ADDRESS",	nil];
    if((nil == dicGHInfo)
       || (![dicGHInfo isKindOfClass:[NSDictionary class]]))
        return nil;
    ...
    [dicParseredOK setObject:[dicGHInfo objectForKey:@"URL"]
					  forKey:@"url"];
    [dicParseredOK setObject:[dicGHInfo objectForKey:@"PRODUCT"]
					  forKey:@"product"];
    [dicParseredOK setObject:[dicGHInfo objectForKey:@"STATION_ID"]
					  forKey:@"station_id"];
    [dicParseredOK setObject:[dicGHInfo objectForKey:@"STATION_NAME"]
					  forKey:@"test_station_name"];
    [dicParseredOK setObject:[dicGHInfo objectForKey:@"OS_BUNDLE_VERSION"]
					  forKey:@"os_bundle_version"];
    [dicParseredOK setObject:[dicGHInfo objectForKey:@"MAC_ADDRESS"]
					  forKey:@"mac_address"];
                    ...
                    WithSN:(NSString*)szSN
{
    ...
    NSMutableURLRequest	*URLRequest	= [[[NSMutableURLRequest alloc]
										initWithURL:[NSURL URLWithString:szRequest]] autorelease];
    [URLRequest setHTTPMethod:@"POST"];
    [URLRequest setValue:@"text/html;charset=UTF8"
    ...
    if(NSNotFound == range.location
	   || range.length <= 0
	   || ((range.length + range.location) > [szResponse length]))
        return nil;
