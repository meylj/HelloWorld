...
- (NSNumber *) SYNC_WITH_OTHER_DEVICE:(NSDictionary *)dicSubItems RETURN_VALUE:(NSMutableString *)szReturnValue
{
    ...
    if ([szIncludeThread ContainString:@"/*"] && [szIncludeThread ContainString:@"*/"])
    {
        ...
        szIncludeThread = [NSString stringWithFormat:@"%lu",[[m_dicPorts allKeys] count]-1]; //get fixture count
    }
    ...
    */
    ...
    ATSDebug(@"SYNC_WITH_OTHER_DEVICE: Parallel thread count %ld , fixture count %ld",(long)iPortCount,(long)iFixtureThread);
    ...
    ATSDebug(@"SYNC_WITH_OTHER_DEVICE: syncKey %@ count %ld",szSyncKey,(long)[[m_objPublicParams.publicMemoryValues objectForKey:szSyncKey] integerValue]);
...
- (void)sampleSingleDevice:(NSArray *)aryObjects
{
    ...
    NSString *szStartTime = [dateStart descriptionWithCalendarFormat:@"YYYY-MM-dd HH:mm:ss.Ms" timeZone:nil locale:nil];
    NSInteger iIndex=0;
...
//        [uartObj Read_UartData:uartResponse TerminateSymbol:aryEndSymbols MatchType:iMatchType IntervalTime:dReadInterval TimeOut:dReadTimeOut Ignore:nil];
    ...
    NSString *szEndTime = [dateEnd descriptionWithCalendarFormat:@"YYYY-MM-dd HH:mm:ss.Ms" timeZone:nil locale:nil];
    ...
    if (bIsGlobal)
    {
        @synchronized(m_objPublicParams.publicMemoryValues)
        {
            ...
            {
                [m_objPublicParams.publicMemoryValues setObject:[NSString stringWithFormat:@"%ld",(long)iIndex] forKey:szSampleCount];
            }
...
- (NSNumber *) MEMORY_COUNT:(NSDictionary *)dicSubItems RETURN_VALUE:(NSMutableString *)szReturnValue
{
    ...
    NSString *szCount = [NSString stringWithFormat:@"%lu",(unsigned long)[aryResponse count]];
...
- (NSNumber *) CALCULATE_PATCH_DATA:(NSDictionary *)dicSubItems RETURN_VALUE:(NSMutableString *)szReturnValue
{
    ...
    [dataValue getBytes:pBuffer length:(iLen+1)];
...
{    
    ...
    NSString    *szCount    = [NSString stringWithFormat:@"%lu",(unsigned long)[arrCounts count]];
