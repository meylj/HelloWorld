...
//path for save CalibrateData
#define KSaveDataDefaultPath  [NSString stringWithFormat:@"%@/Library/Preferences/CalibrateData.plist", NSHomeDirectory()]
...
NSString* const NotificationTestItemSYNC    = @"NotificationTestItemSYNC";
						  ...
						  RETURN_VALUE:(NSMutableString *)strReturnValue
{
    ...
    NSInteger	iLengthofXX		= [szXX length];
    ...
    for (int i = 0; i < [aryJudge count]; i+=1)
    {
        ...
        ATSDebug(@"You want to judge [Line: %ld , Position: %@]",
				 (long)iLine, szPositions);
        ...
        for (int j = 0; j < [aryPositions count]; j++)
        {
            ...
            if ([aryFromXXToXX count] == 2)
            {
                ...
                NSInteger	iMax	= [aryValues count];
                ...
                for (int iIndex = iBegin; iIndex <= iEnd ; iIndex++)
                {
                    ...
                    if (!bWant)
                    {
                        ...
                        {
                            ATSDebug(@"Value in [Line: %ld , Position: %d] doesn't MATCH %@",
									 (long)iLine, iIndex, szXX);
                            [strReturnValue setString:@"PASS"];
                            return [NSNumber numberWithBool:YES];
                        }
                        else
                        {
                            ATSDebug(@"Value in [Line: %ld , Position: %d] MATCH %@",
									 (long)iLine, iIndex, szXX);
                            bTestRet	= NO;
                        }
                    }
            ...
            {
                ATSDebug(@"No Judge in [Line: %ld , Position: %@] Or your Format is Wrong",
						 (long)iLine, szAPosition);
                bTestRet = NO;
                break;
            }
        }
			 ...
			 RETURN_VALUE:(NSMutableString *)strReturnValue
{
	...
	NSInteger		iCount	= [arrValue count];
					   ...
					   RETURN_VALUE:(NSMutableString *)szReturnValue
{
    ...
    if ([fileManager fileExistsAtPath:strPath] != YES) 
    {
        if (!m_bCancelToEnd)
        {
            ...
            alert.informativeText = @"请先对治具进行校验！(Please do fixture calibration first!)";
			 ...
			 ReturnValue:(NSMutableString *)strReturn
{
	...
	else
    {
        ATSDebug(@"The prox seneor is less than 5\n data = %@\nlength = %lu\n",
				 strReturn, (unsigned long)[arrayHexValue count]);
        return [NSNumber numberWithBool:NO];
    }
						...
						ReturnVale:(NSMutableString *)strReturn
{
    ...
    if (bCancelToEND) 
        if (fabsf([strValue floatValue]) >= [strCanceToEndDistance floatValue])
        {
			 ...
			 RETURN_VALUE:(NSMutableString *)strReturn
{
	...
	NSString	*strRowCompare	= (bRowResult
								   ? [NSString stringWithFormat:
									  @"Compare Row OK, row count is [%lu]",
									  (unsigned long)iCheckRowCount]
								   : [NSString stringWithFormat:
									  @"Compare Row NOT OK, checking count is [%lu], actually count is [%lu]",
									  (unsigned long)iCheckColCount, (unsigned long)[aryRow count]]);
	...
	for (NSUInteger i = 0; i < [aryRow count]; i++)
	{
		...
		strRowCompare	= (bColResult
						   ? [NSString stringWithFormat:
							  @"Compare Row[%lu] OK, Column count is [%lu]",
							  (unsigned long)i, (unsigned long)iCheckColCount]
						   : [NSString stringWithFormat:
							  @"Compare Row[%lu] NOT OK, checking count is [%lu], actually count is [%lu]",
							  (unsigned long)i, (unsigned long)iCheckColCount, (unsigned long)[aryRow count]]);
        ATSDebug(@"%@", strRowCompare);
	}
				 ...
				 RETURN_VALUE:(NSMutableString *)strReturnValue
{
    ...
    if ([row count] > 0) 
    {
        ...
        for (NSInteger i = 0; i < [row count]; i++)
            [muArray addObject:[NSString stringWithFormat:@"%lu", (unsigned long)i]];
					...
					return_Value:(NSMutableString *)strReturn
{
    ...
    ATSDebug(@"Prox data = %@\n length = %lu\n",
			 strValue, (unsigned long)[strReturn length]);
					 ...
					 RETURN_VALUE:(NSMutableString *)szReturnValue
{
	...
	if (!dicSetting || [dicSetting count] == 0) {
        NSAlert *alert = [[NSAlert alloc]init];
        alert.messageText = @"警告(Warning)";
        alert.informativeText = @"CUSTOM_COMBINE_DATA:RETURN_VALUE:没有参数。(There are no parameters for CUSTOM_COMBINE_DATA!)";
        [alert addButtonWithTitle:@"确认(OK)"];
        [alert setAlertStyle:NSWarningAlertStyle];
        [alert runModal];
        [alert release];
	...
	int			iEndIndex		= ([dicSetting objectForKey:@"EndIndex"]
								   ? [[dicSetting objectForKey:@"EndIndex"] intValue] : (int)[arySeparate count]-1);
	//Combine
				  ...
				  RETURN_VALUE:(NSMutableString *)strReturnValue
{
	...
	if (!dicSetting || [dicSetting count] == 0) {
        NSAlert *alert = [[NSAlert alloc]init];
        alert.messageText = @"警告(Warning)";
        alert.informativeText = @"CALCULATE_LCM_SN:RETURN_VALUE:没有参数。(There are no parameters for CALCULATE_LCM_SN!)";
        [alert addButtonWithTitle:@"确认(OK)"];
        [alert setAlertStyle:NSWarningAlertStyle];
        [alert runModal];
        [alert release];
	...
	int			iBeginIndex		= ([dicSetting objectForKey:@"BeginIndex"]
								   ? [[dicSetting objectForKey:@"BeginIndex"] intValue] : 0);
	int			iEndIndex		= ([dicSetting objectForKey:@"EndIndex"]
								   ? [[dicSetting objectForKey:@"EndIndex"] intValue] : (int)[aryContent count]-1);
...
/* Add by York 2015.6.23 Just for combining string without space
 NSDictionary			*dictSettings   : Settings
 SeparateString	->	NSString		: " ", "/n", "," ...
 */
- (NSNumber *)COMBINE_STRING_WITHOUT_SPACE:(NSDictionary *)dicSetting
                              RETURN_VALUE:(NSMutableString *)strReturnValue
{
    //If the setting dictionary has no parameters, post a alert panel.
    if (!dicSetting || [dicSetting count] == 0) {
    ...
    if ([aryContent count] > 0)
    {
        ...
        {
            [strReturnValue setString:[aryContent objectAtIndex:i]];
            [strOutput appendFormat:@"%@", [NSString stringWithFormat:@"%@",strReturnValue]];
        }
        ATSDebug(@"The combined string is: %@", strOutput);
    }
					 ...
					 RETURN_VALUE:(NSMutableString *)strReturnValue
{
	...
	if (!dicSetting || [dicSetting count] == 0) {
        NSAlert *alert = [[NSAlert alloc]init];
        alert.messageText = @"警告(Warning)";
        alert.informativeText = @"CATCH_VALUE_BY_INDEX:RETURN_VALUE:没有参数。(There are no parameters for CATCH_VALUE_BY_INDEX!)";
        [alert addButtonWithTitle:@"确认(OK)"];
        [alert setAlertStyle:NSWarningAlertStyle];
        [alert runModal];
        [alert release];
    ...
    for(int i = iStart; i < [arrayProx count]; i++)
    {
        ...
        if ([szTemp contains:@"RAW: "])
        {
			if ([[szTemp SubFrom:@"RAW: "include:NO] length]>=6)
			{
				...
				else if (32768 <= iValue && iValue<= 65535)
                {
                    temp = (65535 - iValue + 1);
                }
                else
...
//Add by Linda
- (NSNumber *)DO_CALCULATE_PROX_CRB:(NSDictionary *)dicSetting
                       RETURN_VALUE:(NSMutableString *)szReturnValue
{
    ...
    [szReturnValue setString:strTemp];
    NSArray	*arrayProx	= [szReturnValue componentsSeparatedByString:@"PROX:"];
    ATSDebug(@"PROX　test value is %@",szReturnValue);
    ...
    for(int i = iStart; i < [arrayProx count]; i++)
    {
        ...
        if ([szTemp contains:@"(Triggered)"])
        {
            szTemp	=[szTemp stringByReplacingOccurrencesOfString:@"(Triggered)" withString:@""];
            unsigned int iValue = 0;
            iValue	=[szTemp intValue];
            NSLog(@"iValue=====%d",iValue);
            fProxSum	+= iValue;
            iProxCount++;
            ATSDebug(@"total value is %f",fProxSum);
        }
        
    }
    NSString * strAverage	= [NSString stringWithFormat:@"%.2f",fProxSum / iProxCount];
    if ([strAverage intValue]==0) {
        [szReturnValue setString:@"FAIL"];
        return [NSNumber numberWithBool:NO];
        
    }
    [szReturnValue setString:strAverage];
    //	[arrProxData release];
			 ...
			 RETURN_VALUE:(NSMutableString *)szReturnValue
{
	...
	NSInteger length = [[dicSetting objectForKey:@"LENGTH"] integerValue];
    ...
    if ([[m_dicMemoryValues objectForKey:descriptionOfGHStationInfo(IP_LINE_NUMBER)] ContainString:@"REL"])
    {
        if ([dicSetting objectForKey:@"CancelItemForREL"])
        {
            ...
            for (NSString   *szCancelForREL in arrCancelForREL)
            {
             ...
             && [[m_dicMemoryValues objectForKey:descriptionOfGHStationInfo(IP_LOCATION)] ContainString:@"FT"]) || [[m_dicMemoryValues objectForKey:descriptionOfGHStationInfo(IP_LOCATION)] ContainString:@"FRT"])
    {
						...
						RETURN_VALUE:(NSMutableString *)strReturnValue
{
	...
	if (!dicSetting || [dicSetting count] == 0) {
        NSAlert *alert = [[NSAlert alloc]init];
        alert.messageText = @"警告(Warning)";
        alert.informativeText = @"SET_RETURNVALUE_FORMAT:RETURN_VALUE:没有参数。(There are no parameters for SET_RETURNVALUE_FORMAT!)";
        [alert addButtonWithTitle:@"确认(OK)"];
        [alert setAlertStyle:NSWarningAlertStyle];
        [alert runModal];
        [alert release];
	...
	if (strFormat == nil || [strFormat isEqualToString:@""]) {
        NSAlert *alert = [[NSAlert alloc]init];
        alert.messageText = @"警告(Warning)";
        alert.informativeText = @"SET_RETURNVALUE_FORMAT:RETURN_VALUE里的参数为空。(The parameters for SET_RETURNVALUE_FORMAT is null.)";
        [alert addButtonWithTitle:@"确认(OK)"];
        [alert setAlertStyle:NSWarningAlertStyle];
        [alert runModal];
        [alert release];
			   ...
			   RETURN_VALUE:(NSMutableString *)strReturnValue
{
	...
	if (!dicSetting || [dicSetting count] == 0) {
        NSAlert *alert = [[NSAlert alloc]init];
        alert.messageText = @"警告(Warning)";
        alert.informativeText = @"MAPPING_TABLE:RETURN_VALUE:没有参数。(There are no parameters for MAPPING_TABLE!)";
        [alert addButtonWithTitle:@"确认(OK)"];
        [alert setAlertStyle:NSWarningAlertStyle];
        [alert runModal];
        [alert release];
		...
		|| !dicMappingTable || [dicMappingTable count] ==0)
	{
        NSAlert *alert = [[NSAlert alloc]init];
        alert.messageText = @"警告(Warning)";
        alert.informativeText = @"MAPPING_TABLE:RETURN_VALUE:缺少参数。(Some parameters of MAPPING_TABLE are missing!)";
        [alert addButtonWithTitle:@"确认(OK)"];
        [alert setAlertStyle:NSWarningAlertStyle];
        [alert runModal];
        [alert release];
    ...
    for (NSString *szTempMap in arrTemp)
    {
        if ([strReturnValue isEqualToString:szTempMap])
        {
            ...
            numRet = [self JUDGE_SPEC:[dicTempJudge objectForKey:@"JudgeSpec"]
                         RETURN_VALUE:strReturnValue];
            NSMutableDictionary *dicPara  = [[NSMutableDictionary alloc] initWithDictionary:[dicTempJudge objectForKey:@"UploadParametric"]];
            [dicPara setObject:numRet forKey:@"Result"];
            [self UPLOAD_PARAMETRIC:dicPara
                       RETURN_VALUE:strReturnValue];
            [dicPara release];
            return numRet;
        }
            ...
            RETURN_VALUE:(NSMutableString *)strReturnValue
{
    ...
    if ([strReturnValue contains:@"??????"])
    {
        ...
        NSAlert *alert = [[NSAlert alloc]init];
        alert.messageText = @"警告(Warning)";
        alert.informativeText = @"There have no loss data in fixture,please do correlation with this fixture.";
        [alert addButtonWithTitle:@"确认(OK)"];
        [alert setAlertStyle:NSWarningAlertStyle];
        [alert runModal];
        [alert release];
    ...
    else if ([strReturnValue contains:@"+99999"])
    {
        ...
        NSAlert *alert = [[NSAlert alloc]init];
        alert.messageText = @"警告(Warning)";
        alert.informativeText = @"calibration did not complete, please re-do calibration.";
        [alert addButtonWithTitle:@"确认(OK)"];
        [alert setAlertStyle:NSWarningAlertStyle];
        [alert runModal];
        [alert release];
                          ...
                          RETURN_VALUE:(NSMutableString *)strReturnValue
{
	...
	if (!dicCatchValue || [dicCatchValue count] == 0) {
        NSAlert *alert = [[NSAlert alloc]init];
        alert.messageText = @"警告(Warning)";
        alert.informativeText = @"DELETE_STRING_FROM_VALUE:RETURN_VALUE::没有参数。(There are no parameters for SET_RETURNVALUE_FORMAT!)";
        [alert addButtonWithTitle:@"确认(OK)"];
        [alert setAlertStyle:NSWarningAlertStyle];
        [alert runModal];
        [alert release];
        ...
        szRegex == nil || [szRegex isEqualToString:@""])
    {
        NSAlert *alert = [[NSAlert alloc]init];
        alert.messageText = @"警告(Warning)";
        alert.informativeText = @"DELETE_STRING_FROM_VALUE:RETURN_VALUE:里的参数为空。(The parameters for SET_RETURNVALUE_FORMAT is null.)";
        [alert addButtonWithTitle:@"确认(OK)"];
        [alert setAlertStyle:NSWarningAlertStyle];
        [alert runModal];
        [alert release];
						 ...
						 RETURN_VALUE:(NSMutableString *)szReturnValue
{
    ...
    NSDateComponents * dateComponents = [gregorian components:(NSCalendarUnitEra |
                                                               NSCalendarUnitYear |
                                                               NSCalendarUnitMonth |
                                                               NSCalendarUnitDay |
                                                               NSCalendarUnitHour |
	...
	NSInteger iWeek					= [dateComponents weekOfYear];
	...
	[m_dicMemoryValues setObject:[NSString stringWithFormat:@"%ld",(long)iEra]					forKey:@"era"];
	[m_dicMemoryValues setObject:[NSString stringWithFormat:@"%ld",(long)iYear]					forKey:@"year"];
	[m_dicMemoryValues setObject:[NSString stringWithFormat:@"%ld",(long)iMonth]				forKey:@"month"];
	[m_dicMemoryValues setObject:[NSString stringWithFormat:@"%ld",(long)iDay]					forKey:@"day"];
	[m_dicMemoryValues setObject:[NSString stringWithFormat:@"%ld",(long)iHour]					forKey:@"hour"];
	[m_dicMemoryValues setObject:[NSString stringWithFormat:@"%ld",(long)iMinute]				forKey:@"minute"];
	[m_dicMemoryValues setObject:[NSString stringWithFormat:@"%ld",(long)iSecond]				forKey:@"second"];
	[m_dicMemoryValues setObject:[NSString stringWithFormat:@"%ld",(long)iWeek]					forKey:@"week"];
	[m_dicMemoryValues setObject:[NSString stringWithFormat:@"%ld",(long)iWeekday]				forKey:@"weekday"];
	[m_dicMemoryValues setObject:[NSString stringWithFormat:@"%ld",(long)iWeekdayOrdinal]		forKey:@"weekdayOrdinal"];
	[m_dicMemoryValues setObject:[NSString stringWithFormat:@"%ld",(long)iQuarter]				forKey:@"quarter"];
	[m_dicMemoryValues setObject:[NSString stringWithFormat:@"%ld",(long)iWeekOfMonth]			forKey:@"weekOfMonth"];
	[m_dicMemoryValues setObject:[NSString stringWithFormat:@"%ld",(long)iWeekOfYear]			forKey:@"weekOfYear"];
	[m_dicMemoryValues setObject:[NSString stringWithFormat:@"%ld",(long)iYearForWeekOfYear]	forKey:@"yearForWeekOfYear"];
...
//Add by Linda 2015.7.6
//Write WMac/BMac/EMac addresses to MLB for Pre-Prov station
- (NSNumber *)WRITE_MAC_ADDRESS:(NSDictionary *)dictPara
                          RETURN_VALUE:(NSMutableString *)strReturn
{
    ...
    if (![fileManager fileExistsAtPath:szFilePath] )
    {
        ...
        NSAlert *alert = [[NSAlert alloc]init];
        alert.messageText = @"警告(Warning)";
        alert.informativeText = @"請確認MAC Address維護文件是否存在";
        [alert addButtonWithTitle:@"确认(OK)"];
        [alert setAlertStyle:NSWarningAlertStyle];
        [alert runModal];
        [alert release];
    ...
    else
    {
        ...
        if ([strMacAddress length] != 12)
        {
            NSAlert *alert = [[NSAlert alloc]init];
            alert.messageText = @"警告(Warning)";
            alert.informativeText = @"請確認MAC Address維護文件格式是否正確";
            [alert addButtonWithTitle:@"确认(OK)"];
            [alert setAlertStyle:NSWarningAlertStyle];
            [alert runModal];
            [alert release];
        ...
        else
        {
            ...
            int iNewMac = [strMacAddrPart intValue] + 4;

            //For station1:
            //15852492 convert to HEX:0xF1E3CC
            if ([strMacAddrPart intValue] == 15852492)
            {
               //15870488 convert to HEX: 0xF22A18
                if (iNewMac >= 15870488)
                {
                    NSAlert *alert = [[NSAlert alloc]init];
                    alert.messageText = @"警告(Warning)";
                    alert.informativeText = @"沒有可用的MAC地址";
                    [alert addButtonWithTitle:@"确认(OK)"];
                    [alert setAlertStyle:NSWarningAlertStyle];
                    [alert runModal];
                    [alert release];
            ...
            //For station2:
            //15852493 convert to HEX:0xF1E3CD
            if ([strMacAddrPart intValue] == 15852493)
            {
                //15870489 convert to HEX: 0xF22A19
                if (iNewMac >= 15870489)
                {
                    NSAlert *alert = [[NSAlert alloc]init];
                    alert.messageText = @"警告(Warning)";
            ...
            //For station3:
            //15852494 convert to HEX:0xF1E3CE
            if ([strMacAddrPart intValue] == 15852494)
            {
                //15870490 convert to HEX: 0xF22A1A
                if (iNewMac >= 15870490)
                {
                    NSAlert *alert = [[NSAlert alloc]init];
                    alert.messageText = @"警告(Warning)";
            ...
            //For station4:
            //15852495 convert to HEX:0xF1E3CF
            if ([strMacAddrPart intValue] == 15852495)
            {
                //15870491 convert to HEX: 0xF22A1B
                if (iNewMac >= 15870491)
                {
                    NSAlert *alert = [[NSAlert alloc]init];
                    alert.messageText = @"警告(Warning)";
            ...
            [strMacAddress setString: [NSString stringWithFormat:@"AC293A%@",strMacAddrPart]];
            [strMacAddrPart release];
            [FirstMacAddr release];
            [strMacAddrHeader release];
			  ...
			  RETURN_VALUE:(NSMutableString *)strReturn
{
    ...
    {
        //Get the content of Mac addr file.
        NSString * strFileContent = [NSString stringWithContentsOfFile:szFilePath encoding:NSUTF8StringEncoding error:nil];
        ATSDebug(@"MACADDR Content: %@",strFileContent);
        NSArray	* arrayContents = [strFileContent componentsSeparatedByString:@"\n"];
        if ([arrayContents count] < 3)
        {
            NSAlert *alert = [[NSAlert alloc]init];
            alert.messageText = @"警告(Warning)";
            alert.informativeText = @"請確認MAC Address配置文件包括如下三個部分的內容：StartAddress:XXXX EndAddress:XXXX StationNumber:X";
            [alert addButtonWithTitle:@"确认(OK)"];
            [alert setAlertStyle:NSWarningAlertStyle];
            [alert runModal];
            [alert release];
        ...
        NSString *strStartAddress;
        NSString * strLastObj = [arrayContents objectAtIndex:([arrayContents count]-2)];
        if ([[strLastObj componentsSeparatedByString:@":"] count] == 1)
        {
            strStartAddress = [[strLastObj componentsSeparatedByString:@","] objectAtIndex:0];
        }else
        {
            strStartAddress = [[[arrayContents objectAtIndex:0] componentsSeparatedByString:@":"] objectAtIndex:1];
        }

         NSString *strEndAddress = [[[arrayContents objectAtIndex:1] componentsSeparatedByString:@":"] objectAtIndex:1];
        int iStationNumber = [[[[arrayContents objectAtIndex:2] componentsSeparatedByString:@":"] objectAtIndex:1] intValue];
		
        if ([strStartAddress length] != 12 ||
            [strEndAddress length] != 12)
        {
            NSAlert *alert = [[NSAlert alloc]init];
            alert.messageText = @"警告(Warning)";
            alert.informativeText = @"請確認MAC Address維護文件中的StartAddress and EndAddress格式是否正確,address长度应该是12位";
            [alert addButtonWithTitle:@"确认(OK)"];
            [alert setAlertStyle:NSWarningAlertStyle];
            [alert runModal];
            [alert release];
        ...
        //Catch the startAddress header and second part.
        NSString * strHeader	=[strStartAddress subByRegex:@"^(.{6})" name:nil error:nil];
        NSString * strPart = [strStartAddress subByRegex:@"^.{6}(.{6})" name:nil error:nil];
        NSMutableString * strMacAddrPart = [[NSMutableString alloc] initWithString:((strPart == nil) ? @"" : strPart)];
        [self NumberSystemConvertion:dictPara RETURN_VALUE:strMacAddrPart];
        
        //Catch the endAddress second part.
        NSString * strEndAddressSecondPart = [strEndAddress subByRegex:@"^.{6}(.{6})" name:nil error:nil];
        NSMutableString * strMacEndAddrPart = [[NSMutableString alloc] initWithString:((strPart == nil) ? @"" : strEndAddressSecondPart)];
        [self NumberSystemConvertion:dictPara RETURN_VALUE:strMacEndAddrPart];
        
        //NewMac = OldMac+stationNumber
        int iNewMac = [strMacAddrPart intValue] + iStationNumber;
        
        //Check the newMac over the endAddress or not.
        if (iNewMac > [strMacEndAddrPart intValue])
        {
            NSAlert *alert = [[NSAlert alloc]init];
            ...
            [alert release];
            [strMacAddress release];
            [strMacEndAddrPart release];
            return [NSNumber numberWithBool:NO];
        }
        
        [strMacAddrPart setString:[NSString stringWithFormat:@"%d",iNewMac]];
        dictPara = [NSDictionary dictionaryWithObjectsAndKeys:
                    [NSNumber numberWithInt:10],	@"CHANGE",
                    [NSNumber numberWithInt:0],		@"FROM",
                    [NSNumber numberWithInt:16],	@"TO", nil];
        ...
        //The header is AC293A.
        [strMacAddress setString: [NSString stringWithFormat:@"%@%@",strHeader,strMacAddrPart]];
        [strMacAddrPart release];
    }
                          ...
                          RETURN_VALUE:(NSMutableString *)strReturn
{
    ...
    NSInteger iOriginal = [mutaStrOriginal length];
    if (iOriginal%8 != 0)
    {
...
- (NSNumber *)FORMAT_HEX_VALUE:(NSDictionary *)dictPara
                  RETURN_VALUE:(NSMutableString *)strReturn
{
    NSString    *szPreKey   = [dictPara objectForKey:@"PREKEY"];
    NSString    *szPostKey    = [dictPara objectForKey:@"POSTKEY"];
    //Set Row string, catch row sub string with range(Begin,Length)
    NSString *strSeparator = [[dictPara objectForKey:@"RowSetting"]objectForKey:@"Separator"];
    NSInteger iBegin = [[[dictPara objectForKey:@"RowSetting"]objectForKey:@"Begin"]integerValue];
    NSInteger iLength = [[[dictPara objectForKey:@"RowSetting"]objectForKey:@"Length"]integerValue];
    if (!iBegin) {
        iBegin = 0;
    }
    //Set Result string, catch Result sub string with range(Begin,Length)
    NSInteger iRetBegin = [[[dictPara objectForKey:@"ResultSetting"]objectForKey:@"Begin"]integerValue];
    NSInteger iRetLength = [[[dictPara objectForKey:@"ResultSetting"]objectForKey:@"Length"]integerValue];
    if (!iRetBegin) {
        iRetBegin = 0;
    }
    NSString    *szPreValue      = [m_dicMemoryValues objectForKey:szPreKey]?[m_dicMemoryValues objectForKey:szPreKey]:strReturn;
    szPreValue = [szPreValue stringByReplacingOccurrencesOfString:@"\t" withString:@" "];
    szPreValue = [szPreValue stringByReplacingOccurrencesOfString:@"0x" withString:@""];
    szPreValue = [szPreValue stringByReplacingOccurrencesOfString:@"0X" withString:@""];
    szPreValue = [szPreValue stringByReplacingOccurrencesOfString:@"\r" withString:@"\n"];
    szPreValue = [szPreValue stringByReplacingOccurrencesOfString:@"\n\n" withString:@"\n"];
    while ([szPreValue contains:@"  "]) {
        szPreValue = [szPreValue stringByReplacingOccurrencesOfString:@"  " withString:@" "];
    }
    //Separated by "\n", Get row data
    NSArray *arrAllData	= [szPreValue componentsSeparatedByString:@"\n"];
    NSMutableArray *arrTargetData = [NSMutableArray array];
    NSMutableString *strTargetString = [NSMutableString stringWithString:@""];
    NSString *strResult = @"";
    for (NSInteger i =0 ; i<[arrAllData count]; i++)
    {
        NSString *strRowValue = [arrAllData objectAtIndex:i];
        strRowValue = [strRowValue stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
        //Every row data separated by strSeparator. and catch the data from array index.
        if (strSeparator)
        {
            NSArray *arrRowData = [strRowValue componentsSeparatedByString:strSeparator];
            if (!iLength || iBegin+iLength>[arrRowData count]) {
                iLength = [arrRowData count]-iBegin;
            }
            for (NSInteger index = iBegin; index < iBegin+iLength; index++)
            {
                NSString *strCell = [arrRowData objectAtIndex:index];
                strCell = [strCell stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
                [arrTargetData addObject:strCell];
            }
        }
        else
        {
            if (!iLength || iBegin+iLength>[strRowValue length]) {
                iLength = [strRowValue length]-iBegin;
            }
            [arrTargetData addObject:[strRowValue substringWithRange:NSMakeRange(iBegin,iLength)]];
        }
    }
    // Tranfer array to string.
    if (strSeparator)
    {
        if (!iRetLength)
        {
            iRetLength = [arrTargetData count]-iRetBegin;
        }
        if (iRetBegin+iRetLength>[arrTargetData count]) {
            [strReturn setString:@"iRetBegin+iRetLength over array limit"];
            return [NSNumber numberWithBool:NO];
        }
        for(NSInteger i = iRetBegin; i<iRetBegin+iRetLength; i++ )
        {
            [strTargetString appendFormat:@"%@ ",[arrTargetData objectAtIndex:i]];
        }
        strResult = [strTargetString stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    }
    else
    {
        for(NSInteger i = 0; i<[arrTargetData count]; i++ )
        {
            [strTargetString appendFormat:@"%@ ",[arrTargetData objectAtIndex:i]];
        }
        if (!iRetLength)
        {
            iRetLength = [strTargetString length]-iRetBegin;
        }
        if (iBegin+iLength>[strTargetString length]) {
            [strReturn setString:@"iRetBegin+iRetLength over string length"];
            return  [NSNumber numberWithBool:NO];
        }
        strResult = [strTargetString substringWithRange:NSMakeRange(iRetBegin,iRetLength)];
        
    }
    //Memory the result string
    if (szPostKey) {
        [m_dicMemoryValues setObject:strResult forKey:szPostKey];
    }
    [strReturn setString:strResult];
    return [NSNumber numberWithBool:YES];
}

- (NSNumber *)CAL_ARC_HEXVALUE:(NSDictionary *)dictPara
                       RETURN_VALUE:(NSMutableString *)strReturn
{
    //init
    NSString    *szKey   = [dictPara objectForKey:@"KEY"];
    NSString *strSeparator = [dictPara objectForKey:@"Separator"];
    NSString    *szValue      = [m_dicMemoryValues objectForKey:szKey]?[m_dicMemoryValues objectForKey:szKey]:strReturn;
    while ([szValue contains:@"  "]) {
        szValue = [szValue stringByReplacingOccurrencesOfString:@"  " withString:@" "];
    }
    NSMutableArray *arrAllData = [NSMutableArray arrayWithArray:[szValue componentsSeparatedByString:strSeparator]];
    
    NSArray *arrChangedIndex = [dictPara objectForKey:@"CALIndex"];
    NSMutableString *strChangeValue = [NSMutableString stringWithString:@""];
    for (NSInteger i = 0; i<[arrChangedIndex count]; i++)
    {
        NSInteger index = [[arrChangedIndex objectAtIndex:i]integerValue];
        if (index>[arrAllData count])
        {
            [strReturn setString:[NSString stringWithFormat:@"index %ld over limit.",(long)index]];
            return [NSNumber numberWithBool:NO];
        }
        [strChangeValue appendFormat:@"%@",[arrAllData objectAtIndex:index]];
    }
    //[strChangeValue setString:@"0014"];
    ATSDebug(@"Catch hex value is: %@",strChangeValue);
    unsigned int	iValue;
    NSScanner		*scanerBuf	= [NSScanner scannerWithString:strChangeValue];
    [scanerBuf scanHexInt:&iValue];
    iValue *=4;
    //add 0 before
    [strReturn setString:[[NSString stringWithFormat:@"%x",iValue]uppercaseString]];
    if ([strReturn length]<[arrChangedIndex count]*2)
    {
        NSMutableString *strTemp = [NSMutableString stringWithString:@""];
        for (int i = 0; i<[arrChangedIndex count]*2-[strReturn length]; i++)
        {
            [strTemp appendString:@"0"];
        }
        [strTemp appendFormat:@"%@",strReturn];
        [strReturn setString:[strTemp uppercaseString]];
    }
    if ([strReturn length]>[arrChangedIndex count]*2)
    {
        NSString *strTemp = [strReturn substringFromIndex:[strReturn length]-[arrChangedIndex count]*2];
        [strReturn setString:[strTemp uppercaseString]];
    }
    ATSDebug(@"The value Multiply 4 is: %@",strReturn);
    //output
    NSMutableString *strTemp = [NSMutableString stringWithString:@""];
    for (NSInteger i = 0; i<[strReturn length]; i=i+2) {
        [strTemp appendFormat:@"%@ ",[strReturn substringWithRange:NSMakeRange(i,2)]];
    }
    [strReturn setString:[strTemp stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]];
    return [NSNumber numberWithBool:YES];
}

- (NSNumber *)MODIFY_HEX_VALUE:(NSDictionary *)dictPara
                  RETURN_VALUE:(NSMutableString *)strReturn
{
    //init
    NSString    *szKey   = [dictPara objectForKey:@"KEY"];
    ...
    NSMutableArray *arrAllData = [NSMutableArray arrayWithArray:[szOriginalValue componentsSeparatedByString:strSeparator]];
    
    NSString    *szModifyKey   = [dictPara objectForKey:@"ModifyKEY"];
    NSString *szValue = [m_dicMemoryValues objectForKey:szModifyKey]?[m_dicMemoryValues objectForKey:szModifyKey]:strReturn;
    while ([szValue contains:@"  "]) {
        szValue = [szValue stringByReplacingOccurrencesOfString:@"  " withString:@" "];
    }
    NSArray *arrChanged = [szValue componentsSeparatedByString:strSeparator];

    NSArray *arrChangedIndex = [dictPara objectForKey:@"ModifyIndex"];
    if ([arrChanged count]<[arrChangedIndex count]) {
        [strReturn setString:@"Format error"];
        return [NSNumber numberWithBool:NO];
    }
    for (NSInteger i =0 ; i<[arrChangedIndex count]; i++)
    {
        NSInteger index = [[arrChangedIndex objectAtIndex:i]integerValue];
        if (index > [arrAllData count]) {
            [strReturn setString:@"index over limit."];
            return [NSNumber numberWithBool:NO];
        }
        [arrAllData setObject:[arrChanged objectAtIndex:i] atIndexedSubscript:index];
    }
    
    NSMutableString *strTemp = [NSMutableString stringWithString:@""];
    for (NSInteger i =0; i<[arrAllData count]; i++) {
        [strTemp appendFormat:@"%@ ",[arrAllData objectAtIndex:i]];
    }
    [strReturn setString:[strTemp stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]];
    [m_dicMemoryValues setObject:strReturn forKey:szKey];
    return [NSNumber numberWithBool:YES];
}


- (NSNumber *)CHECKSUM_ARC_HEXVALUE:(NSDictionary *)dictPara
                  RETURN_VALUE:(NSMutableString *)szReturn
{
    ...
    NSString    *szPostKey    = [dictPara objectForKey:@"POSTKEY"];
    NSString *strSeparator = [dictPara objectForKey:@"Separator"];
    NSString    *szPreValue      = [m_dicMemoryValues objectForKey:szPreKey]?[m_dicMemoryValues objectForKey:szPreKey]:szReturn;
    
    NSMutableArray *arrAllData = [NSMutableArray arrayWithArray:[szPreValue componentsSeparatedByString:strSeparator]];
    int sum = 0;
    NSScanner		*scanerBuf;
    for(NSInteger i = 0; i<[arrAllData count]- 2;i++)
    {
        unsigned int	iValue;
        scanerBuf	= [NSScanner scannerWithString:[arrAllData objectAtIndex:i]];
        [scanerBuf scanHexInt:&iValue];
        sum+=iValue;
    }
    int sum_16bit = sum % 65536;
    int chksum = 65536 - sum_16bit;
    NSString *strHexSum = [[NSString stringWithFormat:@"%x",chksum]uppercaseString];
    //separate by @" "
    NSMutableString *strTemp = [NSMutableString stringWithString:@""];
    for (NSInteger i = 0; i<[strHexSum length]; i=i+2) {
        [strTemp appendFormat:@"%@ ",[strHexSum substringWithRange:NSMakeRange(i,2)]];
    }
    NSString *strRet = [strTemp stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    if (szPostKey) {
        [m_dicMemoryValues setObject:strRet forKey:szPostKey];
    }
    //[szReturn setString:strRet];
    ATSDebug(@"The check sum value is :%@",strRet);
    return [NSNumber numberWithBool:YES];
}
            ...
            RETURN_VALUE:(NSMutableString *)strReturn
{
    ...
    int  iLength     = [dicPara objectForKey:@"RESULTLENGTH"] ? [[dicPara objectForKey:@"RESULTLENGTH"] intValue] : 0;
                       ...
                       RETURN_VALUE:(NSMutableString *)strReturn
{
    ...
    NSString    *szLocalLimitFile  = [dicPara objectForKey:@"LocalLimitFile"];
    
    if (szLocalLimitFile)
    {
        szScriptName = szLocalLimitFile;
    }
    else
    {
        if (nil == szScriptName ||[szScriptName isEqualTo:@""] || nil == szVersion || [szVersion isEqualTo:@""])
        {
    ...
    if (![fileManager fileExistsAtPath:szScriptPath])
    {
        ...
        ATSRunAlertPanel(@"错误(Error)", [NSString stringWithFormat:@"解析%@剧本档失败。(Parser Script Fail)",szScriptName],
                         @"确认(OK)", nil, nil);
    ...
    {
        NSString *strSpec = [[dicSOCLimit objectForKey:szKey] objectForKey:@"spec"];
        [m_dicMemoryValues setObject:strSpec
                              forKey:[NSString stringWithFormat:@"%@_spec",szKey]];
        ATSDebug(@"Sucessful to add the limit [%@] for key [%@]", [dicSOCLimit objectForKey:szKey], szKey);
        if ([strSpec contains:@";cof"]) {
               [m_muArrayLocalCofCase addObject:[[dicSOCLimit objectForKey:szKey] objectForKey:@"mainName"]];
        }
        [m_dicMemoryValues setObject:[[dicSOCLimit objectForKey:szKey] objectForKey:@"units"]
                              forKey:[NSString stringWithFormat:@"%@_units",szKey]];
        ATSDebug(@"Sucessful to add the units [%@] for key [%@]", [dicSOCLimit objectForKey:szKey], szKey);
    }
    return [NSNumber numberWithBool:YES];
}
                    ...
                    RETURN_VALUE:(NSMutableString*)szReturnValue
{
    ...
    for (int i = 1; i < [arrSeparate count]; i++)
    {
...
//Add by raniys for saving 96 pixel parametric data
- (NSNumber *)SAVE_ORB_PARAMETRIC:(NSDictionary *)dicPara
                     RETURN_VALUE:(NSMutableString *)szReturnValue
{
    BOOL bResult        = YES;
    ...
    if (strOriginaleData == nil || [strOriginaleData isEqual:@""] || [strOriginaleData isEqual:@"null"])
    {
        ATSDebug(@"Can't get value from dictionary of Key:%@",dicPara);
        [szReturnValue setString:[NSString stringWithFormat:
                                  @"[%@] not found!",dicPara]];
        bResult = NO;
        return [NSNumber numberWithBool:bResult];
    ...
    else
    {
        ...
        for (int i = 0; i < ([aryRowDatas count]-1); i++)
        {
            ...
            for (int j = 0; j < ([aryColDatas count]-1); j++)
            {
                ...
                //            [m_dicMemoryValues setObject:[aryColDatas objectAtIndex:j] forKey:[NSString stringWithFormat:@"%@%@%lu",strParametricName,@"_",(unsigned long)k++]];//change show format
            }
        }
        
        if ([arrAllValues count] != 0)
        {
            for (int i = 2; i < [arrAllValues count]; i++)
            {
                BOOL    *bDeflection    = YES;
                //Check deflection value
                if ([strParametricName isEqualToString:@"Deflection_1_pix"] || [strParametricName isEqualToString:@"Deflection_2_pix"])
                
                {
                    if ([[arrAllValues objectAtIndex:i] floatValue] >= 0)
                    {
                        bResult = NO;
                        bDeflection = NO;
                    }
                ...
                @synchronized(m_dicMemoryValues)
                {
                    [m_dicMemoryValues setObject:[arrAllValues objectAtIndex:i]
                                          forKey:[NSString stringWithFormat:@"%@_%d",strParametricName,i+1]];
                }
                //
                ...
                //                                         @"NO",@"IsCSVHexParaDec",
                //                                         [NSString stringWithFormat:@"%@_%d",strParametricName,i+1],kFZ_Script_UploadParametric,
                //                                         [NSNumber numberWithBool:bDeflection],@"Result",nil];
...
-(NSNumber *)SCAN_SN_FOR_UPLOAD_FAIL_RECORD:(NSDictionary *)dicPara RETURN_VALUE:(NSMutableString *)strReturn
{
    ...
    else
    {
        ...
        ATSDebug(@"panel return value: %ld",m_iPanelReturnValue);
...
-(void)ShowAlertToStoreFCSN:(id)dicContect
{
    ...
    NSAlert *Alert = [[NSAlert alloc]init];
    [Alert setMessageText:[dicContect objectAtIndex:0]];
    [Alert setInformativeText:@""];
    [Alert addButtonWithTitle:[dicContect objectAtIndex:1]];
    ...
    {
        m_iPanelReturnValue     = (int)button;
        ATSDebug(@"Panel Click %ld",m_iPanelReturnValue);
    }
...
//Check Calibrate status for qt2

-(NSNumber *)CHECK_CALIBRATE_STATUS:(NSDictionary *)dicPara
                       RETURN_VALUE:(NSMutableString *)szReturnValue
{
    
    
    NSString * FixtureID    =szReturnValue;
    NSNumber * Uplimit      =[dicPara objectForKey:@"Uplimit"];
    NSNumber * Lowlimit     =[dicPara objectForKey:@"Lowlimit"];
    NSDictionary * StandardTest =[dicPara objectForKey:@"StandardTest"];
    
    NSFileManager * fileManager =[NSFileManager defaultManager];
    
    if (![fileManager fileExistsAtPath:KSaveDataDefaultPath]) {
        NSDictionary * DailyTest    =[NSDictionary dictionaryWithObjects:@[@"",@""] forKeys:@[@"northData_daily",@"southData_daily"]];
        NSString * SaveTime         =[self FormatterDate];
        
        NSDictionary * dictionary   =[NSDictionary dictionaryWithObjects:@[DailyTest,StandardTest,Uplimit,Lowlimit,SaveTime,FixtureID]
                                                                 forKeys:@[@"DailyTest",@"StandardTest",@"UpLimits",@"LowLimits",@"SaveTime",@"FixtureID"]];
        [dictionary writeToFile:KSaveDataDefaultPath atomically:YES];
        NSAlert *alert = [[NSAlert alloc]init];
        alert.messageText = @"ATTENTION";
        alert.informativeText = @"请重新做校验，并将测试数据正确存入";
        [alert addButtonWithTitle:@"确认(OK)"];
        [alert setAlertStyle:NSWarningAlertStyle];
        [alert runModal];
        [alert release];
    ...
    NSDictionary * dicSaveDataFromFile  =[NSDictionary dictionaryWithContentsOfFile:KSaveDataDefaultPath];
    // update save file when paramatric change
    NSNumber * UplimitsFromFile =[dicSaveDataFromFile objectForKey:@"UpLimits"];
    NSNumber * LowlimitsFronFile    =[dicSaveDataFromFile objectForKey:@"LowLimits"];
    NSDictionary * StandardTestFromFile =[dicSaveDataFromFile objectForKey:@"StandardTest"];
    NSDictionary * dailyTest    =[dicSaveDataFromFile objectForKey:@"DailyTest"];
    NSString * southDailyTest   =[dailyTest objectForKey:@"southData_daily"];
    NSString * northDailyTest   =[dailyTest objectForKey:@"northData_daily"];
    
    NSString * SaveTime         =[dicSaveDataFromFile objectForKey:@"SaveTime"];
    NSString * FixtureIDFromFile    =[dicSaveDataFromFile objectForKey:@"FixtureID"];
    
    //Check daily calibrate data if exist
    
    if ([southDailyTest length] ==0  || [northDailyTest length] ==0 ) {
        NSAlert *alert = [[NSAlert alloc]init];
        alert.messageText = @"ATTENTION";
        alert.informativeText = @"请重新做校验，并将测试数据正确存入";
        [alert addButtonWithTitle:@"确认(OK)"];
        ...
        [alert runModal];
        [alert release];
        ATSDBgLog(@"The daily calibrate data did not been save to file, need to redo calibration.");
        return [NSNumber numberWithBool:NO];
    }
    //Update Static data
    if ((Uplimit != UplimitsFromFile)||(Lowlimit != LowlimitsFronFile) || ![StandardTest isEqual: StandardTestFromFile]) {
        
        NSDictionary * updateDictionary =[NSDictionary dictionaryWithObjects:@[dailyTest,StandardTest,Uplimit,Lowlimit,SaveTime,FixtureIDFromFile]
                                                                     forKeys:@[@"DailyTest",@"StandardTest",@"UpLimits",@"LowLimits",@"SaveTime",@"FixtureID"]];
        NSLog(@"Update File is %@",updateDictionary);
        
        
        [updateDictionary writeToFile:KSaveDataDefaultPath atomically:YES];
        
    }
    
    //Compare if time beyond 24h
    if (![[self CHECK_TIME:SaveTime] boolValue]) {
        NSAlert *alert = [[NSAlert alloc]init];
        alert.messageText = @"ATTENTION";
        alert.informativeText = @"请重新做校验，并将测试数据正确存入";
        [alert addButtonWithTitle:@"确认(OK)"];
        ...
        [alert runModal];
        [alert release];
        ATSDBgLog(@"The time byound 24 until finishing calibration last time, need to redo calibration.");
        return [NSNumber numberWithBool:NO];
        
    }
    //Compare if Fixture id has be changed
    if (!([FixtureID intValue] == [FixtureIDFromFile intValue])) {
        NSAlert *alert = [[NSAlert alloc]init];
        alert.messageText = @"ATTENTION";
        alert.informativeText = @"请重新做校验，并将测试数据正确存入";
        [alert addButtonWithTitle:@"确认(OK)"];
        ...
        [alert runModal];
        [alert release];
        ATSDBgLog(@"The fixture have been changed, need to redo calibration.");
        return [NSNumber numberWithBool:NO];
    }
    ATSDBgLog(@"The save file is %@.\n OK!", dicSaveDataFromFile);
    return [NSNumber numberWithBool:YES];
    
}

// retunn formatter date
...
-(NSNumber *)CHECK_TIME:(NSString *)SaveTime
{
    int OneDayHours =24 * 60 * 60;
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat: @"yyyy-MM-dd HH:mm:ss"];
    NSDate *testTime =[NSDate date];
    NSDate *saveTime= [dateFormatter dateFromString:SaveTime];
    NSTimeInterval TimeBetween  =[testTime timeIntervalSinceDate:saveTime];
    
    if (TimeBetween > OneDayHours) {
        NSLog(@"Test time is %@, Save time is %@, the time between is %f, byound 24 h",testTime,saveTime,TimeBetween)
        ;
        [dateFormatter release];
        return [NSNumber numberWithBool:NO];
    }
    [dateFormatter release];
    NSLog(@"Test time is %@, Save time is %@, the time between is %f, in 24h",testTime,saveTime,TimeBetween)
    ;
...
-(NSNumber *)REPLACE_STRING:(NSDictionary *)dicPara  RETURN_VALUE:(NSMutableString*)strReturnValue
{
    NSString *strSourceKey = [dicPara objectForKey:@"SourceKey"];
    NSString *strSoruce = [m_dicMemoryValues objectForKey:strSourceKey];
    NSString *strReplace = [dicPara objectForKey:@"Replace"];
    NSString *strWithString = [dicPara objectForKey:@"WithString"];
    if (strWithString==nil) {
        strWithString = @"";
    }
    NSString *strResult = [strSoruce stringByReplacingOccurrencesOfString:strReplace withString:strWithString];
    if (strResult)
    {
        [strReturnValue setString:strResult];
         return [NSNumber numberWithBool:YES];
    }
    return [NSNumber numberWithBool:NO];
}
...
- (NSNumber *)MAPPING_TABLE_FOR_CRB:(NSDictionary *)dicSetting
               RETURN_VALUE:(NSMutableString *)strReturnValue
{
    //If the setting dictionary has no parameters, post a alert panel.
    if (!dicSetting || [dicSetting count] == 0) {
        ATSRunAlertPanel([NSString stringWithFormat:@"警告(Warning)"],
                        @"MAPPING_TABLE:RETURN_VALUE:没有参数。(There are no parameters for MAPPING_TABLE!)",
                        @"确认(OK)", nil, nil);
        return [NSNumber numberWithBool:NO];
    }
    
    NSString		*strEEEEKey			= [dicSetting objectForKey:@"EEEE_KEY"];
    NSString		*strCCCCKey			= [dicSetting objectForKey:@"CCCC_KEY"];
    NSString		*strDummySNKey		= [dicSetting objectForKey:@"DummySN_KEY"];
    NSString		*strMPNKey			= [dicSetting objectForKey:@"MPN_KEY"];
    NSString		*strRegionCodeKey	= [dicSetting objectForKey:@"RegionCode_KEY"];
    NSString		*strEEEEValue		= [m_dicMemoryValues objectForKey:strEEEEKey];
    
    //Get mapping table from setting file.
    NSDictionary    *dicEEEETable    = [[self getValueFromXML:kPD_UserDefaults
                                                      mainKey:@"MAPPING_TABLE",nil]objectForKey:strEEEEValue];
    
    NSString        *strCCCCValue       = [dicEEEETable objectForKey:@"CCCC"];
    NSString        *strDummySNValue    = [dicEEEETable objectForKey:@"Dummy SN"];
    NSString        *strMPNValue        = [dicEEEETable objectForKey:@"MPN"];
    NSString        *strRegionCodeValue = [dicEEEETable objectForKey:@"Region Code"];
    
    if (!dicEEEETable || [dicEEEETable count] ==0)
    {
        ATSRunAlertPanel([NSString stringWithFormat:@"警告(Warning)"],
                        @"设定挡中的MAPPING_TABLE没有这个EEEE code。",
                        @"确认(OK)", nil, nil);
        return [NSNumber numberWithBool:NO];
    }
    
    //Some parameters of MAPPING_TABLE are missing.
    if (strCCCCValue == nil || [strCCCCValue isEqualToString:@""]
        || strDummySNValue ==nil || [strDummySNValue isEqualToString:@""]
        || strMPNValue ==nil || [strMPNValue isEqualToString:@""]
        || strRegionCodeValue ==nil || [strRegionCodeValue isEqualToString:@""])
    {
        ATSRunAlertPanel([NSString stringWithFormat:@"警告(Warning)"],
                        @"设定挡中MAPPING_TABLE缺少参数。(Some parameters of MAPPING_TABLE are missing!)",
                        @"确认(OK)", nil, nil);
        return [NSNumber numberWithBool:NO];
    }
    
    //Set the matched value according the value of KEY.
    
    [m_dicMemoryValues setObject:strCCCCValue forKey:strCCCCKey];
    [m_dicMemoryValues setObject:strDummySNValue forKey:strDummySNKey];
    [m_dicMemoryValues setObject:strMPNValue forKey:strMPNKey];
    [m_dicMemoryValues setObject:strRegionCodeValue forKey:strRegionCodeKey];
    
    return [NSNumber numberWithBool:YES];
}
-(NSNumber*)TEST_ITEM_SYNC:(NSDictionary*)dicPara
              RETURN_VALUE:(NSMutableString*)strReturnValue
{
    NSLog(@"[sync]Post notification to muifa");
    m_bTestItemSYNCContinue = NO;
    NSNotificationCenter *notification = [NSNotificationCenter defaultCenter];
    [notification postNotificationName:NotificationTestItemSYNC
                                object:self
                              userInfo:nil];
    while (1)
    {
        if (m_bTestItemSYNCContinue)
        {
            NSLog(@"[sync]Continue the rest test");
            break;
        }
        sleep(1);
    }
...
#pragma mark ############################## For Touch and Velvet Calculate ##############################
-(NSNumber *)GET_PERCENT_VALUE:(NSDictionary *)dicSetting
                   RETURN_VALUE:(NSMutableString *)strReturnValue
{
    NSArray *arrData = [m_dicMemoryValues objectForKey:[dicSetting objectForKey:@"KEY"]];
    int iDecimalPlaces	= [[dicSetting objectForKey:@"DecimalPlaces"] intValue];
    if (!arrData || [arrData count]==0)
    {
        [strReturnValue setString:@"Array data error!"];
        return [NSNumber numberWithBool:NO];
    }
    if([dicSetting objectForKey:@"INDEX"])
    {
        NSInteger iIndex = [[dicSetting objectForKey:@"INDEX"]integerValue];
        if (iIndex > [arrData count]-1)
        {
            [strReturnValue setString:@"INDEX is out!"];
            return [NSNumber numberWithBool:NO];
        }
        double fResult = [[arrData objectAtIndex:iIndex]doubleValue];
        [strReturnValue setString:[NSString stringWithFormat:@"%.*f",iDecimalPlaces,fResult]];
        return [NSNumber numberWithDouble:YES];
    }
    if ([dicSetting objectForKey:@"AllJudgeSpec"])
    {
        NSMutableString *strFailIndex = [NSMutableString stringWithString:@""];
        for (NSInteger i = 0; i<[arrData count]; i++)
        {
            double data = [[arrData objectAtIndex:i]doubleValue];
            [strReturnValue setString:[NSString stringWithFormat:@"%f",data]];
             bool Ret = [[self JUDGE_SPEC:[dicSetting objectForKey:@"AllJudgeSpec"] RETURN_VALUE:strReturnValue]boolValue]
            ;
            if (!Ret)
            {
                [strFailIndex appendString:[NSString stringWithFormat:@"%ld ",(long)i]];
            }
        }
        if ([strFailIndex isEqualToString:@""])
        {
            [strReturnValue setString:@"PASS"];
        }
        else
            [strReturnValue setString:strFailIndex];
        return [NSNumber numberWithBool:YES];
    }
    return [NSNumber numberWithBool:YES];
}

-(NSNumber *)CAL_PERCENT_MEDIAN:(NSDictionary *)dicSetting
               RETURN_VALUE:(NSMutableString *)strReturnValue
{
    NSArray *arrData = [m_dicMemoryValues objectForKey:[dicSetting objectForKey:@"KEY"]];
    if(!arrData || [arrData count]==0)
    {
        [strReturnValue setString:@"Format error"];
        return [NSNumber numberWithBool:NO];
    }
    BOOL byCol = [dicSetting objectForKey:@"BYCOL"];
    ...
    if ([arrData count]==1)
    {
        arrAVG = [arrData objectAtIndex:0];
    }
    else
        arrAVG = [self CAL_AVG_VALUE:arrData BYCOL:byCol];
    if (!arrAVG)
    {
        [strReturnValue setString:@"Format error!"];
        return [NSNumber numberWithBool:NO];
    ...
    //get Median Value
    double fMedian = [self GetMedianValue:arrAVG];
    ATSDebug(@"The Median Value before calculate is:%f",fMedian);
    if ([dicSetting objectForKey:@"MEMORY_MEDIAN"])
    {
        [m_dicMemoryValues setObject:[NSString stringWithFormat:@"%f",fMedian]
                              forKey:[dicSetting objectForKey:@"MEMORY_MEDIAN"]];
    }
    
    //Store the percent array.
    NSMutableArray *arrPercent = [NSMutableArray array];
    for (id obj in arrAVG)
    {
        ...
        double fElement = [obj doubleValue];
        double fPercent = fabs((fElement-fMedian)/fMedian*100);
        [arrPercent addObject:[NSNumber numberWithDouble:fPercent]];
    }
    [strReturnValue setString:[NSString stringWithFormat:@"%@",arrPercent]];
    if ([dicSetting objectForKey:@"MEMORY_PERCENT"]) {
        [m_dicMemoryValues setObject:[NSArray arrayWithArray:arrPercent] forKey:[dicSetting objectForKey:@"MEMORY_PERCENT"]];
    }
    
    //get max value
    NSNumber *maxNumber = [[m_mathLibrary BringOrderToArray:arrPercent]lastObject];
    ATSDebug(@"The MAX in PERCENT Value is:%@",maxNumber);
    if ([dicSetting objectForKey:@"MEMORY_MAX"]) {
        [m_dicMemoryValues setObject:[NSString stringWithFormat:@"%@",maxNumber]
                              forKey:[dicSetting objectForKey:@"MEMORY_MAX"]];
    }
    return [NSNumber numberWithDouble:YES];
}

//get AVG Values by Row or Col
-(NSArray *)CAL_AVG_VALUE:(NSArray *)arrData
                 BYCOL:(BOOL)byCol
{
    if ([arrData count]==0 || !arrData) {
        return nil;
    }
    NSMutableArray *arrAvg = [NSMutableArray array];
    NSInteger iRow = [arrData count];
    NSInteger iCol = [[arrData objectAtIndex:0]count];
    if(byCol)
    {
         NSMutableArray *arrColData = [NSMutableArray array];
        for (NSInteger i = 0; i<iCol; i++)//12
        {
            for (NSInteger j =0; j<iRow; j++)//24
            {
                [arrColData addObject:[[arrData objectAtIndex:j]objectAtIndex:i]];
            }
            double fAvgValue = [m_mathLibrary GetAverageWithArray:arrColData NeedABS:YES];
            [arrAvg addObject:[NSNumber numberWithDouble:fAvgValue]];
            [arrColData removeAllObjects];
        }
    }
    else
    {
        for (id obj in arrData)
        {
            if ([obj isKindOfClass:[NSArray class]])
            {
                double fAvgValue = [m_mathLibrary GetAverageWithArray:obj NeedABS:YES];
                [arrAvg addObject:[NSNumber numberWithDouble:fAvgValue]];
            }
            else
            {
    ...
    return [NSArray arrayWithArray:arrAvg];
}

//get Median Value
- (double)GetMedianValue:(NSArray *)aryValues
{
    NSArray *arrSort = [m_mathLibrary BringOrderToArray:aryValues];
    if (!arrSort || [arrSort count]==0) {
        return 0;
    }
    NSInteger midIndex = 0;
    if ([arrSort count]%2 == 0)
    {
        midIndex = [aryValues count]/2;
        double mid1 = [[arrSort objectAtIndex:midIndex]doubleValue];
        double mid2 = [[arrSort objectAtIndex:midIndex-1]doubleValue];
        return (mid1+mid2)/2;
    }
    else
        midIndex = ([arrSort count]-1)/2;
    return [[arrSort objectAtIndex:midIndex]doubleValue];
}

//Calculate the Mag Value
- (NSNumber *)CAL_MAG_ARRAY:(NSDictionary *)dicSetting
                  RETURN_VALUE:(NSMutableString *)strReturnValue
{
    NSInteger iBeginRow = [dicSetting objectForKey:@"BEGIN_ROW"]?[[dicSetting objectForKey:@"BEGIN_ROW"]integerValue]:0;
    NSInteger iEndRow   = [[dicSetting objectForKey:@"END_ROW"]integerValue];
    NSInteger iBeginCol = [dicSetting objectForKey:@"BEGIN_COL"]?[[dicSetting objectForKey:@"BEGIN_COL"]integerValue]:0;
    NSInteger iEndCol   = [[dicSetting objectForKey:@"END_COL"]integerValue];
    NSString *strInPhase = [m_dicMemoryValues objectForKey:[dicSetting objectForKey:@"InPhaseKey"]];
    NSString *strQuadrature = [m_dicMemoryValues objectForKey:[dicSetting objectForKey:@"QuadratureKey"]];
    
    NSArray *arrInPhase = [self GET_MATRIX_ARRAY:strInPhase BEGIN_ROW:iBeginRow END_ROW:iEndRow BEGIN_COL:iBeginCol END_COL:iEndCol];
    NSArray *arrQuadrature =[self GET_MATRIX_ARRAY:strQuadrature BEGIN_ROW:iBeginRow END_ROW:iEndRow BEGIN_COL:iBeginCol END_COL:iEndCol];
    if (!arrInPhase || !arrQuadrature || [arrInPhase count]!=[arrQuadrature count])
    {
        [strReturnValue setString:@"Can not get right Inphase and Quadrature array!"];
        return [NSNumber numberWithBool:NO];
    }
    NSMutableArray *arrMagRowData = [NSMutableArray array];
    NSMutableArray *arrMag = [NSMutableArray array];
    for (NSInteger i=0; i<iEndRow-iBeginRow+1; i++)
    {
        for (NSInteger j=0; j<iEndCol-iBeginCol+1; j++)
        {
            double fPhase = [[[arrInPhase objectAtIndex:i]objectAtIndex:j]doubleValue];
            double fQuadrature = [[[arrQuadrature objectAtIndex:i]objectAtIndex:j]doubleValue];
            double fMag = sqrt(pow(fPhase, 2) +pow(fQuadrature,2));
            [arrMagRowData addObject:[NSNumber numberWithDouble:fMag]];
        }
        [arrMag addObject:[NSArray arrayWithArray:arrMagRowData]];
        [arrMagRowData removeAllObjects];
    }
    
    [strReturnValue setString:@"The Mag Value is:\n"];
    for (id obj in arrMag)
    {
        for (id number in obj) {
            NSString *strTemp = [NSString stringWithFormat:@"%.2f\t",[number doubleValue]];
            [strReturnValue appendString:strTemp];
        }
        [strReturnValue appendString:@"\n"];
    }
    
    if ([dicSetting objectForKey:@"MEMORY_KEY"]) {
        [m_dicMemoryValues setObject:[NSArray arrayWithArray:arrMag] forKey:[dicSetting objectForKey:@"MEMORY_KEY"]];
    }
    return [NSNumber numberWithBool:YES];
}

- (NSArray *)GET_MATRIX_ARRAY:(NSString *)strValue
                     BEGIN_ROW:(NSInteger)iBeginRow
                       END_ROW:(NSInteger)iEndRow
                     BEGIN_COL:(NSInteger)iBeginCol
                       END_COL:(NSInteger)iEndCol

{
    ...
    strValue = [strValue stringByReplacingOccurrencesOfString:@"\r\r" withString:@"\r"];
    NSArray *arrRowData = [strValue componentsSeparatedByString:@"\r"];
    if(iBeginRow > iEndRow ||
       iBeginCol > iEndCol ||
       !strValue ||
       !arrRowData ||
       [arrRowData count]==0 ||
       [arrRowData count]<=iEndRow)
    {
        return nil;
    }
        
    NSMutableArray *arrMatrix = [NSMutableArray array];
    for (NSInteger i=iBeginRow; i<=iEndRow; i++)
    {
        NSString *strOneRow = [arrRowData objectAtIndex:i];
        strOneRow = [strOneRow stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
        NSArray *arrOneRow = [strOneRow componentsSeparatedByString:@"\t"];
        if ([arrOneRow count]<=iEndCol) {
            return nil;
        }
        NSArray *arrTarRow = [arrOneRow subarrayWithRange:NSMakeRange(iBeginCol, iEndCol-iBeginCol+1)];
        [arrMatrix addObject:arrTarRow];
    }
    return [NSArray arrayWithArray:arrMatrix];
}
-(NSNumber *)JUDGE_IF_EXIST_MISMATCH_VALUE:(NSDictionary *)dicSetting
                              RETURN_VALUE:(NSMutableString *)strReturnValue
{
    NSString * strItemKey   =[dicSetting objectForKey:@"ItemName"];
    NSString * strMemoryKey =[m_dicMemoryValues objectForKey:[dicSetting objectForKey:@"KEY"]];
    
    NSString * strContainString =[NSString stringWithFormat:@"Tigris channel  %@ : Reading mismatch",strItemKey];
    if ([strMemoryKey containsString:strContainString]) {
        [strReturnValue setString:@"Reading mismatch"];
         return [NSNumber numberWithBool:YES];
    }
    [strReturnValue setString:@"OK"];
    return [NSNumber numberWithBool:YES];

}

-(NSNumber*)USBF_TASK_START:(NSDictionary*)dictContents
               RETURN_VALUE:(NSMutableString*)strReturnValue
{
    // Get path and check.
    NSString	*strPath	= [dictContents objectForKey:@"PATH"];
    ...
    // Terminate old task.
    [m_UsbfTask terminate];
    [m_UsbfTask release];	m_UsbfTask	= nil;
    // Create new task.
    m_UsbfTask	= [[NSTask alloc] init];
    [m_UsbfTask setStandardInput:[NSPipe pipe]];
    NSPipe * outPut =[NSPipe pipe];
    
    [m_UsbfTask setStandardOutput:outPut];
    
    
    [m_UsbfTask setStandardError:[m_UsbfTask standardOutput]];
    [m_UsbfTask setLaunchPath:strPath];
    
    if(aryArgs && [aryArgs isKindOfClass:[NSArray class]])
        [m_UsbfTask setArguments:aryArgs];
    @try
    {
        [m_UsbfTask launch];
    }
...
-(NSNumber*)USBF_TASK_END:(NSDictionary*)dictContents
             RETURN_VALUE:(NSMutableString*)strReturnValue
{
    @try
    {
        if([m_UsbfTask isRunning])
            [m_UsbfTask terminate];
    }
    @catch (NSException *exception)
    {
...
-(NSNumber *)FILE_HANDLE:(NSDictionary *)dicContents
            RETURN_VLAUE:(NSMutableString *) strReturnValue
{
    
    
    NSDictionary * Move_File_Path =[dicContents objectForKey:@"Move_File"];
    NSDictionary * Copy_File_Path =[dicContents objectForKey:@"Copy_File"];
    NSDictionary * Remove_File_Path =[dicContents objectForKey:@"Remove_File"];
    ...
    if (Move_File_Path)
    {
        NSString * str_Move_Original_Path =[Move_File_Path objectForKey:@"original_path"];
        NSString * str_Move_target_Path = nil;
        
        if ([[dicContents objectForKey:@"DefaultTargetPath"] boolValue])
        {
            NSString * filename =[str_Move_Original_Path subByRegex:@".*/(.*)" name:nil error:nil];
            str_Move_target_Path =[NSString stringWithFormat:@"%@/%@_%@/%@",kPD_LogPath,m_szPortIndex,m_szStartTime,filename];
            NSLog(@"the target path is %@", str_Move_target_Path);
        }
        else
        {
            str_Move_target_Path =[Move_File_Path objectForKey:@"target_path"];
        }
        if(![fm fileExistsAtPath:str_Move_Original_Path])
        {
            ATSDebug(@"File can not find at the path %@", str_Move_Original_Path);
            
            return [NSNumber numberWithBool:NO];
            
        }
        NSURL * original_path_url =[NSURL fileURLWithPath:str_Move_Original_Path];
        NSURL * target_path_url =[NSURL fileURLWithPath:str_Move_target_Path];
        if([fm moveItemAtURL:original_path_url toURL:target_path_url error:&error])
        {
            ATSDebug(@"File have be moved to path %@", str_Move_target_Path);
            [strReturnValue setString:[NSString stringWithFormat:@"File have be moved to the path %@", str_Move_target_Path]];
            return [NSNumber numberWithBool:YES];
            
        }
        else
        {
            ATSDebug(@"Move file fail !");
    ...
    if (Copy_File_Path)
    {
        NSString * str_Copy_Original_Path =[Copy_File_Path objectForKey:@"original_path"];
        NSString * str_Copy_target_Path = nil;
        if ([[dicContents objectForKey:@"DefaultTargetPath"] boolValue])
        {
            NSString * filename =[str_Copy_Original_Path subByRegex:@".*/(.*)" name:nil error:nil];
            str_Copy_target_Path =[NSString stringWithFormat:@"%@/%@_%@/%@",kPD_LogPath,m_szPortIndex,m_szStartTime,filename];
            
        }
        else
        {
            str_Copy_target_Path =[Copy_File_Path objectForKey:@"target_path"];
        }
        NSString * OriginalFileName = [fm displayNameAtPath:str_Copy_Original_Path];
        NSString * TargetFileName =[fm displayNameAtPath:str_Copy_target_Path];
        if(![fm fileExistsAtPath:str_Copy_Original_Path])
        {
            ATSDebug(@"File can not find at the path %@", str_Copy_Original_Path);
            
            return [NSNumber numberWithBool:NO];
        }
        if ([[Copy_File_Path objectForKey:@"ReplaceSameFile"] boolValue] && [OriginalFileName isEqualToString:TargetFileName])
        {
            [fm removeItemAtPath:str_Copy_target_Path error:&error];
            if ([fm copyItemAtPath:str_Copy_Original_Path toPath: str_Copy_target_Path error:&error])
            {
                ATSDBgLog(@"Have copy to target path");
                [strReturnValue setString:[NSString stringWithFormat:@"File have be replaced to the path %@", str_Copy_target_Path]];
                return [NSNumber numberWithBool:YES];
        ...
        if( [fm copyItemAtPath:str_Copy_Original_Path toPath:str_Copy_target_Path error:&error])
        {
            [strReturnValue setString:[NSString stringWithFormat:@"File have be copied to the path %@", str_Copy_target_Path]];
            ATSDebug(@"File have be copy to path %@", str_Copy_target_Path);
    ...
    if (Remove_File_Path)
    {
        NSString * str_Remove_Original_Path =[Copy_File_Path objectForKey:@"original_path"];
        if(![fm fileExistsAtPath:str_Remove_Original_Path])
        {
            ATSDebug(@"File can not find at the path %@", str_Remove_Original_Path);
            
            return [NSNumber numberWithBool:NO];
        }
        
        if ( [fm removeItemAtPath:str_Remove_Original_Path error:nil])
        {
            ATSDebug(@"File have be remove from path %@", str_Remove_Original_Path);
...
-(NSNumber *)REPLACESTRINGWITH:(NSDictionary *)dicPara
                  RETURN_VALUE:(NSMutableString *)szReturnValue
{
    NSString *key = [dicPara objectForKey:kFZ_Script_MemoryKey];
    NSMutableString *NeedReplaceStr= [NSMutableString string];
    if ([m_dicMemoryValues objectForKey:key]!=nil) {
        
        [NeedReplaceStr setString:[m_dicMemoryValues objectForKey:key]];
    }
    else
    {
        [szReturnValue setString:@"Can not find CFG# im m_dicMemoryValues."];
        return [NSNumber numberWithBool:NO];
    }
    if ([NeedReplaceStr containsString:@"_"])
    {
        NSString *str = [NeedReplaceStr stringByReplacingOccurrencesOfString:@"_" withString:@"-"];
        [szReturnValue setString:str];
    }
    else
    {
        [szReturnValue setString:NeedReplaceStr];
    }
    return [NSNumber numberWithBool:YES];
}


-(NSNumber *)Cal_CheckSum_Satin:(NSDictionary *)dicPara RETURN_VALUE:(NSMutableString *)strReturnValue
{
    NSString    *strKey         = [m_dicMemoryValues  objectForKey:[dicPara objectForKey:@"KEY"]];
    NSString    *strSeperate    = [dicPara objectForKey:@"SeparateString"];
    int         iLength         = [[dicPara objectForKey:@"LENGTH"] intValue];
    int         iBeginIndex     = [[dicPara objectForKey:@"BeginIndex"] intValue];
    //int         iCheckIndex     = [[dicPara objectForKey:@"CheckValueIndex"] intValue];
    strKey = [strKey stringByReplacingOccurrencesOfString:@" " withString:@""];
    NSArray * arrCheckSumValue =[strKey componentsSeparatedByString:strSeperate];
    NSMutableString * strTemp =[NSMutableString string];
    uint32_t iSum = 0;
    uint32_t iCheckValue ;
    
    if (iBeginIndex+iLength > [arrCheckSumValue count])
    {
        ATSDebug(@"The BeginIndex + LENGTH is out of range.");
        return [NSNumber numberWithBool:NO];
    }
    
    for (int i = iBeginIndex; i< iBeginIndex+iLength; i++) {
        
        dicPara	= [NSDictionary dictionaryWithObjectsAndKeys:
                   [NSNumber numberWithInt:16],	@"CHANGE",
                   [NSNumber numberWithInt:10],	@"TO", nil];
        [strTemp setString:[arrCheckSumValue objectAtIndex:i]];
        
        [self NumberSystemConvertion:dicPara
                        RETURN_VALUE:strTemp];
        iSum	+= [strTemp intValue];
        NSLog(@"The check sum of satin is %X",iSum);
    }
    
    ATSDebug(@"The check sum of satin is 0x%X",iSum);
    uint32_t iResult = iSum & 0xFF;
    
    ATSDebug(@"The final check sum result is %X",iResult);
    if (iResult != 0)
    {
        [strReturnValue setString:[NSString stringWithFormat:@"%X",iSum]];
        return [NSNumber numberWithBool:NO];
    }
    [strReturnValue setString:[NSString stringWithFormat:@"%X",iSum]];
    return [NSNumber numberWithBool:YES];
