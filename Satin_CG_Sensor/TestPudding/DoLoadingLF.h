...
typedef IP_API_Reply		def_IP_addAttribute(IP_UUTHandle inHandle,
												const char* name,
												const char* value );
typedef IP_TestSpecHandle	def_IP_testSpec_create( void );
typedef bool				def_IP_testSpec_setTestName(IP_TestSpecHandle testSpecHandle,
														const char* name,
														size_t nameLength );
typedef bool				def_IP_testSpec_setSubTestName(IP_TestSpecHandle testSpecHandle,
															const char* name,
															size_t nameLength );
typedef bool				def_IP_testSpec_setSubSubTestName(IP_TestSpecHandle testSpecHandle,
																const char* name,
																size_t nameLength );
typedef bool				def_IP_testSpec_setLimits(IP_TestSpecHandle testSpecHandle,
														const char* lowerLimit,
														size_t lowerLimitLength,
														const char* upperLimit,
														size_t upperLimitLength );
typedef bool				def_IP_testSpec_setUnits(IP_TestSpecHandle testSpecHandle,
														const char* units, size_t unitsLength );
typedef bool				def_IP_testSpec_setPriority(IP_TestSpecHandle testSpecHandle,
														enum IP_PDCA_PRIORITY priority );
typedef IP_TestResultHandle	def_IP_testResult_create( void );
typedef bool				def_IP_testResult_setResult(void* testResultHandle,
														enum IP_PASSFAILRESULT result );
typedef bool				def_IP_testResult_setValue(void* testResultHandle,
														const char* value,
														size_t valueLength );
typedef bool				def_IP_testResult_setMessage(void* testResultHandle,
															const char* message,
															size_t messageLength );
typedef IP_API_Reply		def_IP_addResult(IP_UUTHandle inHandle,
...
typedef IP_API_Reply		def_IP_addBlob(IP_UUTHandle inHandle,
											const char* inBlobName,
											const char* inPathToBlobFile );
typedef IP_API_Reply		def_IP_UUTDone(IP_UUTHandle inHandle );
typedef IP_API_Reply		def_IP_UUTCommit(IP_UUTHandle inHandle,
												enum IP_PASSFAILRESULT inPassFail );
typedef IP_API_Reply		def_IP_validateSerialNumber(IP_UUTHandle inHandle,
														const char* serialNumber );
typedef IP_API_Reply		def_IP_amIOkay(IP_UUTHandle inHandle,
											const char* inUUTSerialNumber );
typedef IP_API_Reply		def_IP_getGHStationInfo(IP_UUTHandle handleGHStation,
													enum IP_ENUM_GHSTATIONINFO eGHStationInfo,
													char** cppValue,
													size_t *sLength );//Leehua get json file
typedef IP_API_Reply		def_IP_setDUTPosition(IP_UUTHandle inHandle,
