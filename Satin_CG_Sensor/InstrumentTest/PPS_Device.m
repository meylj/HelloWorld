		   ...
		   PrimaryAddr:(int)nPrimaryAddr
{
	...
	nRetCode		= [super setInitialDevice:nBoardID
							PrimaryAddr:nPrimaryAddr];
	if (nRetCode == kSuccessCode)
	{
		...
		if ((aRange.location != NSNotFound)
			&& (aRange.length > 0)
			&& ((aRange.length + aRange.location) <=  [aDeviceName length]))
	...
	NSLog(@"%d = setInitialDevice(nBoardID=%d,nPrimaryAddr=%d)",
		  nRetCode,nBoardID,nPrimaryAddr);
	return nRetCode;
}

-(int)SetVoltage:(int)nChannel
		 Voltage:(float)fVoltage
{
	int			nRetCode	= kSuccessCode;
	NSString	*aStr, *aCmd;
...
-(int)SetCurrLimit:(int)nChannel
		   Current:(float)fCurrent
{
	int			nRetCode	= kSuccessCode;
	NSString	*aStr,*aCmd;
...
-(int)SetPowrSwitch:(int)nChannel
			PowerON:(BOOL)bOnOff
{
	int			nRetCode	= kSuccessCode;
	NSString	*aStr,*aCmd;
