...
{
    [m_PPS_Device SetCurrLimit:[edtChannel intValue]
					   Current:[edtCurrLimit floatValue]];
}

- (IBAction)SetVoltageAct:(id)sender
{
    [m_PPS_Device SetVoltage:[edtChannel intValue]
					 Voltage:[edtVoltage floatValue]];
}

- (IBAction)setOutputOffAct:(id)sender
{
    [m_PPS_Device SetPowrSwitch:[edtChannel intValue]
						PowerON:NO];
}
...
{
	[m_Device_34970 setInitialDevice:[edtBoardID1 intValue]
						 PrimaryAddr:[edtPriAddr1 intValue]];
}

- (IBAction)OpenRoutAct:(id)sender
{
	[m_Device_34970 setRoutSwitch:NO
					 ChannelLists:[edtChannel1 stringValue]];
}

- (IBAction)CloseRoutAct:(id)sender
{
	[m_Device_34970 setRoutSwitch:YES
					 ChannelLists:[edtChannel1 stringValue]];
}

- (IBAction)GetDC_VoltageAct:(id)sender
{
	[m_Device_34970 getMeasureVoltage:YES
						 ChannelLists:[edtChannel1 stringValue]
							  Voltage:aryVoltage];
}
...
{
	[m_Device_34970 getMeasureVoltage:NO
						 ChannelLists:[edtChannel1 stringValue]
							  Voltage:aryVoltage];
}
...
{
	[m_Device_34970 getMeasureCurrent:YES
						 ChannelLists:[edtChannel1 stringValue]
							  Current:aryCurrent];
}
...
{
	[m_Device_34970 getMeasureCurrent:NO
						 ChannelLists:[edtChannel1 stringValue]
							  Current:aryCurrent];
}
...
{
	[m_Device_34970 getMeasureFrequency:[edtChannel1 stringValue]
							  Frequency:aryFrequency];
}
...
- (IBAction)GetResistanceAct:(id)sender
{
	...
	if ([[cbResistanceRange stringValue] isEqualToString:@"100 MΩ"])
		nRange	= k100Mohm;
	else if ([[cbResistanceRange stringValue] isEqualToString:@"10 MΩ"])
		nRange	= k10Mohm;
	else if ([[cbResistanceRange stringValue] isEqualToString:@"1 MΩ"])
		nRange	= k1Mohm;
	else if ([[cbResistanceRange stringValue] isEqualToString:@"100 kΩ"])
		nRange	= k100Kohm;
	else if ([[cbResistanceRange stringValue] isEqualToString:@"10 kΩ"])
		nRange	= k10Kohm;
	else if ([[cbResistanceRange stringValue] isEqualToString:@"1 kΩ"])
		nRange	= k1Kohm;
	else if ([[cbResistanceRange stringValue] isEqualToString:@"100Ω"])
		nRange	= k100ohm;
	else
		nRange	= kAutoRange;
...
{
	[m_Device_53131 setInitialDevice:[edtBoardID2 intValue]
						 PrimaryAddr:[edtPriAddr2 intValue]];
}
