...
@synthesize lbPercentageNumber;				// auto test system
@synthesize m_dicPorts;					    // auto test system

@synthesize templateDelegate;

//For SMT-QT 2UP
@synthesize isFinished  = m_bIsFinished;
@synthesize strSMFixtureInfo;
//For Pressure Test , Leehua
@synthesize publicParas         = m_objPublicParams;

//For Live & JSON file
@synthesize isLiveChanging  = m_LiveChanging;
@synthesize UIVersion       = m_szUIVersion;
...
//		m_bRunningWithUARTTab = [[bundleDict objectForKey:@"LSUIElement"] boolValue];
       ...
       // [NSBundle loadNibNamed:@"Template" owner:self];
        [[NSBundle mainBundle]loadNibNamed:@"Template" owner:self topLevelObjects:nil];
        ...
        //modify for live function, need to change UI version
        m_szUIVersion   = [[NSMutableString alloc] init];
...
- (void)dealloc
{
    ...
    //Modify for live function, need to change the UI version
...
- (void)MonitorFixtureHaveClickStart
{
	...
	PEGA_ATS_UART	*uartFixture	= [[PEGA_ATS_UART alloc] init];
...
- (IBAction)startTest:(id)sender
{
    ...
    if ([[m_dicGHInfo objectForKey:descriptionOfGHStationInfo(IP_LINE_NUMBER)] ContainString:@"REL"] &&
        ![[m_dicMuifa_Plist objectForKey:@"NoDeleteScanerForREL"] boolValue])
        ...
        else
        {
            ...
            else
            {
                ...
                if (m_iSNNumber+1 != [arrSNArrangement count]) 
                {
                    ...
                    NSLog(@"startTest: set next focus to sn id(%lu)", (unsigned long)m_iSNNumber);
					m_bRunning	= NO;
                    return;
                }
                else
...
- (BOOL)checkAboveSNRule
{
    ...
    if (nil != arrSNArrangement) 
    {
        NSInteger	iCheckNumber	= m_iSNNumber;
        NSLog(@"checkAboveSNRule: check sn numbers = %ld", iCheckNumber+1);
        for (int i = 0; i <= iCheckNumber; i++)
        {
            ...
            if (bSNNeedCheckRule)
            {
                ...
                if (!bLength)
                {
                    ...
                    alert.messageText = szRunAlertTitle;
                    alert.informativeText = [NSString stringWithFormat:@"%@长度不匹配，长度应该为[%@]。(%@ length should be [%@])",szSNKey,[dicSNInfomation objectForKeyedSubscript:kUserDefaultSNRule_Length],szSNKey,[dicSNInfomation objectForKeyedSubscript:kUserDefaultSNRule_Length]];
				...
				if (bIsCheckDigital)
				{
					...
					if (!isDigital)
					{
                        ...
                        NSAlert *alert = [[NSAlert alloc]init];
                        alert.messageText = szRunAlertTitle;
                        alert.informativeText = [NSString stringWithFormat:@"%@[%@]不是数值类型的！(%@ is not digital!)",szSNKey,szSNValue,szSNKey];
                        [alert addButtonWithTitle:@"确认(OK)"];
                        [alert setAlertStyle:NSWarningAlertStyle];
                        [alert runModal];
                        [alert release];
                ...
                if (nil != szNoContainString && ![szNoContainString isEqualToString:@""])
				{
                    ...
                    if (NSNotFound != rangeNoContain.location)
					{
                        ...
                        NSAlert *alert = [[NSAlert alloc]init];
                        alert.messageText = szRunAlertTitle;
                        alert.informativeText = [NSString stringWithFormat:@"%@中含有不期待的字符\"%@\",請重刷正確序號。(%@ contain invalid key string \"%@\",Please re-scan with the correct SN)",szSNKey,szNoContainString,szSNKey,szNoContainString];
                        [alert addButtonWithTitle:@"确认(OK)"];
                        [alert setAlertStyle:NSWarningAlertStyle];
                        [alert runModal];
                        [alert release];
                ...
                if (nil != szContainString && ![szContainString isEqualToString:@""])
                {
                    ...
                    if (!bContain)
                    {
                        ...
                        NSAlert *alert = [[NSAlert alloc]init];
                        alert.messageText = szRunAlertTitle;
                        alert.informativeText = [NSString stringWithFormat:@"%@中没有特定的字符\"%@\"。(%@ does not contain key string \"%@\")",szSNKey,szTempString,szSNKey,szTempString];
                        [alert addButtonWithTitle:@"确认(OK)"];
                        [alert setAlertStyle:NSWarningAlertStyle];
                        [alert runModal];
                        [alert release];
                ...
                if (nil != szContainOneOfString && ![szContainOneOfString isEqualToString:@""])
                {
                    ...
                    if (!bContainOneOf)
                    {
                        ...
                        NSAlert *alert = [[NSAlert alloc]init];
                        alert.messageText = szRunAlertTitle;
                        alert.informativeText = [NSString stringWithFormat:@"%@中没有特定的字符\"%@\"。(%@ does not contain key string \"%@\")",szSNValue,szTempString,szSNValue,szTempString];
                        [alert addButtonWithTitle:@"确认(OK)"];
                        [alert setAlertStyle:NSWarningAlertStyle];
                        [alert runModal];
                        [alert release];
...
- (void)startSingleTest:(NSDictionary *)dicInfo
{
    ...
    for(int iCount = 0;iCount < m_iRunCount;iCount++)
    {
        ...
        //for SMTQT fixture info
        if (strSMFixtureInfo != nil) {
            
            [testProgress.memoryValues setObject:strSMFixtureInfo forKey:@"SMT-QT Fixture Info"];
            
        }
        // get start time and transfer to FunnyZone
		testProgress.startTime	= [[NSDate date] descriptionWithCalendarFormat:@"YYYY-MM-dd HH-mm-ss"
																		 timeZone:nil
																		   locale:nil];
        testProgress.startDate = [[NSDate date] descriptionWithCalendarFormat:@"YYYYMMdd"
																		 timeZone:nil
																		   locale:nil];
...
//                    g_iPassForNFMU ++;
				...
				{
					[lbTotalCount setStringValue:[NSString stringWithFormat:@"LoopTest: %d/%ld (P:%d F:%d)",iCount+1, (long)m_iRunCount, iPass, iFail]];
				}
				else
				{
	...
	/* No need to select the table view after finished test
	if (m_bRunningWithUARTTab)
		[tabTestInfo selectTabViewItemAtIndex:2];
	*/
    [pool drain];
...
- (void)setDefaultValueAndGetScriptInfo:(NSString *)szIdentifier
{
    ...
    //For Live & JSON file
    if ([[m_dicMuifa_Plist objectForKey:@"DisableLiveFunction"] boolValue])
    {
        [m_szUIVersion setString:[[[NSBundle mainBundle] infoDictionary] valueForKey:@"CFBundleShortVersionString"]];
    }
    ...
    @synchronized(g_szMutex_Lock)
    {
        ...
        if(0 != m_iRunCount && 1 != m_iRunCount)
        {
            ...
            [lbTotalCount setStringValue:[NSString stringWithFormat:
										  @"LoopTest: %d/%ld (P:%d F:%d)",
										  iHaveRunCycleCount, (long)m_iRunCount, iPass, iFail]];
        }
        else
        {
            // if normal test
...
-(void)setTimeIndicator:(id)iThread
{
    ...
    while (m_bRunning)
    {
        ...
        NSString	*szHour		= [NSString stringWithFormat:@"%ld",(long)iHour];
        if (1==[szHour length])
            szHour	= [NSString stringWithFormat:@"0%ld", (long)iHour];
        NSString	*szMinute	= [NSString stringWithFormat:@"%ld",(long)iMinute];
        if (1==[szMinute length])
            szMinute	= [NSString stringWithFormat:@"0%ld", (long)iMinute];
        NSString	*szSecond	= [NSString stringWithFormat:@"%ld",(long)iSecond];
        if (1==[szSecond length])
            szSecond	= [NSString stringWithFormat:@"0%ld", (long)iSecond];
						...
						row:(NSInteger)rowIndex
{
    ...
    @synchronized(arrCommon)
    {
        ...
        if (nil == identifier)
        {
            NSLog(@"TABLEVIEW ERROR: Nil indentifier at row index: %ld", rowIndex);
            [pool drain];
            return @"";
        }
        ...
        {
            theValue	= @"";
            NSLog(@"TABVIEW EERO: Tableview can't get information at index \"%ld\"", rowIndex);
        }
    }
	[pool drain];
	return theValue; 
                  ...
                  completionHandler:^(NSModalResponse returnCode) {
                      if (returnCode == NSAlertFirstButtonReturn)
                      {
                          if (returnCode == NSAlertFirstButtonReturn)
                          {
                              ...
                              if ([szNewScriptFile isEqualTo:@"FATP_CB Erase.plist"] )
                                  [self changeSNManage];
                              //=============modify for Clear CB===============
                              //modified by lucy for test once or not once
                              [self parseAndLoadScriptFile:szNewScriptFile
                                              OnlyTestOnce:m_bTestOnceScript];
                              BOOL	bNoSnRuleCheck	= [[[[kFZ_UserDefaults objectForKey:kUserDefaultScriptInfo]
                                                         objectForKey:kUserDefaultTestOnceScriptName]
                                                        objectForKey:@"NOSNRuleCheck"] boolValue];
...
- (void)showScanSN:(NSNotification *)notification
{
    ...
    for (NSInteger i = 0, x = 30 + 40*[arrayKeys count]; i < [arrayKeys count]; i++)
    {
        // set panel height
...
- (IBAction)checkMPNRegion:(id)sender 
{
    ...
    for (int i =0;i < [arrayTxt count]; i++) 
    {
        ...
        NSInteger	iLength	= [szTxt length];
