...
- (NSDictionary *)getStationInfo
{
    ...
    for (NSInteger iIndex=1; iIndex<IP_GHSTATIONINFO_COUNT; iIndex++) 
    {
        ...
        NSLog(@"getGHStationInfo : %@ = %@",szKey , [dicGhInfo objectForKey:szKey]);
    } 
...
- (NSArray *)parserScriptFile:(NSString *)szScriptFilePath
{
	...
	if (!arrTestItems)
	{
		...
		NSLog(@"Terminate at [%@], Parser Script fail, Get Nil array from the path [%@]", [[NSDate date] description], szScriptFilePath);
		[NSApp terminate:nil];
		return nil;
	}
...
- (BOOL)singleItemTest:(NSArray *)arrDetailTest ItemName:(NSString *)szItemName currentIndex:(NSInteger)iIndex
{
    ...
    [m_dicMemoryValues setObject:[NSString stringWithFormat:@"%.6fs",dSingleItemSpend] forKey:kFZ_SingleTime];
...
/*  NSString	*strStationNameIni = [self getValueFromXML:kPD_UserDefaults mainKey:kPD_UserDefaults_ScriptFileName,nil];
    if([strStationNameIni isEqualTo:@"Prox Cal.plist"])
    {
        if(![self IS_NeedTo_Start])
        {
            return;
        }
    }
*/
	...
	NSString *szCSVBegin	= [NSString stringWithFormat:@"TestName,Status,Value,DownLimit,UpLimit,SingleTime\n"];
	...
	[IALogs CreatAndWriteSingleCSVLog:szBeginCSV withPath:szCSVFileName];
    ...
    //start PDCA
    //[self start_PDCA];
	if (!m_bNoUploadPDCA && [m_objPudding StartPDCA_Flow] != kFZ_InstantPudding_Success)
    {
	...
	for (NSUInteger i = 0; i < [m_arrayScript count]; i++)
	{
        ...
        //set default
        //Set cancel to end when this item test fail 
        m_bCancelToEnd = NO;
		...
		if (m_bAbort)
		{
            ...
            int iCancelNumber = 3; //default cancel to end number = 3
           /*
        ...
        else if([[m_dicForceFaileCase allKeys] containsObject:szCurrentItemName])
        {
        ...
        else if ([[m_dicForcePassCase allKeys] containsObject:szCurrentItemName])
        {
...
//		
        ...
        {
            if ([strStatusColor isEqualToString:@"RED"])
            {
                imageResult = [NSImage imageNamed:NSImageNameStatusUnavailable];
                m_szStatusColor = @"RED";
            }
            else if ([strStatusColor isEqualToString:@"YELLOW"])
            {
                imageResult = [NSImage imageNamed:NSImageNameStatusPartiallyAvailable];
            ...
            else if ([strStatusColor isEqualToString:@"WHITE"])
            {
    ...
    if(![[self getValueFromXML:kPD_UserDefaults mainKey:kPD_UserDefaults_NoCycleTimeLog,nil]boolValue])
    {
        ...
        {
            szFinalResult = @"FAIL";
            NSSpeechSynthesizer *bitch = [[NSSpeechSynthesizer alloc] init];
            [bitch startSpeakingString:@"NG"];
            [bitch release];
...
- (void)endDUTPrg:(double)dStartTime TestResult:(BOOL)bResult
{
    ...
    BOOL bSave = [[self getValueFromXML:kPD_UserDefaults mainKey:kPD_UserDefaults_SaveLog,nil] boolValue];//wheter save log (just one flag to control all the logs)
    ...
    if (bIS_1_to_N)
    {
        ...
        {
            NSString *szPortPathTemp = [[[m_dicPorts objectForKey:@"MOBILE"] objectAtIndex:0] objectForKey:@"Slot1"];
             ...
             Mode:(int)iMode
{
	...
	else
	{
		...
		double	dValue		= [szValue doubleValue];
...
- (NSString *)formatLog_transferObject:(id)objItem
{
    ...
    {
        szRet = [NSString stringWithFormat:@"%d",objItem];
    }
