...
-(NSNumber*)COMPASS_SAMPLE:(NSDictionary*)dicContents RETURN_VALUE:(NSMutableString*)strReturnValue
{
	...
	if([strReturnValue length] >= 10)
	{
		...
		if([arrayRawData count] >= count)
		{
			...
			{
                //ATSDebug(@"%d",i);
				arrayRawDataItem[i] = [[arrayRawData objectAtIndex:i] componentsSeparatedByString:@","];
				if ([arrayRawDataItem[i] count] >= 4)
				{
					if([arrayRawDataItem[i] count] == 4)
					{
						dCompassData_X = [[arrayRawDataItem[i] objectAtIndex:1]doubleValue];
						dCompassData_Y = [[arrayRawDataItem[i] objectAtIndex:2]doubleValue];
						dCompassData_Z = [[arrayRawDataItem[i] objectAtIndex:3]doubleValue];
					}
					...
					dCompassData_X = ((doubleSEN_X-128.0)*0.5/128.0+1)*0.15*dCompassData_X;
					dCompassData_Y = ((doubleSEN_Y-128.0)*0.5/128.0+1)*0.15*dCompassData_Y;
					dCompassData_Z = ((doubleSEN_Z-128.0)*0.5/128.0+1)*0.15*dCompassData_Z;
    ...
    if (strReturnValue != nil)
    {
        ...
        if ([arrayRowData count] >= iCount)
        {
            ...
            for (int k=0; k < 12; k++)
            {
                ...
                {
                    double data = [[arraySensorReadingValue[m] objectAtIndex:k ] doubleValue];
                    
                    dAverageStage[k] += data; 
                    [arrayStageData[k] addObject:[NSNumber numberWithDouble:data]];
                }
                dAverageStage[k] /=iCount*1.0;
                [self Cal_STD:[NSArray arrayWithArray: arrayStageData[k]] ReturnValue:&szSTD];
            ...
            else
            {
                ...
                NSString *szBaselineProx_Adj = [m_dicMemoryValues valueForKey:@"baselineProx_Adj"];
...
-(NSNumber*)WRITECPCL:(NSDictionary*)dicContents RETURN_VALUE:(NSMutableString*)strReturnValue
{
	...
	//Determine the Critical Angle, meaning the position that measured the lowest Delta Value of all Three Position Fixtures
...
- (NSNumber *)CHANGE_TESTCASENAME_WITH_BOARDID:(NSDictionary *)dicContents RETURN_VALUE:(NSMutableString *)szReturnValue
{
    ...
    if ([szBoardId isEqualToString:@"0x0C"]) 
    {
        ...
        szCurrentTestItem = [NSString stringWithFormat:@"P106 %@",[m_dicMemoryValues valueForKey:kFunnyZoneCurrentItemName]];
    }
    else if ([szBoardId isEqualToString:@"0x0E"])
    {
		[m_strSpecName setString:kFZ_Script_JudgeCommonP107];
        szCurrentTestItem = [NSString stringWithFormat:@"P107 %@",[m_dicMemoryValues valueForKey:kFunnyZoneCurrentItemName]];
    }
    else if([szBoardId isEqualToString:@"0x0A"]) 
    {
        ATSDebug(@"the board shouldn't be tested at this station %@",szBoardId);
        szReturnValue = [NSString stringWithFormat:@"Wrong Board id! %@",szBoardId];
        m_bAbort = YES;
        NSRunAlertPanel(@"警告(Warning)", @"请停止测试，P105机台。(This unit is P105, Please Stop Test)", @"确认(OK)", nil, nil);
        return [NSNumber numberWithBool:NO];
    }
    else
    {
...
- (NSNumber *)CHANGE_TESTCASENAME_WITH_NAME:(NSDictionary *)dicContents RETURN_VALUE:(NSMutableString *)szReturnValue
{
    ...
    szCurrentTestItem = [NSString stringWithFormat:@"%@ %@",szName, [m_dicMemoryValues valueForKey:kFunnyZoneCurrentItemName]];
