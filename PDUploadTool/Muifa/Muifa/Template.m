...
NSString * const BNRSingleTestCompeteNotification = @"SingleTestcomplete";
NSString * const BNRSingleTestStartNotification = @"SingleTestStart";
NSString * const BNRChangeScriptFileMotification= @"ChangeScriptFile";
NSString * const BNRCancelChangeScriptFileMotification= @"CancelNoChangeScriptFile";
...
@synthesize m_dicEmptyResponse;

// auto test system (need this port to change sn)
@synthesize testProgress;

//add for auto init fixture
...
- (id)initWithParametric:(NSDictionary *)dicPara WithSlot:(int)iSlot
{
    ...
    if (self) 
    {
        ...
        m_bNoNeedConsole = [[[[NSDictionary dictionaryWithContentsOfFile:kMuifaPlistPath] objectForKey:@"Log_Path"] objectForKey:@"NoNeedConsole"] boolValue];
        ...
        else
        {
            ...
            m_szFixturePortPath = [[[m_dicPorts objectForKey:@"FIXTURE"] objectAtIndex:0] retain];
        }
        //
        ...
        m_iSNNumber = 0;
        m_szTestScriptFilePath =[[self getPlanPlistNameByProductLineName] retain];
    ...
    // auto test system (iSlot)
    m_iSlot = iSlot;
    
    m_bDislplayMode = YES;
    //set Display item name
    m_aryDisplayItem = [[NSArray alloc] initWithArray:[[[NSDictionary dictionaryWithContentsOfFile:kMuifaPlistPath] objectForKey:@"ScriptInfo"] objectForKey:@"DisplayItemName"]];
...
- (void)dealloc
{
    ...
    if (m_strControlCable)[m_strControlCable release];
    
    
    [m_aryDisplayItem release];
...
- (void)MonitorSerialPortPlugOut
{
    ...
    if (nil==testProgress)
    {
        ...
        //set dislpay mode, add by sam
        testProgress.m_bDisplayMode = m_bDislplayMode;
        testProgress.m_aryDisplayName = m_aryDisplayItem;
...
// funciton for monitor unit by fixture for Auto run test
- (void)MonitorUnitInFromFixture
{
   ...
   // bool    bIS_1_to_N  =   [[[m_dicMuifa_Plist objectForKey:@"ModeSetting"] objectForKey:@"IS_1_to_N"] boolValue];
    ...
    NSLog(@"MonitorUnitInFromFixture begin");
    testProgress.m_bMonitorIn = YES;
    [testProgress monitorUnitInByFixtureWithPath:m_szFixturePortPath];
    NSLog(@" MonitorUnitInFromFixture end");
    
    
    //normal end
    if(1 == testProgress.m_iFlagIn)
    {
        [lbUnitMark setStringValue:[NSString stringWithFormat:@"[%@]%@", m_szCurrentUnit, szSNInUnit]];
        [NSThread detachNewThreadSelector:@selector(startTest:) toTarget:self withObject:nil];
    }
...
// funciton for monitor unit by fixture for Auto run test
- (void)MonitorUnitOutFromFixtureForInitFixture
{
    NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
    NSMutableString * szSNInUnit= [[NSMutableString alloc] init];
    // bool    bIS_1_to_N  =   [[[m_dicMuifa_Plist objectForKey:@"ModeSetting"] objectForKey:@"IS_1_to_N"] boolValue];
    ...
    NSLog(@" MonitorUnitInFromFixture begin");
    testProgress.m_bMonitorOut = YES;
    [testProgress monitorUnitOutByFixtureWithPath:m_szFixturePortPath];
    NSLog(@" MonitorUnitInFromFixture end");
    
    if(testProgress.m_iFlagOut == 1)
    {
        [lbUnitMark setStringValue:[NSString stringWithFormat:@"[%@]%@", m_szCurrentUnit, szSNInUnit]];
        [NSThread detachNewThreadSelector:@selector(startTest:) toTarget:self withObject:nil];
    }
...
- (void)MonitorUnitOutFromFixture
{
    ...
    NSLog(@" monitorUnitOutByfixture begin");
    testProgress.m_bMonitorOut = YES;
    [testProgress monitorUnitOutByFixtureWithPath:m_szFixturePortPath];
    NSLog(@" monitorUnitInByixture end");
    ...
    if(testProgress.m_iFlagOut == 1)
    {
        ...
        [lbUnitMark setStringValue:m_szCurrentUnit];
        // NSLog(@" MonitorSerialPortPlugOut end");
        [NSThread detachNewThreadSelector:@selector(MonitorUnitInFromFixture) toTarget:self withObject:nil];
    }
...
- (IBAction)startTest:(NSButton *)sender
{
    ...
    if (!bAutoDetectRun && (nil != arrSNArrangement) && (0 != [arrSNArrangement count])/* && !m_bManualClickTest*/)
    { 
        ...
        else
        {
            ...
            if (bBrotherTest)
            {
                ...
                if ([arrObjTemplate containsObject:self])
                {
                    ...
                    {
                        NSString *szAlert = [NSString stringWithFormat:@"請重新刷入SN, 當前的SN: %@ 已重複刷入了。(You have scanned the same SN with previous slots.Please scan SN again)",szCurrentSN];
                        NSRunAlertPanel(@"警告(Warning)", szAlert, @"确认(OK)", nil, nil, nil);
...
- (BOOL)checkAboveSNRule
{
    ...
    if (nil != arrSNArrangement) 
    {
        ...
        for (int i = 0; i <= iCheckNumber; i++)
        {
            ...
            {
                // check sn rule
				//check sn prefix and suffix ，added by Lucky
				NSString	*strPrefix = [dicSNInfomation objectForKey:kUserDefaultSNRule_Prefix];
				if (strPrefix == nil || [strPrefix isEqualToString:@""] || [szSNValue hasPrefix:strPrefix])
				{
					NSLog(@"CheckAboveSNRule:check sn prefix rule Pass or no need check sn prefix");
				}else
				{
					NSLog(@"checkAboveSNRule: check sn length rule: FAIL");
					NSString *szRunAlertTitle = [NSString stringWithFormat:@"错误(Error)(slot:%@)", m_szCurrentUnit];
					NSRunAlertPanel(szRunAlertTitle, @"%@内容不匹配，前缀应该为%@。(%@ Prefix should be %@)", @"确定(OK)", nil, nil, szSNKey,strPrefix,szSNKey, strPrefix);
				...
				// suffix
				NSString	*strSuffix = [dicSNInfomation objectForKey:kUserDefaultSNRule_Suffix];
				if (strSuffix ==nil || [strSuffix isEqualToString:@""]||[szSNValue hasSuffix:strSuffix])
				{
					 NSLog(@"checkAboveSNRule: check sn suffix rule pass or no need to check sn suffix");
				}else
				{
					NSLog(@"checkAboveSNRule: check sn length rule: FAIL");
					NSString *szRunAlertTitle = [NSString stringWithFormat:@"错误(Error)(slot:%@)", m_szCurrentUnit];
                    NSRunAlertPanel(szRunAlertTitle, @"%@内容不匹配，后缀应该为%@。(%@ Suffix should be %@)", @"确定(OK)", nil, nil, szSNKey,strSuffix,szSNKey, strSuffix);
					//UI
                    [[m_arrSNTextFields objectAtIndex:i] setStringValue:@""];
					if (bRet)
                    {
...
- (void)startSingleTest:(NSDictionary *)dicInfo
{
    ...
    [nc postNotificationName:BNRSingleTestStartNotification object:self];
...
//======================  add for interposer and grape-1, when the start button have pressed , start test auto.========
    ...
    if (bBrotherTest || bAntsTest)
    {
        ...
        if (bBrotherTest)
        {
            ...
            // transter UI mode to FunnyZone
            testProgress.m_szMuifaUIMode = m_szCurrentMode;
...
//========================== add for interposer and grape-1, when the start button have pressed , start test auto.===========
    ...
    for(int iCount = 0;iCount < m_iRunCount;iCount++)
    {
     ...
     //jeff   
        ...
        // auto test system (regist noti)
        if (isRaft)
        {
            [nc addObserver:self selector:@selector(test_finished:) name:TEST_FINISHED object:testProgress];
        }
        ...
        @synchronized(m_aryFail)
        {
            [m_aryFail removeAllObjects];
        }
        
        @synchronized(m_aryCSV)
        {
            [m_aryCSV removeAllObjects];
        }
        ...
        [tvCSVInfo performSelectorOnMainThread:@selector(reloadData) withObject:nil waitUntilDone:NO];
        [tvFailInfo performSelectorOnMainThread:@selector(reloadData) withObject:nil waitUntilDone:NO];
        [tvUARTInfo performSelectorOnMainThread:@selector(reloadData) withObject:nil waitUntilDone:NO];
...
//        [tvUARTInfo reloadData];
        ...
        [testProgress.m_dicNoResponse setDictionary:m_dicEmptyResponse];
               ...
               if (m_bTestOnceScript)
                {
                    ...
                    m_bTestOnceScript =NO;
                    [self parseAndLoadScriptFile:[self getPlanPlistNameByProductLineName] OnlyTestOnce:m_bTestOnceScript];
                    if (isRaft) {
                        [self RegisteStation];
                    }
                }
                // auto test system (remove 2nd regist)
//                    [self parseAndLoadScriptFile:m_szTestScriptFilePath OnlyTestOnce:m_bTestOnceScript];
				...
				// auto test system (remove noti)
                if (gbIfNeedRemoteCtl)
                {
                    [nc removeObserver:self name:TEST_FINISHED object:testProgress];//for automation by Leehua
                }
                [testProgress release];
                testProgress = nil;
                ...
                if(m_bDislplayMode)
                {
                    [m_aryCSV setArray:m_arrCSVInfo];
                    [m_aryFail setArray:m_arrFailInfo];
				...
				else
				{
                    ...
                    BOOL    bAutoDetectRun  = [[[m_dicMuifa_Plist objectForKey:kUserDefaultModeSetting]
                                                objectForKey:kUserDefaultAutoDetectAndRun] boolValue];
                    BOOL    bAutoFixtureRun  = [[[m_dicMuifa_Plist objectForKey:kUserDefaultModeSetting]
                                                objectForKey:kUserDefaultMonitorFixture] boolValue];
                    // auto test system(NO autorun)
                    if (gbIfNeedRemoteCtl){
                        bAutoDetectRun  = NO;
                    }
                    if (bAutoDetectRun)
                    {
                        if(bAutoFixtureRun)
                        {
                            // open a thread to monitor fixture whether unit out fixture and run test
                            [NSThread detachNewThreadSelector:@selector(MonitorUnitOutFromFixture) toTarget:self withObject:nil];
                        }
                        else
                        {
    ...
    [tfSN6 setEditable:YES];
    [btnStart setEnabled:YES];
    // auto test system (Disable Start button for manual test at auto line)
    if (!gbIfNeedRemoteCtl&&([[lbScriptFile stringValue] ContainString:@"RAFT.plist"]))
    {
        [btnStart setEnabled:NO];
    }
    ...
    [nc postNotificationName:BNRSingleTestCompeteNotification object:self];
...
-(void)processNormalTest:(NSNotification*)noti
{
    ...
    BOOL bDisplayMode = [[dicResult objectForKey:kAllTableViewDisplayMode] boolValue];
    ...
    {
        //[tvCSVInfo performSelectorInBackground:@selector(reloadData) withObject:nil];
        [tvCSVInfo performSelectorOnMainThread:@selector(reloadData) withObject:nil waitUntilDone:NO];
        //[tvCSVInfo reloadData];
    }
...
- (void)setDefaultValueAndGetScriptInfo:(NSString *)szIdentifier CurrentMode:(NSString *)szCurrentMode
{
   ...
   // [lbTotalCount setStringValue:@"Count: 0 (Pass:0 Fail:0)"];
    ...
    if(m_bDislplayMode)
    {
        [lbDislplayMode setStringValue:@"DisplayMode: Normal"];
    }
    else
    {
        [lbDislplayMode setStringValue:@"DisplayMode: Engineer"];
    }
    ...
    //auto test system(choose the RAFT.plist)
    NSString * szPlanFileName  = [self getPlanPlistNameByProductLineName];
    ...
    if (isRaft && !bAutoRunInit)
    {
        szPlanFileName  = [[m_dicMuifa_Plist objectForKey:kUserDefaultScriptInfo]objectForKey:kUserDefaultRaftScriptName];
        if ([szPlanFileName isEqualToString:@""] || szPlanFileName == nil)
        {
            szPlanFileName = [[[m_dicMuifa_Plist objectForKey:kUserDefaultScriptInfo] objectForKey:kUserDefaultScriptFileName]objectForKey:@"DefaultFileName"];
        }
        m_szTestScriptFilePath =[szPlanFileName retain];
        if (![self GetSNRuleCheckByPlist])
        {
...
- (void)setSNDefaultLables
{
    ...
    else
    {
        ...
        // Only grape-1 test call becomFirstResponder
        // If grape-1 call this function will be crashed
        BOOL bBrotherTest = [[[[m_dicMuifa_Plist objectForKey:kUserDefaultModeSetting] objectForKey:@"BROTHER"] objectForKey:@"BrotherTest"] boolValue];
        if(!bBrotherTest)
        {
...
- (NSString *)Get_Script_Version;
{
    ...
    // get scriptFileName
    NSString *scriptFileName = [self getPlanPlistNameByProductLineName];
    // get scriptFilePath
...
- (void)controlTextDidBeginEditing:(NSNotification *)aNotification
{
    if ([m_arrSNTextFields containsObject:[aNotification object]]) 
    {
        ...
        if ([[[m_dicMuifa_Plist objectForKey:kUserDefaultModeSetting] objectForKey:kUserDefaultManualStartTest] boolValue]
            && [m_szCurrentMode isEqualToString:kMuifa_Window_Mode_MFMU])
        {
            if ([m_arrObjTemplate containsObject:self])
            {
...
//set and open a alert panel to notice test mode modify to hide display mode
...
- (void)alertDidEnd:(NSAlert *)alert returnCode:(NSInteger)returnCode contextInfo:(void *)contextInfo
{
    ...
    if (returnCode == NSAlertFirstButtonReturn) 
    {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:BNRChangeScriptFileMotification object:self];
    }
    else if(returnCode == NSAlertSecondButtonReturn)
    {
        // if click NO button, do nothing
        
        [[NSNotificationCenter defaultCenter] postNotificationName:BNRCancelChangeScriptFileMotification object:self];
    }
    else
    {
        // else, do nothing
...
- (IBAction)CheckMPNRegion:(id)sender 
{
    ...
    for (int i =0;i < [arrayTxt count]; i++) 
    {
        ...
        else if ((0 != [arraySize objectAtIndex:i]) && (iLength != [[arraySize objectAtIndex:i] intValue]))
        {
            ...
            NSRunAlertPanel(@"错误(Error)", [NSString stringWithFormat:@"%@长度不匹配，长度应该为%d。(%@ lengh error ! must be %d)",szName,iTemp,szName,iTemp], @"确认(OK)", nil, nil,nil);
...
-(void)showMessage:(NSNotification *)note
{
   ...
   // NSView *view = [viewInfo superview];
    ...
    NSView *view = [viewDisplayMode superview];
    NSRect rect1 = [viewDisplayMode frame];
    ...
    if (bHaveButton)
    {
        ...
        switch (m_iBtnCount) {
            case 1:
            {
                ...
                [btnPass setTitle:[szAttString string]];
                [szAttString release];
                break;
            }
            case 2: 
            {
                ...
                [btnPass setTitle:[szAttPassString string]];
                [btnFail setTitle:[szAttFailString string]];
...
{           
    ...
    if (m_strControlCable != nil) {
        [m_strControlCable release];
        m_strControlCable = nil;
    }
    m_strControlCable = [[NSString stringWithString:szUUT] retain];
    ...
    if (![szUUT isEqualToString:[NSString stringWithString:m_strSerialPort]]) 
    {
        ...
        if ([objTestProgress checkUnitConnectWellWithFixturePort:szFixturePort UARTCommand:@"clr channels" UartResponse:szFixtureResponse CheckTime:1.0]) 
        {
            ...
            else
            {
                ...
                m_strControlCable = [@"NONE" retain];
                NSDictionary *dicFixture = [NSDictionary dictionaryWithObjectsAndKeys:m_strControlCable,@"FIXTURENUMBER",nil];
                [dicFixture writeToFile:kFxitureControlPlistPath atomically:NO];
                [btnChooseCable setState:NSOffState];
                NSRunAlertPanel(@"Warning", strWarning, @"YES", nil, nil, nil);
            }
        }
        else
        {
...
- (NSString *)getPlanPlistNameByProductLineName
{
    NSDictionary * dicPlanFileName = [[m_dicMuifa_Plist objectForKey:kUserDefaultScriptInfo] objectForKey:kUserDefaultScriptFileName];
    NSString *szLineName = [m_dicGHInfo objectForKey:kPD_GHInfo_STATION_ID];
    NSString *szPlanFileName = [dicPlanFileName objectForKey:@"DefaultFileName"];
    
    NSArray *arrKeys = [dicPlanFileName allKeys];
    int iKeys = [arrKeys count];
    for (int i = 0; i < iKeys; i++)
    {
        if ([szLineName ContainString:[arrKeys objectAtIndex:i]])
        {
            szPlanFileName = [dicPlanFileName objectForKey:[arrKeys objectAtIndex:i]];
            break;
        }
        else
        {
    ...
    return szPlanFileName;
}

// auto test system(registe station)
-(void)RegisteStation
{
    if (gbIfNeedRemoteCtl) {
        m_testStation = [AppleControlledStation controlledStationWithName:[NSString stringWithFormat:@"TESTER-%d",m_iSlot]
                                                                 andClass:[m_dicGHInfo objectForKey:kPD_GHInfo_STATION_ID]
                                                         andNumberOfSlots:[NSNumber numberWithInt:m_iSlot]]; //this must be the same with that in config.plist file
        [m_testStation setDelegate:self];
        [AppleTestStationAutomation registerStation:m_testStation];
    }
}

// auto test system(test finish)
...
//    NSString *szFailInfo = [[note userInfo] objectForKey:@"Fail Info"];
    NSString *szErrorInfo = [[note userInfo] objectForKey:@"Error Info"];
    NSString *szISN = [[note userInfo] objectForKey:@"ISN"]?[[note userInfo] objectForKey:@"ISN"]:@"XXX";
    if (szErrorInfo != nil && ![szErrorInfo isEqualToString:@""])// normal error
    {
        m_bAlreadyError = YES;
        NSLog(@"TTTTTTError");
        return;
    ...
    if (bResult) // Pass
    {
        NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:eTraveler_TestPassedResult, eTraveler_TestResultKey, [NSString stringWithFormat:@"%d", m_iSlot], eTraveler_TrayIDKey, szISN, eTraveler_SerialNumberKey, nil];
        NSArray *result = [NSArray arrayWithObject:dic];
        [AppleTestStationAutomation testStation:m_testStation finishedWithResults:result];
    }
    else // Fail
    {
        NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:eTraveler_TestFailedResult, eTraveler_TestResultKey, [NSString stringWithFormat:@"%d", m_iSlot], eTraveler_TrayIDKey, szISN, eTraveler_SerialNumberKey, nil];
        NSArray *result = [NSArray arrayWithObject:dic];
        [AppleTestStationAutomation testStation:m_testStation finishedWithResults:result];
    }
}
...
// auto test system(receive start from client)
- (BOOL) station:(AppleControlledStation *)station startWithTravelers:(NSArray *)travelers {
    ...
    NSLog(@"Perform button on Slot %d", m_iSlot);
    m_bAlreadyError = NO;
    ...
    {
        NSLog(@"Template Index Error on Slot %d",m_iSlot);
