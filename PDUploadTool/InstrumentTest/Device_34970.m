...
- (int) setRoutSwitch:(BOOL) bOnOff ChannelLists:(NSString *) aChannels {
	int nRetCode = kSuccessCode;
...
- (int) getMeasureVoltage:(BOOL) bIsDC_VOLT ChannelLists:(NSString *) aChannels 
	...
	NSLog(@"%d = getMeasureVoltage(%d,%@,%@)",nRetCode,bIsDC_VOLT,aChannels,strResult);
	return nRetCode;
}


- (int) getCurrent:(BOOL) bIsDC_CURR Quick:(BOOL) bQuick Result:(NSMutableString *) strResult 
	  ChannelLists:(NSString *) aChannels Current:(NSMutableArray *) ArrayCurrent {
	int nRetCode = kSuccessCode;
	...
	if (bQuick)
		...
		strQuick = @"MIN";
	strCommand = [NSString stringWithFormat:@"MEAS:CURR:%@? MAX,%@, (@%@)",strDC_CURR,strQuick,aChannels];
	nRetCode = [self sendGPIB_Command:strCommand];
	if (nRetCode == kSuccessCode) {
		[NSThread sleepForTimeInterval:m_fDelayTime];
		nRetCode = [self getCommand_Result:strResult];
...
- (int) getMeasureCurrent:(BOOL) bIsDC_CURR ChannelLists:(NSString *) aChannels 
	...
	nRetCode = [self getCurrent:bIsDC_CURR Quick:NO Result:strResult ChannelLists:aChannels Current:ArrayCurrent];
	NSLog(@"%d = getMeasureCurrent(%d,%@,%@)",nRetCode,bIsDC_CURR,aChannels,strResult);
	return nRetCode;	
}
...
- (int) getQuickMeasureCurrent:(BOOL) bIsDC_CURR ChannelLists:(NSString *) aChannels 
	...
	nRetCode = [self getCurrent:bIsDC_CURR Quick:YES Result:strResult ChannelLists:aChannels Current:ArrayCurrent];
	NSLog(@"%d = getQuickMeasureCurrent(%d,%@,%@)",nRetCode,bIsDC_CURR,aChannels,strResult);
	return nRetCode;	
}
...
- (int) setMeasureCommand:(NSString *) aChannels IntervalTime:(float) fIntervalTime 
	...
	if (nRetCode == kSuccessCode) {
		...
		while (YES) {
			...
			if (nRetCode == kSuccessCode) {
				...
				break;
			if (abs([nowDT timeIntervalSinceNow]) > kFetchCountTimeOut) {
...
- (int) readMeasureFrequency:(NSString *) aChannels IntervalTime:(float) fIntervalTime 
	...
	nRetCode = [self sendGPIB_Command:strCommand];
	if (nRetCode == kSuccessCode) 
		nRetCode = [self setMeasureCommand:aChannels IntervalTime:fIntervalTime FetchCount:iFetchCount];
	if (nRetCode == kSuccessCode) {
		nRetCode = [self getCommand_Result:strResult];
		if (nRetCode == kSuccessCode) {
            if ([ArrayFrequency count]) 
	...
	NSLog(@"%d = readMeasureFrequency(%@,%f,%d,%@)",nRetCode,aChannels,fIntervalTime,iFetchCount,strResult);
	return nRetCode;
}
...
- (int) readMeasureResistance :(NSString *) aChannels Range:(int) nRange IntervalTime:(float) fIntervalTime 
	...
	NSLog(@"%d = readMeasureResistance(%@,%d,%f,%d,%@)",nRetCode,aChannels,nRange,fIntervalTime,iFetchCount,strResult);
	return nRetCode;	
}

- (int) getMeasureAvgCurrent:(BOOL) bIsDC_CURR Quick:(BOOL) bQuick ChannelLists:(NSString *) aChannels IntervalTime:(float) fIntervalTime 
	...
	NSLog(@"%d = getMeasureAvgCurrent(%@,%f,%d,%@)",nRetCode,aChannels,fIntervalTime,iFetchCount,strResult);
	return nRetCode;
}
