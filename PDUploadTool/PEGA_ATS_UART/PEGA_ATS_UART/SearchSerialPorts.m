...
{    
    ...
    //find the usb port
    kern_return_t  kr = IOServiceGetMatchingServices(kIOMasterPortDefault, myUSBMatchDictionary, &iterator);
	CFMutableDictionaryRef	dicProperty = nil;
	while((iterDeviceTemp = IOIteratorNext(iterator)))
	{//get the porperties of the device
		kr = IORegistryEntryCreateCFProperties(iterDeviceTemp, &dicProperty, kCFAllocatorDefault, kNilOptions);
		NSLog(@"%@",(NSDictionary*)dicProperty);
        if(kIOReturnSuccess == kr)
		{
            CFStringRef bsdPath = (CFStringRef)IORegistryEntrySearchCFProperty(iterDeviceTemp, kIOServicePlane,CFSTR(kIOCalloutDeviceKey),kCFAllocatorDefault, kIORegistryIterateRecursively);
            CFStringRef serialNumber = (CFStringRef)IORegistryEntrySearchCFProperty(iterDeviceTemp, kIOServicePlane,CFSTR(kUSBSerialNumberString),kCFAllocatorDefault, kIORegistryIterateRecursively);
    ...
    //CFRelease(myUSBMatchDictionary);
    return  iFound;
}
