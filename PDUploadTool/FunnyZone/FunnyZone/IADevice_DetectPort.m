    ...
    for (NSInteger iIndex=0; iIndex<iDeviceCount; iIndex++) 
    {
        ...
        if(iRet == kUart_SUCCESS)
        {
            ...
            if([kPD_DeviceSet_Type intValue] == kPD_DeviceSet_Type_String)
            {
                ...
                iRet = [uartObj Read_UartData:szReturn TerminateSymbol:kPD_DeviceSet_Expect MatchType:iMatchType IntervalTime:kUart_IntervalTime TimeOut:kUart_CommandTimeOut Ignore:nil];                
                [szReturn release]; 
                szReturn = nil;
            }
            else
            {
                ...
                iRet = [uartObj Read_UartData:dataReturn TerminateSymbol:kPD_DeviceSet_Expect/*[kPD_DeviceSet_Expect componentsSeparatedByString:@" "]*/ MatchType:iMatchType IntervalTime:0.01 TimeOut:5.1 Ignore:nil];
                [dataReturn release];
                dataReturn = nil;                
            }
            if (iRet == kUart_SUCCESS) 
...
// Return:
//      Actions result
-(BOOL)forNFMU:(NSMutableArray *)in_out_aryPorts ports:(NSMutableArray *)aryAllProts UIMode:(NSString *)szMode
{
    NSDictionary *dicAllUnitSet = [[[NSUserDefaults standardUserDefaults] objectForKey:@"ModeSetting"] objectForKey:szMode];
    NSString *szPortName = [dicAllUnitSet objectForKey:kPD_UserDefaults_PortName];
    
    [aryAllProts sortUsingComparator:^NSComparisonResult(id obj1, id obj2) {
    ...
    for(NSInteger iIndex=0; iIndex<iAllCount; iIndex++)
    {
        ...
        NSLog(@"Port Name : %@",szSerialPort);
        NSLog(@"Will find Port Name : %@",szPortName);
    ...
    if (!retFlag)
    {
        NSString *errMessage = [NSString stringWithFormat:@"请确认电脑是否有连接测试用线，且线名为%@. (The station need at least one device,but now you plug into 0 or the cable name %@ is not right.Please check it !)", szPortName, szPortName];
          NSRunAlertPanel(@"警告(Warning)", errMessage , @"确认(OK)", nil, nil, nil);
         return retFlag;
     }
    NSNumber *iEmptyCancelCount = [dicAllUnitSet objectForKey:kPD_UserDefaults_NoResponseCancelCount];
    if (iEmptyCancelCount != nil) {
        [m_dicNoResponse setObject:iEmptyCancelCount forKey:@"MOBILE"];
    }
    return YES;
}
...
// Return:
//      Actions result
-(BOOL)forSFSU:(NSMutableArray *)in_out_aryPorts ports:(NSMutableArray *)aryAllProts UIMode:(NSString *)szMode
{
    BOOL bRet = YES;
    
    NSDictionary *dicDevices;
    // auto test system (for QT0b multi test mode)
    NSDictionary *dicSocket = [NSDictionary dictionaryWithContentsOfFile: [NSString stringWithFormat:@"%@/Library/Preferences/ATS_Socket.plist", NSHomeDirectory()]];
    if (dicSocket != nil)
    {
        NSString *szAutoTestMode = [[dicSocket objectForKey:@"Socket"] objectForKey:@"AutoTestMode"];
        if (szAutoTestMode != nil)
        {
            dicDevices = [self getValueFromXML:kPD_UserDefaults mainKey:@"ModeSetting",@"SFSU",@"DeviceRequire",nil];
        }
        else if(![[kPD_ModeSet_UnitInfo objectForKey:kPD_UserDefaults_PutInUse] boolValue])
        {
            return YES;
        }
        else
        {
            dicDevices = [kPD_ModeSet_UnitInfo objectForKey:kPD_UserDefaults_DeviceRequire];
        }
    }
    else if (![[kPD_ModeSet_UnitInfo objectForKey:kPD_UserDefaults_PutInUse] boolValue])
    {
    ...
    else
    {
        ...
        */
        ...
        for(NSInteger iIndex=0; iIndex<iCount; iIndex++)
        {
            ...
            NSLog(@"main key : %@",szMainKey);
            
            // auto test system(NoNeed always NO)
            BOOL    bNoNeedThisDevice   = [[self getValueFromXML:dicDevices
                                                         mainKey:szMainKey,kPD_UserDefaults_NoNeed,nil]
                                           boolValue];
            if (gbIfNeedRemoteCtl) {
                bNoNeedThisDevice   = NO;
            }
            
            if (!bNoNeedThisDevice) {
                NSNumber    *iEmptyCancelCount = [self getValueFromXML:dicDevices mainKey:szMainKey,kPD_UserDefaults_NoResponseCancelCount,nil];
                if (iEmptyCancelCount != nil) {
                    [m_dicNoResponse setObject:iEmptyCancelCount forKey:szMainKey];
                }
        ...
        for(NSInteger jIndex=0; jIndex<iNeedDeviceNum; jIndex++)
        {
            ...
            bool bCheckPortName = [[self getValueFromXML:kPD_UserDefaults mainKey:kPD_UserDefaults_CheckPortNameForSFSU] boolValue];
            
            if (bCheckPortName) 
            {
                for (NSString *szNeedPort in aryNeedPorts) 
                {
                    NSString *szPortName = [self getValueFromXML:dicDevices mainKey:szNeedPort,kPD_UserDefaults_PortName,nil];
                    NSRange range   =   [szSerialPort rangeOfString:szPortName];
                    if(range.location !=NSNotFound && range.length>0 && (range.location +range.length) <=[szSerialPort length])
                    {
                        szPortType = [NSString stringWithString:szNeedPort];
                    }
                } 
            }
            else
            {
                szPortType = [self tryPort:dicStatus serialPath:szSerialPort deviceList:aryNeedPorts uart:uartObj];
            }
            //for Prox cal
            ...
            else
            {
                ...
                if ((szSerialPortName != nil) && [szSerialPortName isNotEqualTo: @""])
                {
...
// Return:
//      Actions result
-(BOOL)forMFMU:(NSMutableArray *)in_out_aryPorts ports:(NSMutableArray *)aryAllProts UIMode:(NSString *)szMode
{
    BOOL bRet = YES;
	//For Prox cal
    bool bIS_1_to_N =   [[self getValueFromXML:kPD_UserDefaults mainKey:kPD_UserDefaults_IS_1_to_N] boolValue];
    ...
    NSDictionary *dicAllUnitSet = [[[NSUserDefaults standardUserDefaults] objectForKey:@"ModeSetting"] objectForKey:szMode];
    NSArray *aryKeys = [dicAllUnitSet allKeys];
    ...
    for (NSInteger iIndex=0; iIndex<iSetListCount; iIndex++) 
    {
        ...
        NSDictionary *dicEachUnitSet = [dicAllUnitSet objectForKey:szCurrentKey];
        if ([dicEachUnitSet isKindOfClass:[NSDictionary class]]) 
        {
            if ([[dicEachUnitSet objectForKey:kPD_UserDefaults_PutInUse] boolValue]) 
            {
                ...
                {   
                    ...
                    // auto test system(NoNeed always NO)
                    BOOL    bNoNeedThisDevice   = [[self getValueFromXML:dicDevices
                                                                 mainKey:szPortType,kPD_UserDefaults_NoNeed,nil]
                                                   boolValue];
                    if (gbIfNeedRemoteCtl) {
                        bNoNeedThisDevice   = NO;
                    }
                    ...
                    if (!bNoNeedThisDevice)
                    {
                        NSNumber *iEmptyCancelCount = [self getValueFromXML:dicDevices mainKey:szPortType, kPD_UserDefaults_NoResponseCancelCount, nil];
                        if (iEmptyCancelCount != nil ) {
                            [m_dicNoResponse setObject:iEmptyCancelCount forKey:szPortType];
                        }
                        bComparedOneName = NO;
                        ...
                        for (; jIndex<iAllPortCount; jIndex++) 
                        {
							...
							else if ((szPortName != nil) && [szPortName isNotEqualTo: @""])
							{
								NSRange range = [szSerialPort rangeOfString:szPortName];
								if (range.location != NSNotFound 
									&& range.length >0 
...
// Return:
//      Actions result
-(BOOL)assignPorts:(NSMutableArray *)in_out_aryPorts UIMode:(NSString *)in_szMode
{
    BOOL bRet = YES;
    ...
    // Add for different test mode between normal line and auto line
    NSDictionary *dicSocket = [NSDictionary dictionaryWithContentsOfFile: [NSString stringWithFormat:@"%@/Library/Preferences/ATS_Socket.plist", NSHomeDirectory()]];
    if (dicSocket != nil)
    {
        NSString *szAutoTestMode = [[dicSocket objectForKey:@"Socket"] objectForKey:@"AutoTestMode"];
        if (szAutoTestMode != nil && isRaft)
        {
            if ([szAutoTestMode isEqualToString:kPD_Mode_SFSU])
            {
                //for single fixture single ui
                bRet = [self forSFSU:in_out_aryPorts ports:arrSerialPorts UIMode:in_szMode];
            }
            else if([szAutoTestMode isEqualToString:kPD_Mode_NFMU])
            {
                //for no fixture multi ui
                bRet = [self forNFMU:in_out_aryPorts ports:arrSerialPorts UIMode:in_szMode];
            }
            else if([szAutoTestMode isEqualToString:kPD_Mode_MFMU])
            {
                //for multi fixture multi ui
                bRet = [self forMFMU:in_out_aryPorts ports:arrSerialPorts UIMode:in_szMode];
            }
            else
            {
                //unnormal
...
/*
 * abstract : get unit sn by in_szUARTPath
*/
-(void)monitorSerialPortPlugInWithUartPath:(NSString *)in_szUARTPath withOutputSN:(NSMutableString *) out_szSN
{
    ...
    do {
        // send command "" at in_szUARTPath and make sure unit is in diag model
...
//            }
    ...
    NSLog(@"%@ SerialPortPlugIn end!",self);
    ...
    if(kUart_SUCCESS == iRet)//iRet== 0 --> succeed
    {
        if (0 == iRet)
        {
            for (int i = 0; i < 3; i++)
            {
                ...
                if (0 == iRet)
                {	
                    iRet = [uartObj Read_UartData:szReadString TerminateSymbol:[NSArray arrayWithObjects:@"*_*", nil] MatchType:1 IntervalTime:0.3 TimeOut:inCheckTime Ignore:nil];
...
//2012-4-16 add note by Sunny
//		Check unit out fixture.
// Parameter:
//      NSString        in_szFixruePath : serial port path
// Return:
//      the action result

- (void)monitorUnitOutByFixtureWithPath:(NSString *)in_szFixruePath
{
    
    PEGA_ATS_UART *uartObj = [[PEGA_ATS_UART alloc] init];
    int     iRet = -1;
    
    NSString *szPortType = kPD_Device_FIXTURE;
    
     NSArray *aryFixtureCommand = [self getValueFromXML:kPD_UserDefaults mainKey:kPD_UserDefaults_MonitorFixtureCommand];
    
    iRet = [uartObj openPort:in_szFixruePath baudeRate:[kPD_DeviceSet_BaudeRate intValue] dataBit:[kPD_DeviceSet_DataBit intValue] parity:kPD_DeviceSet_Parity stopBits:[kPD_DeviceSet_StopBit intValue] endSymbol:kPD_DeviceSet_EndFlag];
    
    NSDictionary * dicFailRecv = [self getValueFromXML:kPD_UserDefaults mainKey:@"ModeSetting",@"FailRecv",nil];
    ...
    do
    {
        ...
        if(m_bMonitorOut)
        {
            if(iRet == 0)
            {
                ...
                //monitor unit whether out unit
                for (NSString * szCommand in aryFixtureCommand)
                {
                    
                    NSString * szFailRecv = [dicFailRecv objectForKey:szCommand];
                    
                    if(szFailRecv == nil || [szFailRecv isEqualToString:@""])
                    {
                        szFailRecv = @"Fail";
                    }
                    [uartObj Clear_UartBuff:kUart_ClearInterval TimeOut:kUart_CommandTimeOut readOut:nil];
                    ...
                    int iRetOther = -1;
                    iRetOther = [uartObj Write_UartCommand:szCommand PackedNum:1 Interval:0 IsHex:NO];
                    
                    NSMutableString * szData = [[NSMutableString alloc] init];
                    [szData setString:@""];
                    
                    if(iRetOther == 0)
                    {
                        [uartObj Read_UartData:szData TerminateSymbol:[NSArray arrayWithObjects:@"*_*",nil] MatchType:1 IntervalTime:0.01 TimeOut:kUart_CommandTimeOut Ignore:nil];
                        
                        if([szData ContainString:szFailRecv])
                        {
                            obel = YES;
                        }
                        else
                ...
                if(obel)
                {
                    m_iFlagOut = 1;     //normal end
                    break;
                }
            }
            else
            {
                //if open port fail, open again
                iRet = [uartObj openPort:in_szFixruePath baudeRate:[kPD_DeviceSet_BaudeRate intValue] dataBit:[kPD_DeviceSet_DataBit intValue] parity:kPD_DeviceSet_Parity stopBits:[kPD_DeviceSet_StopBit intValue] endSymbol:kPD_DeviceSet_EndFlag];
            }
...
- (void)monitorSerialPortPlugOutWithUartPath:(NSString *)in_szUARTPath
{
    ...
    m_iFlagIn = 0;
    do {
        // send command "" for loop , if send success wait 0.1s and go on, if send fail deem port is pluged out 
        if (![self checkUnitConnectWellWithSerialPort:in_szUARTPath UARTCommand:@"" UartResponse:szReadString CheckTime:1.0])
        {
            NSLog(@"%@ SerialPortPlugOut!",self);
            break;
        }
        else
        {
...
//2014-11-06 add note by sam
//		Check unit in fixture.
// Parameter:
//      NSString        in_szUARTPath : fixture port path
// Return:
//      the action result

-(void)monitorUnitInByFixtureWithPath:(NSString *)in_szFixruePath
{
    PEGA_ATS_UART *uartObj = [[PEGA_ATS_UART alloc] init];
    int     iRet = -1;
    ...
    NSDictionary * dicPassRecv = [self getValueFromXML:kPD_UserDefaults mainKey:@"ModeSetting",@"PassRecv",nil];
        ...
        if(m_bMonitorIn)
        {
            ...
            if(iRet == 0)
            {
                ...
                for (NSString * szCommand in aryFixtureCommand)
                {
                    ...
                    NSString * szPassRecv = [dicPassRecv objectForKey:szCommand];
                    
                    if((szPassRecv == nil) || [szPassRecv isEqualToString:@""])
                    {
                        szPassRecv = @"Pass";
                    }
                    [uartObj Clear_UartBuff:kUart_ClearInterval TimeOut:kUart_CommandTimeOut readOut:nil];
                    ...
                    if(iRetOther == 0)
                    {
                        ...
                        if([szData ContainString:szPassRecv])
                        {
                            obel = YES;
                        }
                        else
                ...
                if(obel)
                {
                    ...
                    //after 3s delay, make sure unit in fixture again
                    for (NSString * szCommand in aryFixtureCommand)
                    {
                        NSString * szPassRecv = [dicPassRecv objectForKey:szCommand];
                        if((szPassRecv == nil) || [szPassRecv isEqualToString:@""])
                ...
                if(obel)
                {
                    m_iFlagIn = 1;//normal end
                    break;
                }
            }
            else
            {
                //if port open fail, open again
                iRet = [uartObj openPort:in_szFixruePath baudeRate:[kPD_DeviceSet_BaudeRate intValue] dataBit:[kPD_DeviceSet_DataBit intValue] parity:kPD_DeviceSet_Parity stopBits:[kPD_DeviceSet_StopBit intValue] endSymbol:kPD_DeviceSet_EndFlag];
            }
...
- (BOOL)checkUnitConnectWellWithSerialPort:(NSString *)in_szUARTPath UARTCommand:(NSString *)in_szCommand UartResponse:(NSMutableString *)in_out_szResponse CheckTime:(NSTimeInterval)inCheckTime
{
    ...
    if (0 == iRet)
    {
        for (int i = 0; i < 3; i++)
        {
            //Mark for Gatekeeper comport dead 040111
            //iRet = [uartObj Clear_UartBuff:kUart_ClearInterval TimeOut:kUart_CommandTimeOut readOut:nil];
            if (0 == iRet)
            {
                iRet = [uartObj Write_UartCommand:in_szCommand PackedNum:1 Interval:0 IsHex:NO];
                NSLog(@"%@ checkUnitConnectWellWithSerialPort Write command : %@",self,in_szCommand);
            }
            if (0 == iRet)
            {
                //For prox cal
				if (bIS_1_to_N)
				{
                    ...
                    {
                        //040111 modify interval from 0.3 to 0.01 for gatekeeper comport dead
                        iRet = [uartObj Read_UartData:szReadString TerminateSymbol:[NSArray arrayWithObjects:@":-)",@"]", nil] MatchType:1 IntervalTime:0.01 TimeOut:inCheckTime Ignore:nil];
                    }
                    NSString *strMemoryECID = [m_dicMemoryValues objectForKey:@"LORKY_DEVICE_ECID"];
                    if (strMemoryECID) 
                    {
                         //040111 modify interval from 0.3 to 0.01 for gatekeeper comport dead
                        iRet = [uartObj Read_UartData:szReadString TerminateSymbol:[NSArray arrayWithObjects:kFZ_EndFlagFormat, nil] MatchType:1 IntervalTime:0.01 TimeOut:inCheckTime Ignore:nil];
                    }
                ...
                {
                     //040111 modify interval from 0.3 to 0.01 for gatekeeper comport dead
                    iRet = [uartObj Read_UartData:szReadString TerminateSymbol:[NSArray arrayWithObjects:@":-)",@"]", @"login: ", @"Password:", @"root# ", nil] MatchType:1 IntervalTime:0.01 TimeOut:inCheckTime Ignore:nil];
                    NSLog(@"%@ checkUnitConnectWellWithSerialPort Read command : %@",self,szReadString);
                }
                [in_out_szResponse setString:[NSString stringWithString:szReadString]];
            }
            if (0 == iRet)
    ...
    [uartObj Close_Uart];
    NSLog(@"%@ checkUnitConnectWellWithSerialPort Close Uart!",self);
    [uartObj release];
    uartObj = nil;
    [szReadString release];
    if (0 == iRet) 
