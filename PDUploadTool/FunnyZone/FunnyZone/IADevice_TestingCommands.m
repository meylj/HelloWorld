...
-(NSNumber*)CLEAR_TARGET:(NSDictionary*)dicClearSettings
{
    ...
    NSString        *szTimeOut = [dicClearSettings objectForKey:@"TimeOut"];
    double  dTimeOut = [szTimeOut isEqualToString:@""]? kUart_CommandTimeOut:[szTimeOut doubleValue];
	PEGA_ATS_UART   *uartObj    = [[m_dicPorts objectForKey:szTarget] objectAtIndex:kFZ_SerialInfo_UartObj];
	// Close
	[uartObj Clear_UartBuff:kUart_ClearInterval TimeOut:dTimeOut readOut:nil];
	return [NSNumber numberWithBool:YES];
}
...
//          NOUARTLOG       -> Boolean      : the key decide whether to write uart log     
// Return:
//		Actions result
...
//          END_SYMBOL      -> NSArray      : End symbol of the read data
//          MATCHTYPE       -> NSNumber*    : For END_SYMBOL, 0: Match all item, 1: Match any one
//          RETURN_DATA     -> Boolean      : For CB read
//          NUART           -> Boolean      : this key decide whether to save uart log
//          IGNORE          -> NSString*    : ex.Press 'q' to quit any other key to continue...
//          NOUARTLOG       -> Boolean      : the key decide whether to write uart log 
//          INTERVAL        -> NSString     : define the interval time for reading uart data
//          IsNumber        -> Boolean      : Decide the value that we want to catch is Number or not.
//          RIGHTSTRING     -> Boolean      : When catching value ,if begin is equal to end and U want to get value, Please set YES
			...
			RETURN_VALUE:(NSMutableString*)szReturnValue
{
    ...
    NSString *szIgnore = [dicReadSettings objectForKey:@"IGNORE"];
	...
	{
		NSString* objInAry = [aryEndSymbols objectAtIndex:i];
		if ([objInAry isKindOfClass:[NSString class]] && [objInAry isEqualToString:@":-)"])
		{
        ...
        else if ([objInAry isKindOfClass:[NSString class]] && [objInAry isEqualToString:@":-):-)"])
        {
            if ([m_dicMemoryValues objectForKey:@"LORKY_DEVICE_ECID"])
			{
				[aryEndSymbols replaceObjectAtIndex:i withObject:[NSString stringWithFormat:@"%@ :-) \n\r%@ :-)", [m_dicMemoryValues objectForKey:@"LORKY_DEVICE_ECID"], [m_dicMemoryValues objectForKey:@"LORKY_DEVICE_ECID"]]];
			}
        }
	}
    ...
    if([[dicReadSettings objectForKey:kFZ_Script_ReceiveReturnData] boolValue])
    {
        ...
        if (NULL == uartObj) {
            ...
            iRet = [uartObj Read_UartData:data TerminateSymbol:aryEndSymbols MatchType:iMatchType IntervalTime:dReadInterval TimeOut:dReadTimeOut Ignore:nil];
        ...
        {
            iRet = kUart_ERROR;
            ATSDebug(@"Read from <--- %@ : %@, Data length is Zero",szTarget,data); 
        }
    ...
    else
    {
        ...
        for (NSInteger iIndex=0; iIndex<uiRepeat; iIndex++) 
        {
			...
			{
                iRet = [uartObj Read_UartData:strRead TerminateSymbol:aryEndSymbols MatchType:iMatchType IntervalTime:dReadInterval TimeOut:dReadTimeOut Ignore:szIgnore];
			}
        ...
        // if there is no response from devices continuously,cancel to end
        if (0 == [szReturnValue length])
        {
            int iTargetEmptyCount = [[m_dicEmptyResponse objectForKey:szTarget] intValue];
            iTargetEmptyCount++;
            [m_dicEmptyResponse setObject:[NSNumber numberWithInt:iTargetEmptyCount] forKey:szTarget];
            if (iTargetEmptyCount == [[m_dicNoResponse objectForKey:szTarget]intValue])
            {
                m_bCancelNoMatterWhat = YES;
            }
        }
        else
        {
            [m_dicEmptyResponse setObject:[NSNumber numberWithInt:0] forKey:szTarget];
        }
    ...
    //write uart log leehua 11.09.27
    //Get if need to save binary when writting Uart log
    BOOL bBinarySave = [[self getValueFromXML:kPD_UserDefaults mainKey:kPD_UserDefaults_SaveBinary,nil] boolValue];
    ...
    if(kUart_SUCCESS == iRet)//iRet== 0 --> succeed
    {
        ...
        //m_newItemFlag : if run new item , set YES
        //m_itemHasDUTRX : if new item has DUT command and RX normal,then set YES
        ...
        //if response doesn't contain all strings which in pass recieve array , return fail
        ...
        if(nil != aryPassReceive && [aryPassReceive isKindOfClass:[NSArray class]])
        {
            ...
            for(int i=0; i<iCount; i++)
            {
                ...
                {
                    ATSDebug(@"READ_COMMAND => Can't Catch Pass Key:%@!",[aryPassReceive objectAtIndex:i]);
        ...
        //According to the request to catch value from the ReturnValue.
    ...
    else
    {
        ...
        if ([szReturnValue ContainString:@":-)"]||[szReturnValue ContainString:@"*_*"]|| [szReturnValue ContainString:@":-D"] ||[szReturnValue ContainString:@"]"])
        {
            ...
            ATSDebug(@"READ_COMMAND => RX [%@] Error response",szCommand);
        ...
        else
        {
            if (0 == [szReturnValue length])
            {
                ATSDebug(@"READ_COMMAND => RX [%@] Empty response",szCommand); 
                [szReturnValue setString:[NSString stringWithFormat:@"RX [%@] Empty response",szCommand]];
            }
            else
            {
                ATSDebug(@"READ_COMMAND => RX [%@] Incomplete response",szCommand);
...
//          Key             -> NSString     : The key which memory the cut string 
//          IsNumber        -> Boolean      : Decide the Catched value is Number or not
//          RIGHTSTRING     -> Boolean      : If begin is equal to end , and want to get catched value, set YES
...
-(BOOL)JudgeSpec:(NSString *)szSpec return_value:(NSString *)szCatchedValue caseInsensitive:(BOOL)bCaseMode
{
	...
	if(![self JudgeValue:szCatchedValue 
              WithLimits:arraySpec 
                    Mode:iJudgeMode])
	{
        ATSDebug(@"JUDGE_SPEC => Judge Fail");
		return NO;
	}
    ATSDebug(@"JUDGE_SPEC => Judge Pass");
	return YES;
}
...
//			COMMON_SPEC     -> NSDictionary : Common spec key
//              P_LimitBlack  -> NSString   : SPEC for Black Unit. (?,?), [?,?], {?,?,?}
//              P_LimitWhite  -> NSString   : SPEC for White Unit. (?,?), [?,?], {?,?,?}
//          NoCaseInsensitive -> Boolean    : Judge if need uppercase 
//          NoChangeResult    -> Boolean    : Judge if need to change the action result
//          ISNOSpec          -> Boolean    : Judge if need to set and save spec
//          COCO_SPEC       -> NSDictionary : COCO Spec key
//          SHOWUI            -> Boolean    : Judge if need to show on UI 
//          EXCEPTION_SPEC  -> NSDictionary : Exception Spec Key 
//          MaxAppear         -> NSNumber   : Max appear count when judge exception
...
-(NSNumber*)JUDGE_SPEC:(NSDictionary*)dicSpecSettings RETURN_VALUE:(NSMutableString*)szReturnValue
{
    ...
    id dicForCof = [dicSpecSettings objectForKey:kFZ_Script_JudgeCofSpec]; //Get COCO Spec
    if (dicForCof != nil) 
    {
    ...
    id dicForCommon = [dicSpecSettings objectForKey:kFZ_Script_JudgeCommonSpec];   //Get Common Spec
    if (dicForCommon != nil)
    {          
        ...
        if (!bUnShowFlag) //If Yes,Set Spec Or not set
        {
        ...
        if (!bChangeResult) //If Yes, Set Result YES. 
        {
    ...
    if (!bUnShowFlag) 
    {
        if (!bMPRet && dicForCof != nil) //If have coco spec ,and no common spec or common spec judge fail.Set coco spec as TestLimit
        {
            szSpec		= [dicForCof objectForKey:m_strSpecName];
        }
        [m_dicMemoryValues setObject:szSpec forKey:kFZ_TestLimit];
...
-(NSNumber*)COMPARE:(NSDictionary*)dicCompareItems RETURN_VALUE:(NSMutableString*)szMatch
{
    // Compare
    BOOL    bResult = YES;
    BOOL    bNotEqual  = [[dicCompareItems objectForKey:@"NotEqual"] boolValue];
    BOOL    bRegex     = [[dicCompareItems objectForKey:@"Regex"] boolValue];
    ...
    {
        [szMatch setString:@""];
        NSDictionary * dicRexValue = [dicCompareItems objectForKey:@"RegexComparison"];
        
        if(!dicRexValue)
        {
            [szMatch setString:@"value is Nil"];
            return [NSNumber numberWithBool:NO];
        }
        NSString     * szRexValue = [dicRexValue objectForKey:@"RegexValue"];
        NSString     * szValue = [dicRexValue objectForKey:@"Value"];
        
        [self TransformKeyToValue:szRexValue returnValue:&szRexValue];
        [self TransformKeyToValue:szValue returnValue:&szValue];
        
        if([szValue matches:szRexValue])
        {
            [szMatch setString:szValue];
            return [NSNumber numberWithBool:YES];
        }
        
        [szMatch setString:[NSString stringWithFormat:@"%@ not match %@",szRexValue,szValue]];
        return [NSNumber numberWithBool:NO];
    ...
    {
        if([szComparison isEqualToString:@"NotEqual"])
        {
            continue;
        }
        id idTemp = [dicCompareItems objectForKey:szComparison];
        if(![idTemp isKindOfClass:[NSDictionary class]])
        {
            continue;
        }
        NSMutableDictionary *dicComparison = [[NSMutableDictionary alloc] initWithDictionary:idTemp];
        
        if (nil == dicComparison || (0 == [dicComparison count]))
        {
            continue;
        }
        //save key and value of compared
        NSString *szComparedValue1 = @"";
        NSString *szComparedKey1 = @"";
        NSString *szComparedValue2 = @"";
        NSString *szComparedKey2 = @"";
        
        NSArray *arrTempKeys = [[dicComparison allKeys] sortedArrayUsingSelector:@selector(compare:)];
        NSMutableArray *arrComparedKeys = [[NSMutableArray alloc] initWithArray:arrTempKeys];
        
        //get the contain case dictionary and remove it from normal comparison
        NSArray *arrContainCase = nil;
        BOOL bContainCase = NO;
        if ([arrComparedKeys containsObject:@"ContainCase"])
        {
            bContainCase = YES;
            arrContainCase = [dicComparison objectForKey:@"ContainCase"];
            [dicComparison removeObjectForKey:@"ContainCase"];
            [arrComparedKeys removeObject:@"ContainCase"];
        }
        //record key and value base on different thing
        ...
        {
            if ([[dicCompareItems objectForKey:@"ToMajuscule"] boolValue]) {
                szComparedValue1 = [szComparedValue1 uppercaseString];
                szComparedValue2 = [szComparedValue2 uppercaseString];
            }
            // contain case begin
            if (bContainCase)
            {
                BOOL bTempResult = YES;
                int iCount = [arrContainCase count];
                // Use for loop to deal with the multiple contation condition
                for (int i = 0; i < iCount; i++)
                {
                    NSDictionary *dicContainCase = [arrContainCase objectAtIndex:i];
                    NSArray *arrContainValue1 = [[dicContainCase objectForKey:szComparedKey1] componentsSeparatedByString:@","];
                    NSArray *arrContainValue2 = [[dicContainCase objectForKey:szComparedKey2] componentsSeparatedByString:@","];
                    if ([arrContainValue1 containsObject:szComparedValue1] && [arrContainValue2 containsObject:szComparedValue2])
                    {
                        
                        [szMatchInfo appendFormat:@"[%@<%@> match %@<%@>]",szComparedValue1, szComparedKey1, szComparedValue2, szComparedKey2];
                        bTempResult = YES;
                        break;
                    ...
                    {
                        ATSDebug(@"Contain Case fail: \n arrContainValue1 = %@\n szComparedValue1 = %@ \n arrContainValue2 = %@\n szComparedValue2 = %@",arrContainValue1, szComparedValue1, arrContainValue2, szComparedValue2);
                        bTempResult = NO;
                    }
                }
                ...
                if (!bTempResult)
                {
                    ...
                    {
                        [szMatchInfo appendFormat:@"[%@<%@> unmatch %@<%@>]",szComparedValue1, szComparedKey1, szComparedValue2, szComparedKey2];
                        bTempResult = NO;
                    }
        ...
        [dicComparison release];
        [arrComparedKeys release];
    }
    if(bResult)
    {
...
-(NSNumber *)UPLOAD_SLOT_TO_PDCA:(NSDictionary *)dicInsertItems RETURN_VALUE:(NSMutableString*)szReturn
{
    ...
    NSInteger nPosition = [self GetUSB_PortNum];
    if (nPosition > 6) // If nPosition is 12
    {
        nPosition = nPosition%6;
    }
    if (nPosition <= 0) // nPosition will be 1
    {
...
-(NSNumber *)SHOW_RESULT_ON_SCREEN:(NSDictionary *)dicInsertItems RETURN_VALUE:(NSMutableString *)szReturnValue
{
    ...
    NSString *passCommand = showSecPassLogo ? @"pattern 1":@"pattern e";//torres add for show second pass logo in screen.
...
-(NSNumber *)AMIOK_CHECK:(NSDictionary *)dicSubSetting RETURN_VALUE:(NSMutableString *)szReturnValue
{
    ...
    else
    {
        ...
        NSPanel *Panel = NSGetAlertPanel(@"Am I OK Error", strErrorMsg, @"OK", nil, nil, nil);
...
{
    [m_dicMemoryValues setObject:[[NSDate date] descriptionWithCalendarFormat:@"%Y%m%d" timeZone:nil locale:nil]  forKey:kFZ_NowDate];
    return [NSNumber numberWithBool:YES];
}
...
-(NSNumber*)CHECK_DUT_ALIVE:(NSDictionary *)dicDUTContents RETURN_VALUE:(NSMutableString*)strReturnValue;
{
    ...
    NSMutableString    *strRead=[[NSMutableString alloc] initWithString:@""];
    UInt16 iRet = [uartObj Read_UartData:strRead TerminateSymbol:[NSArray arrayWithObjects:kPD_DeviceSet_Expect, nil] MatchType:kUart_MatchAllItems IntervalTime:kUart_IntervalTime TimeOut:dReadTimeOut Ignore:nil];
...
/*
 ...
 BOOL bRetFlag    : get value from dicpara : When we judge and cancel some items and need to return NO, Need to set YES
 @description:
 ...
 Description:    This function can make u easy to catch value by a certain row and column from a matrix.
          eg:       row:    3
                 ...
                 matrix:    xxxxxxx
                            ...
                            xxxxxxx
 Para:           
            NSDictionary	*dicpara           : Save read settings
            CatchArray     --> NSDictionary    : Get an arrsy by row or column
            character      --> NSString        : Separate character
            KEY            --> NSString        : The key is which memory the cut string
            ROW            --> NSNumber        : Catch value from the matrix with the row
            COLUMN         --> NSNumber        : Catch value form the matrix with the column 
            (NSMutableString *)szReturnValue   : The value which would write in csv and show on UI.
 Return:        
            If exist the certain value in the matrix and can catch return YES, else return NO.
 */
-(NSNumber *)CatchValuebyRowAndColumn:(NSDictionary*)dicpara RETURN_VALUE:(NSMutableString *)szReturnValue
{
    ...
    //Judge if has catchedValue
    if(nil == szCatchedValue)
    {
        szCatchedValue = @"Value not found";
        szReturn    =   @"";
    }//When read data fail,the catchedValue contain @"RX [" ,set return @""
    ...
    bool bData = NO;  //judge if the catchedValue is number
    ...
    if(nil == dicArray)            //return string value by Row and Column
    {
        if([dicpara valueForKey:@"ROW"] && [dicpara valueForKey:@"COLUMN"])
        {
            ...
            ATSDebug(@"CatchValuebyRowAndColumn => row is %d; column is %d",iRow,iColumn);
            
            if (abs(iRow) < [arrRow count])
            {
                int iRowTemp = (iRow < 0)?([arrRow count] + iRow):iRow;             //judge whether to catch forward or backward.
                ...
                if (iColumn < [arrColumn count])
                {
                    ...
                    ATSDebug(@"CatchValuebyRowAndColumn => catch ok : return value:%@",szReturnValue);
                    return [NSNumber numberWithBool:YES];
                }
                else
                {
                    ATSDebug(@"CatchValuebyRowAndColumn => catch fail : the iColumn %i > the column count %i",iColumn,[arrColumn count]);
                }
            }
            else
            {
                ATSDebug(@"CatchValuebyRowAndColumn => catch fail : the iRow %i > the Row Count %i",abs(iRow),[arrRow count]);
            }

        }
        else
        {
            ATSDebug(@"CatchValuebyRowAndColumn => catch fail : the row or column is nil");
        }
        if (bData)
        {
            [szReturnValue setString:kFZ_99999_Value_Issue];
        ...
        else
        {
            ...
            ATSDebug(@"CatchValuebyRowAndColumn => catch fail : return value:%@",szReturnValue);
            return [NSNumber numberWithBool:NO];
        }
    ...
    else                  // return an array
    {
        ...
        if([szReturn isEqualToString:@"Row"])  // return an array by row  eg: xxxxxxxx   Index : 1
        {                                                              //     xxooooxx   Start : 3
            if(nil == szIndex)                                         //     xxxxxxxx   End   : 6
            {                                                          //     xxxxxxxx   ReturnValue: oooo
                ATSDebug(@"CatchValuebyRowAndColumn => catch fail : Index does not have value");
                [mutableArr release];
                return [NSNumber numberWithBool:NO];
            }
            ...
            {
                ATSDebug(@"CatchValuebyRowAndColumn => catch fail : Index value is too large");
                [mutableArr release];
                return [NSNumber numberWithBool:NO];
            }
            ...
            ATSDebug(@"CatchValuebyRowAndColumn => index is :%d ; start: %d ; end: %d ",iIndex,rectselect.startRow,rectselect.endRow);
            if(rectselect.startRow > rectselect.endRow || rectselect.endRow > [arrTemp count]|| rectselect.endRow<=0)
            {
                ATSDebug(@"CatchValuebyRowAndColumn => catch fail : EndRow value is too large or End Row can not be 0");
                [mutableArr release];
                return  [NSNumber numberWithBool:NO];
            }
        ...
        if([szReturn isEqualToString:@"Column"])  // return an array by column eg:  xxxxxx   Index : 3
        {                                                                       //  xxoxxx   Start : 1
            if(nil  ==  szIndex )                                               //  xxoxxx   End   : 3
            {                                                                   //  xxoxxx   ReturnValue : ooo
                [mutableArr release];
                ATSDebug(@"CatchValuebyRowAndColumn => catch fail : Index does not have value");
                return [NSNumber numberWithBool:NO];
            }
            ...
            ATSDebug(@"CatchValuebyRowAndColumn => index is :%d ; start: %d ; end: %d ",iIndex,rectselect.startColumn,rectselect.endColumn);
            if(rectselect.startColumn > rectselect.endColumn || rectselect.endColumn >=[arrRow count])
            {
                ATSDebug(@"CatchValuebyRowAndColumn => catch fail : EndRow value is too large");
                [mutableArr release];
                return [NSNumber numberWithBool:NO];
            }
            for(;rectselect.startColumn <= rectselect.endColumn;rectselect.startColumn++)  // add the value from StartColumn to EndColumn into MutableArrary
            {
                ...
                {
                    ATSDebug(@"CatchValuebyRowAndColumn => catch fail : Index value is %i larger than Column Count %i",iIndex,[arrTemp count]);
                    [mutableArr release];
                    return [NSNumber numberWithBool:NO];
                }
        ...
        [mutableArr release];
        ATSDebug(@"CatchValuebyRowAndColumn => catch fail : The Return is %@ not equal to Row or Column",szReturn);
        return [NSNumber numberWithBool:NO];
    }
}
...
- (NSNumber *)NumberSystemConvertion:(NSDictionary*)dictSettings RETURN_VALUE:(NSMutableString *)szReturnValue
{
    ...
    //refer to length of sourceSystem
    ...
    ATSDebug(@"ulBegin = %ld,ulLength = %ld",ulBegin,ulLength);
	...
	// Convert to target system(evaluation residue ,then overturn)
	NSString	*strResult	        = @"";
...
{   
    ...
    // auto test system (add for QT0b, if test fail, cancel unit shutdown)
    if ([[dicSetting objectForKey:@"AccordToFinalResult"] boolValue])
    {
        if (m_bFinalResult)
        {
            bFlag = YES;
    ...
    {
        bFlag        = [[self JUDGE_SPEC:dicSetting RETURN_VALUE:[NSMutableString stringWithString:szReturnValue]] boolValue];
    }
 ...
 *TITLE      => Panel title
 *MESSAGE    => Message showed on Panel or UI
 *ABUTTONS   => Buttons showed on Panel or UI
 *OpenPanel  => Judge if need to open a panel or add to UI directly
 *SESSION    => Judge if the panel is a separate session
...
{
    m_bOpenPanel = NO; //judge if need open an panel 
    m_StopFlag = NO;   //When having buttons, we need to sleep until we press button to start test again
    ...
    //m_szPortIndex like "Unit1","Unit2","Unit3"..... ; 48 represents '0' 49 represents '1'
    ...
    if (m_bOpenPanel) 
    {
        ...
        sdPanel = [[SDPanel alloc] initWithParentWindow:nil panelContents:dicIn]; //init an panel
        enum SD_RESULT enmRet = sdPanel.m_enmRet; //get the sdPanel result(eg:SD_PASS,SD_FAIL,SD_NA)
        ATSDebug(@"MESSAGE_PANEL => The Panel result is : %i",enmRet);
        if (!sdPanel.m_bNoButton && !sdPanel.m_bIsSession)  //If have button and not a session, after user operation ,need to release the panel 
        {
            [sdPanel release];
            sdPanel = nil;
        }
        if(enmRet == SD_FAIL)
        {
            ATSDebug(@"MESSAGE_PANEL => Panel Return Fail");
            return [NSNumber numberWithBool:NO];
        }
        else if((enmRet == SD_PASS) || (enmRet == SD_NA))
        {
            ATSDebug(@"MESSAGE_PANEL => Panel Return Pass");
            return [NSNumber numberWithBool:YES];
        }
    }
    else
    {
        //not open a panel , need to add to our UI. So need to post an Notification to Muifa
        NSNotificationCenter    *nc     =   [NSNotificationCenter defaultCenter];
        [nc postNotificationName:ShowMessage object:self userInfo:dicIn];
        if (szButtons && ![szButtons isEqualToString:@""]) // If have buttons, need to wait user operation. Else return YES.
        {
            do {
                usleep(2000);
            } while (!m_StopFlag);
        ...
        NSString *szDadugInfo = (m_ReturnValue == YES)?[NSString stringWithFormat:@"MESSAGE_PANEL => Panel Return Pass"]:[NSString stringWithFormat:@"MESSAGE_PANEL => Panel Return Fail"];
        ATSDebug(@"%@",szDadugInfo);
    }
...
{
    //If open a panel,need to release the panel
    if (m_bOpenPanel) 
    {
        BOOL bRet = YES;
    ...
    else //Post a notification to Muifa to closeMessage
    {
        NSNotificationCenter    *nc     =   [NSNotificationCenter defaultCenter];
        [nc postNotificationName:CloseMessage object:self userInfo:nil];
        ATSDebug(@"CLOSE_PANEL => Close Panel Pass");
        return [NSNumber numberWithBool:YES];
    }
}
...
{
    NSArray *aryItems = [NSArray arrayWithObjects:@"AUO",@"LG", nil];
    if ([aryItems containsObject:szReturnValue]) 
    {
        NSString *szCurrentItemName = [NSString stringWithFormat:@"%@ %@",szReturnValue,[m_dicMemoryValues valueForKey:kFunnyZoneCurrentItemName]];
        [m_dicMemoryValues setObject:szCurrentItemName forKey:kFZ_UI_SHOWNNAME];
        [m_strSpecName setString:[NSString stringWithFormat:@"P_Limit%@",szReturnValue]];
    }
    else
    {
        NSString *szDefaultSpecName = [dicContents objectForKey:@"Default_SPEC"];
        [m_strSpecName setString:szDefaultSpecName];
    }
    return [NSNumber numberWithBool:YES];
}
@end
