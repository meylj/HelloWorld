...
- (void)showMoal : (NSString *)  aTitle ReturnValue : (NSMutableString *) aValue{
    if (mPanel == nil)
...
- (void)beginSheetWithWindow:(NSWindow *)window panelContents:(NSDictionary *)dicPanelContents
{
    ...
    if (szPoint!=nil) 
    {
        ...
        {
            NSRunAlertPanel(@"警告（Warning）", @"请使用正确的测试点format: Point1.x,Point1.y,Point2.x,Point2.y,Radius。(You didn't follow the Point Format\nFormat:Point1.x,Point1.y,Point2.x,Point2.y,Radius)", @"确认(OK)", nil, nil);
        }
    ...
    NSString *szImgPath = [NSString stringWithFormat:@"%@/Contents/Frameworks/FunnyZone.framework/Versions/A/Resources/",[[NSBundle mainBundle] bundlePath]];            
	if (mPanel == nil)
        [NSBundle loadNibNamed:@"panel" owner:self];
    ...
    [mPanel setFrame:NSMakeRect(212, 184, 600, 400) display:YES];
    ...
    //add Buttons
    if (1 == [arrButtons count] && [[arrButtons objectAtIndex:0] isEqualToString:@" "]) 
    {
...
- (IBAction) btnPassClick:(id)sender
{
	...
	{
		NSRunAlertPanel(@"警告(warning)", @"请输入电流。(Please input the current!)", @"确认(OK)", nil, nil);
		return;
	}
	bPassFlag=YES;
