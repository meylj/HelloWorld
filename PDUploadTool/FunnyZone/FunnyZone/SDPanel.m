...
- (id)initWithParentWindow:(NSWindow *)window panelContents:(NSDictionary *)dicPanelContents
{
    self = [super init];
    if (self) {
        m_enmRet = SD_NA;
        ...
        {
            NSRunAlertPanel(@"警告(Warning)", @"请为窗口设定正确的初始化位置。(Please set correct window location!)", @"确定(OK)", nil, nil);
        ...
        //Create window
        NSRect rc = NSMakeRect(fOriginX, fOriginY, SDPanelWidth, SDPanelHeight); //win width:350 ; win height:150     
        ...
        //add image view
        NSString *szImgPath = [NSString stringWithFormat:@"%@/Contents/Resources/Muifa.icns",[[NSBundle mainBundle] bundlePath]];
        ...
        if (!szMsg) 
        {
            szTitle = @"You didn't set MESSAGE in script file";
        }
        //140=20(leftmargin)+120(imageWidth)+10(middleMargin)
        //50=15(belowMargin)+20(buttonHeight)+15(middleMargin)
        //190=350(windWidth)-140(leftMargin)-20(rightMargin)
        //85=150(windowHeight)-15(topMargin)-50(belowMargin)
        NSTextField  *txtMessage = [[NSTextField alloc] initWithFrame:NSMakeRect(150, 50, SDPanelWidth-140-20, SDPanelHeight-15-50)];
        ...
        /*if (m_iBtnCount == 1 && [[arrButtons objectAtIndex:0] isEqualToString:@"FAIL"])
        {
            m_bFail = YES;
        }*/
        ...
        {
            //170=150+20(marginMore)
            //70(button Width)
            //20(distance between buttons)
            //15(belowMagrin)
            ...
            if ([szBtnTitle isEqualToString:@""] || [szBtnTitle isEqualToString:@" "]) 
            {
                ...
                {
                    NSRunAlertPanel(@"警告(Warning)", @"请在剧本档中设置正确的按键名称。(Please set correct button title in script file!)", @"确定(OK)", nil, nil);
                    return nil;
                }
            }
