...
{   
    ...
    for(int i=0;i<[arrBsdPath count];i++)
    {
        ...
        {
            NSRunAlertPanel(@"警告(Warning)",
							@"请确认/usr/local/bin目录下有名为diag_status的文件。(Please make sure /usr/local/bin/diag_status is exist.)",
							@"确认(OK)", nil, nil);
            bSetPass	= NO;
            break;
        }
    ...
    if(bSetPass)
        NSRunAlertPanel(@"警告(Warning)",
						@"所有的Kong Cable已设置成使机台自动进diag模式，请重插拔所有的Kong Cable并重开程式。(Already set Kong Cable be auto enter diag mode. Please replug all the Kong cables,and restart the program)",
						@"确认(OK)", nil, nil);
    return NO;
}
//2012-4-16 add note by Sunny
