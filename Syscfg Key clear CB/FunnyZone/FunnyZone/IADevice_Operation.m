			  ...
			  RETURN_VALUE:(NSMutableString*)strReturnValue
{
	...
	else 
    {
        ...
        else if ([@" " isEqualToString:szButtons]) 
        {
            ...
            {
                m_panel = NSGetAlertPanel(@"Attention!!!", szMSG, nil, @"", @"",nil);
...
-(void)ShowAlertPanel:(id)dicContect {
    ...
    if ([dicContect count] > 3)
        m_panel	= NSGetAlertPanel([dicContect objectAtIndex:0],[dicContect objectAtIndex:1],
                                  [dicContect objectAtIndex:2],[dicContect objectAtIndex:3], @"");
    else
    if ([dicContect count] > 2)
        m_panel	= NSGetAlertPanel([dicContect objectAtIndex:0],[dicContect objectAtIndex:1],
                                  [dicContect objectAtIndex:2], @"",@"");
    else
    if ([dicContect count] > 1)
        m_panel	= NSGetAlertPanel([dicContect objectAtIndex:0],[dicContect objectAtIndex:1],@"", @"",@"");
    else
        return ;
    if(color)
