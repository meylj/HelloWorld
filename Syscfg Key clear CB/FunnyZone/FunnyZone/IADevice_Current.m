 ...
 ****************************************************************************************************/
// add by Stephen
- (NSNumber *)CALCULATEPOWER:(NSDictionary *)dicpara
				RETURN_VALUE:(NSMutableString *)strReturnValue
{  
    ...
    {
        NSString	*szVoltTemp	= [m_dicMemoryValues objectForKey:
								   [arrVolt objectAtIndex:i]];
        if ([szVoltTemp isEqualTo:@"NA"])
            return [NSNumber numberWithBool:NO];
        for (int j = 0; j != [arrCurrent count]; j++)
        {
            NSString	*szCurrentTemp	= [m_dicMemoryValues objectForKey:
										   [arrCurrent objectAtIndex:j]];
            if ([szCurrentTemp isEqualTo:@"NA"])
                continue;
            else
            {
                double		dVoltTemp		= [szVoltTemp doubleValue];
                double		dCurrentTemp	= [[m_dicMemoryValues objectForKey:
												[arrCurrent objectAtIndex:j]]
											   doubleValue];
                ...
                NSDictionary	*dicParaUpload		= [NSDictionary dictionaryWithObjectsAndKeys:
													   (szDownLimitTemp ? szDownLimitTemp : @""),
													   kFZ_Script_ParamLowLimit,
													   (szUpLimitTemp ? szUpLimitTemp : @""),
													   kFZ_SCript_ParamHighLimit,
													   @"mW",
													   kFZ_Script_ParamUnit,
													   (szParamatricDataNameTemp ? : @""),
													   kFZ_Script_UploadParametric,
													   [NSNumber numberWithBool:NO],
													   kFunnyZoneNOWriteCsvToLogFile, nil];
                if (szUpLimitTemp && szDownLimitTemp)
                {
                    NSString		*szSPEC		= [NSString stringWithFormat:@"[%@,%@]",
												   szDownLimitTemp, szUpLimitTemp];
                    ...
                    dicSpec	= [NSDictionary dictionaryWithObjectsAndKeys:
							   dicSpec,							kFZ_Script_JudgeCommonSpec ,
							   [NSNumber numberWithBool:YES],	@"ISNOSpec", nil];
                    NSMutableString	*szReturnValue	= [NSMutableString stringWithFormat:
													   @"%f", power];
                    numResult	= [self JUDGE_SPEC:dicSpec
                    ...
                    if (![numResult boolValue]
						&& szUpLimitTemp_CoCoPass
						&& szDownLimitTemp_CoCoPass)
                    {
                        //Initial CoCoPass Spec
                        NSString	*szCOCOPASS_SPEC	= [NSString stringWithFormat:
														   @"[%@,%@]",
														   szDownLimitTemp_CoCoPass,
														   szUpLimitTemp_CoCoPass];
                        ATSDebug(@"the CoCoPass Spec is %@", szCOCOPASS_SPEC);
                        NSDictionary	*dicCoCoSpec	= [NSDictionary dictionaryWithObject:szCOCOPASS_SPEC
																				forKey:kFZ_Script_JudgeCommonBlack];
                        dicCoCoSpec	= [NSDictionary dictionaryWithObjectsAndKeys:
									   dicCoCoSpec,						kFZ_Script_JudgeCommonSpec,
									   [NSNumber numberWithBool:YES],	@"ISNOSpec", nil];
                        //Judge CoCoPass Spec
                        numResult	= [self JUDGE_SPEC:dicCoCoSpec
										RETURN_VALUE:szReturnValue];
                        ...
                        if ([numResult boolValue])
                        {
                            ...
                            [m_dicMemoryValues setObject:myImage
												  forKey:@"ImageType"];
                        }
                    }
                }
                [self UPLOAD_PARAMETRIC:dicParaUpload
						   RETURN_VALUE:[NSString stringWithFormat:@"%f", power]];
            }
        }
    }
    return numResult;
