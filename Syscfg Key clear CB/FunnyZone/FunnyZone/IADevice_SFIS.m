		  ...
		  RETURN_VALUE:(NSMutableString *)szReturnValue
{
	...
	{
        // query sfc record.
        acpResult	= [postObj TrySFCQueryRecord:strQueryNum
        ...
        ATSDebug(@"Query URL : %@?c=query_record&sn=%@%@",
                 [m_dicMemoryValues objectForKey:descriptionOfGHStationInfo(IP_SFC_URL)], strQueryNum, szPtype);
        [szPtype release];
         ...
         RETURN_VALUE:(NSMutableString*)strReturn
{
    ...
    NSString	*szStartTime            = [[dateStationStartTime dateByAddingTimeInterval:-10] descriptionWithCalendarFormat:@"%Y-%m-%d %H:%M:%S"
                                                                                        timeZone:nil
                                                                                          locale:nil];
	
	NSString	*szStopTime             = [[dateStationStartTime dateByAddingTimeInterval:-5] descriptionWithCalendarFormat:@"%Y-%m-%d %H:%M:%S"
                                                                                        timeZone:nil
                                                                                          locale:nil];
    if (!szISN
        || !szURL
			  ...
			  RETURN_VALUE:(NSMutableString*)strReturnValue
{
    ...
    {   
		...
		rangeFrontEnd	= [szValue rangeOfString:@"0x400 :"];               // change from 200 to 400
		...
		rangeFrontStart	= [szValue rangeOfString:@"NVM Data 1024 bytes :"];    // change from 512 to 1024
		if(rangeFrontStart.location == NSNotFound
		   || rangeFrontStart.length <= 0 )
		{
			ATSDebug(@"Front NVM Format Error!");
    ...
    else if ([szKey isEqualToString:@"Back"])
    {
        ...
        rangeBackEnd	= [szValue rangeOfString:@"0xC00 :"];           // change from 800 to C00
        ...
        rangeBackStart	= [szValue rangeOfString:@"NVM Data 3072 bytes :"];   // change from 2048 to 3072
        if(rangeBackStart.location == NSNotFound
		   || rangeBackStart.length <=0 )
        {
            ATSDebug(@"Back NVM Format Error!");
