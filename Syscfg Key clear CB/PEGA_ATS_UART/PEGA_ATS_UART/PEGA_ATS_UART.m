					 ...
					 IsHex:(BOOL)bIsHex
{
    ...
    [delegate writeDebugLog:[NSString stringWithFormat:
							@"[%s](%s,%d) Para:%@%@,%d,%d;return value:%d!",
							 __FILE__,__FUNCTION__,__LINE__,inCommand,m_szEndSymbol,nPackNumber,nIntervalTime,nRet]];
    [dataCmd release];
	return nRet;
}
			   ...
			   TimeOut:(NSTimeInterval)inTimeOut;
{
				...
				else if ([aryMatchTbl count] > 0)
				{
					for (int i1 = [aryMatchTbl count]-1; i1 >= 0; i1 --)
					{
						...
						if ([self memmem:pSource
							SourceLength:[receiveBuffer length]
								  Symbol:pDest
	...
	[delegate writeDebugLog:[NSString stringWithFormat:
							 @"[%s](%s,%d) (Read_UartData:%@ TerminateSymbol:%@ MatchType:%d IntervalTime:%f TimeOut:%f);return value:%d!",
							 __FILE__,__FUNCTION__,__LINE__,outUartData,inArySymbols,inType,inIntervalTime,inTimeOut,nRet]];
	return nRet;
}
