...
extern "C"
{
	...
	///@param inHandle : the handle originally returned from IP_UUTStart()
	IP_API_Reply IP_addResult( IP_UUTHandle inHandle,
	...
	///@param inHandle : the handle originally returned from IP_UUTStart()
	IP_API_Reply IP_addAttribute( IP_UUTHandle inHandle,
								 const char* name,
								 const char* value );
	...
	IP_API_Reply IP_addBlob( IP_UUTHandle inHandle,
							const char* inBlobName,
							const char* inPathToBlobFile );
	
	IP_API_Reply IP_addBlobWithPolicy(IP_UUTHandle inHandle,
									  enum IP_ENUM_BLOBPOLICY eIPBlobPolicy,
									  const char* kcpBlobName,
	...
	IP_API_Reply IP_addBlobData( IP_UUTHandle inHandle,
								const char* inBlobName,
								const void* inBlobPointer,
								long inBlobLength );
	
	IP_API_Reply IP_addBlobDataWithPolicy( IP_UUTHandle inHandle,
										  enum IP_ENUM_BLOBPOLICY eIPBlobPolicy,
										  const char* kcpBlobName,
										  const void* kpBlobPointer,
										  long lBlobLength );
	...
	///@param UUTHandle : the handle originally returned from IP_UUTStart()
	IP_API_Reply IP_amIOkay( IP_UUTHandle inHandle,
							const char* inUUTSerialNumber );
	...
	bool IP_reply_isOfClass( IP_API_Reply replyToCheck,
							IP_MSG_FLAG classToCompare );
	...
	bool IP_testResult_setResult( void* testResultHandle,
								 enum IP_PASSFAILRESULT result );
	...
	///set the 'value' of the TestResult
	///@param testResultHandle : the handle originally returned from IP_testResult_create()
	bool IP_testResult_setValue(void* testResultHandle,
								const char* value,
								size_t valueLength );
	
	///set the 'value' of the TestResult
	///@param testResultHandle : the handle originally returned from IP_testResult_create()
	bool IP_testResult_setMessage(void* testResultHandle,
								  const char* message,
	...
	bool IP_testSpec_setTestName(IP_TestSpecHandle testSpecHandle,
								 const char* name,
								 size_t nameLength );

	///set the sub Test Name
	///@param testSpecHandle : the handle originally returned from IP_testSpec_create()
	bool IP_testSpec_setSubTestName(IP_TestSpecHandle testSpecHandle,
									const char* name,
									size_t nameLength );

	///set the sub-sub Test name
	///@param testSpecHandle : the handle originally returned from IP_testSpec_create()
	bool IP_testSpec_setSubSubTestName( IP_TestSpecHandle testSpecHandle,
									   const char* name,
									   size_t nameLength );

	///set the test limits
	///@param testSpecHandle : the handle originally returned from IP_testSpec_create()
	bool IP_testSpec_setLimits(IP_TestSpecHandle testSpecHandle,
							   const char* lowerLimit,
							   size_t lowerLimitLength, 
							   const char* upperLimit,
							   size_t upperLimitLength );

	///set the units that the test result is measured measured in
	///@param testSpecHandle : the handle originally returned from IP_testSpec_create()
	bool IP_testSpec_setUnits(IP_TestSpecHandle testSpecHandle,
							  const char* units,
							  size_t unitsLength );

	///set the PDCA analysis priority for this result
	///@param testSpecHandle : the handle originally returned from IP_testSpec_create()
	bool IP_testSpec_setPriority(IP_TestSpecHandle testSpecHandle,
								 enum IP_PDCA_PRIORITY priority );
	...
	IP_API_Reply IP_validateSerialNumber(IP_UUTHandle inHandle,
	...
	IP_API_Reply IP_getGHStationInfo(IP_UUTHandle handleGHStation,
									 enum IP_ENUM_GHSTATIONINFO eGHStationInfo,
									 char** cppValue,
									 size_t *sLength );
	
	///sets the DUT Position provided by the developer
	IP_API_Reply IP_setDUTPosition(IP_UUTHandle handleDUTPosition,
								   enum IP_ENUM_FIXTURE_ID eFixId,
								   enum IP_ENUM_FIXTURE_HEAD_ID eHeadId);
	///Twin API of IP_setDUTPosition(). 
	IP_API_Reply IP_setDUTPos(IP_UUTHandle handleDUTPosition,
							  const char* cpFixId,
							  const char* cpHeadId);
	...
	IP_API_Reply IP_setStartTime(IP_UUTHandle handleStartTime,
								 time_t rawTimeToUse );
	IP_API_Reply IP_setStopTime(IP_UUTHandle handleStopTime,
								time_t rawTimeToUse );
#ifdef __cplusplus
}
#endif
