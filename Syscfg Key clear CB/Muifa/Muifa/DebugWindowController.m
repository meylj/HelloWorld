...
#import "FunnyZone/NSStringCategory.h"
...
- (IBAction)ExportCommand:(id)sender
{
	NSAttributedString * strUartLogs = [m_Informations objectForKey:@"FunnyZoneUartLog"];
	NSString * currentDevice = @"";
	if (strUartLogs)
	{
		NSMutableString * strCommand = [[NSMutableString alloc] init];
		NSArray * arrayLines = [[strUartLogs string] componentsSeparatedByString:@"\n"];
		for (NSString * strLine in arrayLines)
		{
			if ([strLine contains:@"] (TX ==> ["])
			{
				NSString * thisDevice = [strLine subFrom:@"(TX ==> [" include:NO];
				thisDevice = [thisDevice SubTo:@"]) :" include:NO];
				// init current device
				if ([currentDevice isEqualToString:@""])
				{
					currentDevice =[NSString stringWithFormat:@"%@", thisDevice];
					[strCommand appendFormat:@"[%@]\n",thisDevice];
				}
				if ([currentDevice isNotEqualTo:thisDevice])
				{
					[strCommand appendFormat:@"\n[%@]\n",thisDevice];
					currentDevice =[NSString stringWithFormat:@"%@", thisDevice];
				}
				
				NSString * oneCommand = [strLine subFrom:@"]) : " include:NO];
				[strCommand appendFormat:@"%@\n",oneCommand];
			}
		}
		
		NSString * strFilePath = [NSString stringWithFormat:@"%@/Desktop/%@.txt",NSHomeDirectory(),[m_Informations objectForKey:@"ItemName"]];
		[strCommand writeToFile:strFilePath atomically:YES encoding:NSUTF8StringEncoding error:nil];
